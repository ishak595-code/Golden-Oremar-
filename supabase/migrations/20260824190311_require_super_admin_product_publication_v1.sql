...
as $$
declare exposure_requested boolean:=false;
begin
  ...
  if not exists(select 1 from public.content_entries e where e.related_product_id=new.id and e.content_type='product_health' and e.locale='tr' and e.status='published' and e.deleted_at is null) then raise exception 'published_product_health_required' using errcode='55000'; end if;
  perform private.write_admin_audit_v2('product.publication_authorized','product',new.id::text,case when tg_op='UPDATE' then jsonb_build_object('status',old.status,'isActive',old.is_active) else null end,jsonb_build_object('status',new.status,'isActive',new.is_active),jsonb_build_object('gate','product.publish','healthPackageVerified',true),null);
  return new;
end;$$;

drop trigger if exists enforce_super_admin_product_publication_v1 on public.products;
create trigger enforce_super_admin_product_publication_v1 before insert or update of status,is_active on public.products for each row execute function private.enforce_super_admin_product_publication_v1();

create or replace function private.protect_active_product_health_content_v1()
returns trigger language plpgsql security definer set search_path=''
as $$
declare product_id uuid:=old.related_product_id; remains_valid boolean:=false;
begin
  if old.content_type<>'product_health' or old.locale<>'tr' or old.status<>'published' or old.deleted_at is not null or product_id is null then if tg_op='DELETE' then return old; else return new; end if; end if;
  if not exists(select 1 from public.products p where p.id=product_id and p.status='published' and p.is_active=true and p.deleted_at is null) then if tg_op='DELETE' then return old; else return new; end if; end if;
  if tg_op='UPDATE' then remains_valid:=new.related_product_id=product_id and new.content_type='product_health' and new.locale='tr' and new.status='published' and new.deleted_at is null; end if;
...
create trigger protect_active_product_health_content_v1 before update of related_product_id,content_type,locale,status,deleted_at or delete on public.content_entries for each row execute function private.protect_active_product_health_content_v1();
...
begin
 ...
 select coalesce(jsonb_agg(jsonb_build_object('requestId',r.id,'productId',r.product_id,'productName',p.name,'productSlug',p.slug,'producerId',producer.id,'producerName',producer.display_name,'proposedBy',r.proposed_by,'payload',r.payload,'status',r.status,'createdAt',r.created_at,'updatedAt',r.updated_at) order by r.created_at),'[]'::jsonb) into result from private.product_health_change_requests r join public.products p on p.id=r.product_id join public.producers producer on producer.id=p.producer_id where r.status='pending';
 return result;
end;$$;
...
begin
 ...
 if entry.id is null then
   ...
   insert into public.content_entries(content_type,slug,title,summary,body_markdown,body_html_sanitized,author_user_id,related_product_id,status,locale,metadata,published_at) values('product_health',slug_value,p.name||' - ürün bilgileri',clean->>'summary','','',uid,p.id,'published','tr',jsonb_build_object('safetyV2',clean->'safety','productInfoV1',clean->'productInfo','recipeV1',clean->'recipe'),timezone('utc',now())) returning * into entry;
 else
