...
begin
  ...
  if jsonb_array_length(options)=0 then
    if application.applicant_type='individual' then
      options:=jsonb_build_array(jsonb_build_object('type','verified_individual','label','Kimliği doğrulanmış bireysel üretici','reference',null,'legalName',legal_name,'publicName',p.display_name,'verified',true));
    else
      options:=jsonb_build_array(jsonb_build_object('type','verified_business','label','Belgesi doğrulanmış işletme','reference',null,'legalName',legal_name,'publicName',coalesce(legal_name,p.display_name),'verified',true));
    end if;
  end if;
  return options;
end;
...
as $$
  select item
  from jsonb_array_elements(private.storefront_business_options_v1(p_producer_id)) item
  where item->>'type'=lower(btrim(coalesce(p_type,''))) and coalesce((item->>'verified')::boolean,false)=true
  limit 1;
$$;
...
declare
  p public.producers%rowtype;
  business jsonb;
  identity_ready boolean:=false;
  name_ready boolean:=false;
  about_ready boolean:=false;
  logo_ready boolean:=false;
  cover_ready boolean:=false;
  email_ready boolean:=false;
  phone_ready boolean:=false;
  address_ready boolean:=false;
  business_ready boolean:=false;
  ready boolean:=false;
  digits text;
begin
  select * into p from public.producers where id=p_producer_id and deleted_at is null;
  ...
  business_ready:=business is not null
    and coalesce(p.storefront_business_name,'')=coalesce(business->>'publicName','')
    and coalesce(p.storefront_business_reference,'')=coalesce(business->>'reference','')
    and p.storefront_business_verified_at is not null;
...
begin
  ...
  if p.store_kind='official' then
    select * into settings from public.brand_settings where slug='golden-oremar';
    identity:=coalesce(settings.public_config->'businessIdentity','{}'::jsonb);
    fallback_email:=settings.support_email;
    fallback_phone:=settings.support_phone;
    fallback_line1:=identity->>'registeredAddress';
    fallback_city:=p.production_district;
    fallback_region:=p.production_province;
    fallback_country:=coalesce(nullif(upper(identity->>'countryCode'),''),p.production_country_code);
  elsif p.application_id is not null then
    select * into kyc from private.producer_application_kyc where application_id=p.application_id;
    fallback_email:=kyc.contact_email;
    fallback_phone:=kyc.phone;
    ...
    fallback_line2:=coalesce(nullif(kyc.address->>'line2',''),nullif(kyc.address->>'addressLine2',''));
    fallback_postal:=coalesce(nullif(kyc.address->>'postalCode',''),nullif(kyc.address->>'postal_code',''));
    fallback_city:=coalesce(nullif(kyc.address->>'city',''),nullif(kyc.address->>'district',''),p.production_district);
    fallback_region:=coalesce(nullif(kyc.address->>'region',''),nullif(kyc.address->>'province',''),p.production_province);
    fallback_country:=upper(coalesce(nullif(kyc.address->>'countryCode',''),nullif(kyc.address->>'country_code',''),p.production_country_code));
  end if;
  options:=private.storefront_business_options_v1(p.id);

  return jsonb_build_object(
    'producerId',p.id,'storeKind',p.store_kind,'storefrontStatus',p.storefront_status,'publishedAt',p.storefront_published_at,
    'displayName',p.display_name,'description',p.description,'story',p.story,'headline',p.storefront_headline,'subheadline',p.storefront_subheadline,
    'logoPath',p.logo_path,'coverPath',p.cover_path,
    ...
    'businessIdentity',jsonb_build_object('type',p.storefront_business_identity_type,'name',p.storefront_business_name,'reference',p.storefront_business_reference,'verifiedAt',p.storefront_business_verified_at),
...
create or replace function public.get_storefront_setup_editor_v1(p_producer_id uuid)
returns jsonb
language sql
stable
set search_path=''
as $$ select private.get_storefront_setup_editor_v1(p_producer_id); $$;
revoke all on function public.get_storefront_setup_editor_v1(uuid) from public,anon;
grant execute on function public.get_storefront_setup_editor_v1(uuid) to authenticated;
...
begin
  ...
  display_name:=btrim(coalesce(p_payload->>'displayName',''));
  description:=btrim(coalesce(p_payload->>'description',''));
  story:=btrim(coalesce(p_payload->>'story',''));
  headline:=nullif(btrim(coalesce(p_payload->>'headline','')),'');
  subheadline:=nullif(btrim(coalesce(p_payload->>'subheadline','')),'');
  email:=lower(nullif(btrim(coalesce(p_payload->'contact'->>'email','')),''));
  phone:=nullif(btrim(coalesce(p_payload->'contact'->>'phone','')),'');
  website:=nullif(btrim(coalesce(p_payload->'contact'->>'websiteUrl','')),'');
  line1:=nullif(btrim(coalesce(p_payload->'address'->>'line1','')),'');
  line2:=nullif(btrim(coalesce(p_payload->'address'->>'line2','')),'');
  postal:=nullif(btrim(coalesce(p_payload->'address'->>'postalCode','')),'');
  city:=nullif(btrim(coalesce(p_payload->'address'->>'city','')),'');
  region:=nullif(btrim(coalesce(p_payload->'address'->>'region','')),'');
  country:=upper(nullif(btrim(coalesce(p_payload->'address'->>'countryCode','')),''));
  visibility:=lower(coalesce(nullif(btrim(p_payload->'address'->>'visibility'),''),'city_region'));
  business_type:=lower(nullif(btrim(coalesce(p_payload->>'businessIdentityType','')),''));
  ...
  options:=private.storefront_business_options_v1(p.id);
  if business_type is null and jsonb_array_length(options)>0 then business_type:=options->0->>'type'; end if;
  business:=private.storefront_business_selection_v1(p.id,business_type);

  update public.producers set
    display_name=display_name,
    description=description,
    story=story,
    storefront_headline=headline,
    storefront_subheadline=subheadline,
    storefront_contact_email=email,
    storefront_contact_phone=phone,
    storefront_website_url=website,
    storefront_address_line1=line1,
    storefront_address_line2=line2,
    storefront_postal_code=postal,
    storefront_city=city,
    storefront_region=region,
    storefront_country_code=country,
    storefront_address_visibility=visibility,
    storefront_business_identity_type=case when business is null then null else business->>'type' end,
    storefront_business_name=case when business is null then null else business->>'publicName' end,
    storefront_business_reference=case when business is null then null else nullif(business->>'reference','') end,
    storefront_business_verified_at=case when business is null then null else coalesce(storefront_business_verified_at,timezone('utc',now())) end,
    storefront_setup_version=1,
    updated_at=timezone('utc',now())
  where id=p.id;

  readiness:=private.storefront_readiness_v1(p.id);
  if old_status='published' and coalesce((readiness->>'ready')::boolean,false)=false then raise exception 'published_storefront_must_remain_ready' using errcode='55000'; end if;
  insert into private.outbox_events(aggregate_type,aggregate_id,event_type,payload)
  values('storefront',p.id,'storefront.updated',jsonb_build_object('producerId',p.id,'actorUserId',auth.uid(),'storefrontStatus',old_status,'ready',coalesce((readiness->>'ready')::boolean,false)));
  return private.get_storefront_setup_editor_v1(p.id);
end;
$$;
revoke all on function private.save_storefront_setup_v1(uuid,jsonb) from public,anon;
grant execute on function private.save_storefront_setup_v1(uuid,jsonb) to authenticated,service_role;

create or replace function public.save_storefront_setup_v1(p_producer_id uuid,p_payload jsonb)
returns jsonb
language sql
set search_path=''
as $$ select private.save_storefront_setup_v1(p_producer_id,p_payload); $$;
revoke all on function public.save_storefront_setup_v1(uuid,jsonb) from public,anon;
grant execute on function public.save_storefront_setup_v1(uuid,jsonb) to authenticated;
...
$$;
revoke all on function private.publish_storefront_v1(uuid) from public,anon;
grant execute on function private.publish_storefront_v1(uuid) to authenticated,service_role;

create or replace function public.publish_storefront_v1(p_producer_id uuid)
returns jsonb
language sql
set search_path=''
as $$ select private.publish_storefront_v1(p_producer_id); $$;
revoke all on function public.publish_storefront_v1(uuid) from public,anon;
grant execute on function public.publish_storefront_v1(uuid) to authenticated;

update public.producers p set
  storefront_status='published',
  storefront_published_at=coalesce(p.storefront_published_at,timezone('utc',now())),
  storefront_contact_email=coalesce(p.storefront_contact_email,b.support_email),
  storefront_contact_phone=coalesce(p.storefront_contact_phone,b.support_phone),
  storefront_address_line1=coalesce(p.storefront_address_line1,nullif(b.public_config->'businessIdentity'->>'registeredAddress','')),
  storefront_city=coalesce(p.storefront_city,p.production_district),
  storefront_region=coalesce(p.storefront_region,p.production_province),
  storefront_country_code=coalesce(p.storefront_country_code,nullif(upper(b.public_config->'businessIdentity'->>'countryCode'),''),p.production_country_code),
  storefront_address_visibility='full',
  ...
  storefront_business_name=coalesce(nullif(b.public_config->'businessIdentity'->>'registeredLegalName',''),b.legal_name,p.display_name),
  storefront_business_reference=coalesce(nullif(b.public_config->'businessIdentity'->>'mersisNumber',''),nullif(b.public_config->'businessIdentity'->>'tradeRegistryNumber','')),
  storefront_business_verified_at=case when lower(coalesce(b.public_config->'businessIdentity'->>'verificationStatus',''))='verified' then coalesce(p.storefront_business_verified_at,timezone('utc',now())) else p.storefront_business_verified_at end,
  storefront_setup_version=1,
  updated_at=timezone('utc',now())
from public.brand_settings b
where p.store_kind='official' and p.slug='golden-oremar' and p.deleted_at is null and b.slug='golden-oremar';
