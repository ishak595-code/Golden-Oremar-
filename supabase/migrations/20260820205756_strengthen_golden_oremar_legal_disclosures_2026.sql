...
$returns$,
    metadata=coalesce(metadata,'{}'::jsonb)||jsonb_build_object('legalStatus','prelaunch_pending_registered_business_identity','reviewedAgainst','Ticaret Bakanlığı Mesafeli Sözleşmeler bilgilendirmesi','reviewedAt','2026-08-20'),
...
$privacy$,
    metadata=coalesce(metadata,'{}'::jsonb)||jsonb_build_object('legalStatus','prelaunch_pending_registered_data_controller_identity','reviewedAgainst','KVKK Kanun m.10 ve Aydınlatma Tebliği','reviewedAt','2026-08-20'),
    updated_at=timezone('utc',now())
where content_type='legal' and slug='privacy' and locale='tr' and deleted_at is null;

update public.content_entries
set body_markdown=body_markdown||$terms_append$
...
where content_type='legal' and slug='terms' and locale='tr' and deleted_at is null
  and position('## 9. Production mesafeli satış açılış kapısı' in body_markdown)=0;
