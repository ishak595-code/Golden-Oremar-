...
begin
  ...
  if p_gift is not null then
    ...
    if char_length(recipient_name) not between 2 and 120 then raise exception 'gift_recipient_name_required' using errcode='22023'; end if;
    if recipient_phone is not null and char_length(recipient_phone) not between 7 and 40 then raise exception 'invalid_gift_recipient_phone' using errcode='22023'; end if;
    ...
    if sender_name is not null and char_length(sender_name)>120 then raise exception 'gift_sender_name_too_long' using errcode='22023'; end if;
  end if;
  ...
  if p_gift is not null then
    insert into private.order_gifts(order_id,user_id,recipient_name,recipient_phone,recipient_email,gift_message,sender_name,hide_price)
    values(order_id,caller_id,recipient_name,recipient_phone,recipient_email,gift_message,sender_name,hide_price)
    on conflict(order_id) do update set recipient_name=excluded.recipient_name,recipient_phone=excluded.recipient_phone,recipient_email=excluded.recipient_email,gift_message=excluded.gift_message,sender_name=excluded.sender_name,hide_price=excluded.hide_price,updated_at=timezone('utc',now());
  end if;
  return base||jsonb_build_object('gift',p_gift is not null);
end;
$function$;
