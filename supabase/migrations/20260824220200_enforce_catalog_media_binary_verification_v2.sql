-- Golden Oremar Task 3 final catalog media binary verification.
-- Keeps existing Storage/RLS ownership and publish gates, but requires a server-verified
-- binary signature before a catalog image may satisfy product media integrity.
...
create index if not exists catalog_media_binary_verifications_v2_path_idx
  on private.catalog_media_binary_verifications_v2(bucket_id,object_path);

create or replace function public.catalog_media_record_binary_verification_v2(
  p_path text,
...
begin
  ...
  if not (
    (detected='image/jpeg' and lower(normalized) ~ '\.(jpg|jpeg)$') or
    (detected='image/png' and lower(normalized) ~ '\.png$') or
    (detected='image/webp' and lower(normalized) ~ '\.webp$') or
    (detected='image/avif' and lower(normalized) ~ '\.avif$')
  ) then raise exception 'catalog_media_extension_mismatch' using errcode='22023'; end if;
  ...
  if coalesce(obj.metadata->>'size','') !~ '^[0-9]{1,12}$' or (obj.metadata->>'size')::bigint<>p_byte_size then
    raise exception 'catalog_media_metadata_size_mismatch' using errcode='22023';
  end if;

  insert into private.catalog_media_binary_verifications_v2(
    object_id,bucket_id,object_path,object_version,object_updated_at,detected_mime,byte_size,sha256,verified_by,verified_at
  ) values (
    obj.id,'catalog-public',normalized,obj.version,obj.updated_at,detected,p_byte_size,digest,p_verified_by,timezone('utc',now())
  )
  ...
  delete from private.catalog_media_binary_verifications_v2 v
  where v.bucket_id='catalog-public' and v.object_path=normalized and v.object_id<>obj.id;
...
begin
  ...
  if exists(
    select 1
    from storage.objects o
    join private.catalog_media_binary_verifications_v2 v on v.object_id=o.id
    where o.bucket_id='catalog-public'
      and o.name=normalized
      and coalesce(o.is_delete_marker,false)=false
      and o.archived_at is null
      and v.bucket_id='catalog-public'
      and v.object_path=normalized
      ...
      and v.byte_size between 1 and 10485760
      and v.sha256 ~ '^[0-9a-f]{64}$'
      and (
        (v.detected_mime='image/jpeg' and lower(normalized) ~ '\.(jpg|jpeg)$') or
        (v.detected_mime='image/png' and lower(normalized) ~ '\.png$') or
        (v.detected_mime='image/webp' and lower(normalized) ~ '\.webp$') or
        (v.detected_mime='image/avif' and lower(normalized) ~ '\.avif$')
      )
...
set search_path=''
as $$
  select private.catalog_media_binary_verified_path_v2(p_path);
$$;
revoke all on function private.verified_catalog_product_image_path_v1(text) from public,anon,authenticated;

create table if not exists private.catalog_media_integrity_state_v2 (
  singleton boolean primary key default true check(singleton),
  last_scanned_at timestamptz null,
  last_quarantined_count integer not null default 0 check(last_quarantined_count>=0)
);
...
begin
  ...
  insert into private.catalog_media_integrity_state_v2(singleton,last_scanned_at,last_quarantined_count)
  values(true,timezone('utc',now()),affected_count)
  on conflict(singleton) do update set last_scanned_at=excluded.last_scanned_at,last_quarantined_count=excluded.last_quarantined_count;
...
begin
  ...
  with image_rows as (
    select
      ...
      i.storage_path,
      private.verified_public_storage_path_v1('catalog-public',i.storage_path) as normalized_path,
      o.id as object_id,
      o.metadata,
      case
        ...
        when private.catalog_media_binary_verified_path_v2(i.storage_path) is null then 'unverified_binary'
        else 'healthy'
      end as status
    from public.product_images i
    join public.products p on p.id=i.product_id
    left join storage.objects o
      on o.bucket_id='catalog-public'
     and o.name=private.verified_public_storage_path_v1('catalog-public',i.storage_path)
     and coalesce(o.is_delete_marker,false)=false
     and o.archived_at is null
  ), orphan_rows as (
    select null::uuid as image_id,null::uuid as product_id,null::text as product_name,o.name as storage_path,o.name as normalized_path,o.id as object_id,o.metadata,'orphan_object'::text as status
    from storage.objects o
    where o.bucket_id='catalog-public'
      and coalesce(o.is_delete_marker,false)=false
      and o.archived_at is null
      ...
      and not exists(
        ...
        where private.verified_public_storage_path_v1('catalog-public',i.storage_path)=o.name
      )
  ), all_rows as (
    select * from image_rows union all select * from orphan_rows
  ), summary as (
    select
      count(*) filter(where status='healthy')::integer as healthy,
      count(*) filter(where status='missing_object')::integer as missing,
      count(*) filter(where status='orphan_object')::integer as orphan,
      count(*) filter(where status in ('invalid_path','invalid_mime','invalid_size','unverified_binary'))::integer as invalid,
      count(*)::integer as total
    from all_rows
  )
  select jsonb_build_object(
    'scannedAt',timezone('utc',now()),
    'lastScheduledScanAt',(select s.last_scanned_at from private.catalog_media_integrity_state_v2 s where s.singleton=true),
    'lastQuarantinedCount',coalesce((select s.last_quarantined_count from private.catalog_media_integrity_state_v2 s where s.singleton=true),0),
    'summary',jsonb_build_object('total',summary.total,'healthy',summary.healthy,'missing',summary.missing,'orphan',summary.orphan,'invalid',summary.invalid),
    'items',coalesce((select jsonb_agg(jsonb_build_object('status',r.status,'productId',r.product_id,'productName',r.product_name,'imageId',r.image_id,'path',r.storage_path) order by r.status,r.product_name nulls last,r.storage_path) from all_rows r where r.status<>'healthy'),'[]'::jsonb)
  ) into result
  from summary;
  return result;
end;
...
-- Record an initial scan state without publishing or deleting any product media.
