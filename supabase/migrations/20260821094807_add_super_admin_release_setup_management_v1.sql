create or replace function private.super_admin_get_release_setup_v1()
returns jsonb
language plpgsql
stable
security definer
...
begin
  ...
  origin_value:=nullif(btrim(coalesce(release_config#>>'{publicShareOrigin,url}','')),'');
  return_url_value:=nullif(btrim(coalesce(payment_config->>'return_url',release_config#>>'{payment,returnUrl}','')),'');
  return jsonb_build_object(
    ...
    'publicOrigin',origin_value,
    'publicOriginConfigured',origin_value is not null,
    'iyzicoReturnUrl',return_url_value,
    'iyzicoReturnUrlConfigured',return_url_value is not null,
    'iyzicoReturnPath',coalesce(nullif(release_config#>>'{payment,callbackPath}',''),'/payment/iyzico/return'),
    'transactionalEmailFrom',nullif(btrim(coalesce(release_config#>>'{transactionalEmail,from}','')),''),
    'fcmProjectId',nullif(btrim(coalesce(release_config#>>'{notifications,fcmProjectId}','')),''),
    'apnsBundleId',coalesce(nullif(btrim(coalesce(release_config#>>'{notifications,apnsBundleId}','')),''),coalesce(nullif(release_config->>'iosBundleId',''),'com.goldenoremar.app')),
    'googleOAuthClientId',nullif(btrim(coalesce(release_config#>>'{socialOAuth,googleClientId}','')),''),
    'facebookAppId',nullif(btrim(coalesce(release_config#>>'{socialOAuth,facebookAppId}','')),''),
    'secretsPolicy','server_only_never_store_in_public_config',
    'updatedAt',settings.updated_at
  );
end;
$$;
revoke all on function private.super_admin_get_release_setup_v1() from public,anon,authenticated;
grant execute on function private.super_admin_get_release_setup_v1() to postgres,service_role;
...
create or replace function public.super_admin_get_release_setup_v1()
returns jsonb language sql stable security invoker set search_path='' as $$ select private.super_admin_get_release_setup_v1(); $$;
revoke all on function public.super_admin_get_release_setup_v1() from public,anon;
grant execute on function public.super_admin_get_release_setup_v1() to authenticated;

create or replace function public.super_admin_update_release_setup_v1(
  p_public_origin text,
  p_iyzico_return_url text,
  p_transactional_email_from text,
  p_fcm_project_id text,
...
returns jsonb language sql security invoker set search_path='' as $$
 select private.super_admin_update_release_setup_v1(p_public_origin,p_iyzico_return_url,p_transactional_email_from,p_fcm_project_id,p_apns_bundle_id,p_google_oauth_client_id,p_facebook_app_id);
$$;
revoke all on function public.super_admin_update_release_setup_v1(text,text,text,text,text,text,text) from public,anon;
grant execute on function public.super_admin_update_release_setup_v1(text,text,text,text,text,text,text) to authenticated;
