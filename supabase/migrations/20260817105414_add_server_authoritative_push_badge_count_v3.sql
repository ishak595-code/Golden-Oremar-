create or replace function private.claim_push_deliveries_v3(
  p_limit integer,
  p_worker_id text,
  p_providers text[]
)
...
begin
  ...
  select updated.id,
         ...
         least(2147483647, coalesce(unread.total,0))::integer
    from updated
    ...
    left join lateral (
      ...
      from public.notifications pending_notification
      where pending_notification.user_id=updated.user_id
        and pending_notification.read_at is null
        and (pending_notification.expires_at is null or pending_notification.expires_at>timezone('utc',now()))
    ) unread on true;
end;
$$;

create or replace function public.claim_push_deliveries_v3(
  p_limit integer,
  p_worker_id text,
  p_providers text[]
)
...
set search_path to ''
as $$
  select * from private.claim_push_deliveries_v3(p_limit,p_worker_id,p_providers);
$$;

revoke all on function public.claim_push_deliveries_v3(integer,text,text[]) from public, anon, authenticated;
grant execute on function public.claim_push_deliveries_v3(integer,text,text[]) to service_role;
