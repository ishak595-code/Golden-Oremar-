...
set search_path=''
as $$
declare normalized text:=private.catalog_media_binary_verified_path_v2(p_path); kind text:=lower(btrim(coalesce(p_kind,'')));
begin
...
begin
  ...
  if not ((p.owner_user_id=caller_id and p.status in ('active','suspended')) or (p.store_kind='official' and coalesce(private.has_permission('product.publish'),false))) then raise exception 'store_branding_access_required' using errcode='42501'; end if;
...
set search_path=''
as $$
declare kind text:=lower(btrim(coalesce(p_kind,''))); normalized text; previous_path text; locked_id uuid;
begin
  ...
  if kind='logo' then select logo_path into previous_path from public.producers where id=p_producer_id; update public.producers set logo_path=normalized,updated_at=timezone('utc',now()) where id=p_producer_id;
  else select cover_path into previous_path from public.producers where id=p_producer_id; update public.producers set cover_path=normalized,updated_at=timezone('utc',now()) where id=p_producer_id; end if;
...
set search_path=''
as $$
declare caller_id uuid:=auth.uid(); producer public.producers%rowtype; clean_logo_path text:=nullif(btrim(coalesce(p_logo_path,'')),''); clean_cover_path text:=nullif(btrim(coalesce(p_cover_path,'')),'');
begin
  if caller_id is null then raise exception 'authentication_required' using errcode='42501'; end if;
  ...
  update public.producers set display_name=btrim(p_display_name),description=btrim(coalesce(p_description,'')),story=btrim(coalesce(p_story,'')),logo_path=clean_logo_path,cover_path=clean_cover_path,updated_at=timezone('utc',now()) where id=producer.id;
  return private.get_my_producer_profile_v1();
end;
$$;

create or replace function public.get_store_branding_editor_v1(p_producer_id uuid) returns jsonb language sql stable set search_path='' as $$ select private.get_store_branding_editor_v1(p_producer_id); $$;
create or replace function public.set_store_branding_asset_v1(p_producer_id uuid,p_kind text,p_path text) returns jsonb language sql set search_path='' as $$ select private.set_store_branding_asset_v1(p_producer_id,p_kind,p_path); $$;
...
create policy storage_catalog_brand_insert_owner_or_official_v1 on storage.objects for insert to authenticated with check (
  ...
  and exists(select 1 from public.producers p where p.id::text=(storage.foldername(name))[1] and p.deleted_at is null and p.status='active' and p.is_verified=true and (p.owner_user_id=(select auth.uid()) or (p.store_kind='official' and coalesce(private.has_permission('product.publish'),false))))
);

drop policy if exists storage_catalog_brand_delete_owner_or_official_v1 on storage.objects;
create policy storage_catalog_brand_delete_owner_or_official_v1 on storage.objects for delete to authenticated using (
  bucket_id='catalog-public' and (storage.foldername(name))[2]='profile'
  and exists(select 1 from public.producers p where p.id::text=(storage.foldername(name))[1] and p.deleted_at is null and (p.owner_user_id=(select auth.uid()) or (p.store_kind='official' and coalesce(private.has_permission('product.publish'),false))))
