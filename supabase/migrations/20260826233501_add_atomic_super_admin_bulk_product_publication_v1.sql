...
begin
  ...
  if p_product_ids is null or cardinality(p_product_ids)=0 then
    raise exception 'product_ids_required' using errcode='22023';
  end if;
  ...
  from unnest(p_product_ids) as u(value);
  if distinct_count<>requested_count then
    raise exception 'duplicate_product_ids' using errcode='22023';
  end if;
  ...
  if cardinality(candidate_ids)<>requested_count then
    raise exception 'bulk_product_set_mismatch' using errcode='55000';
  end if;
  if exists(
  ...
  perform private.write_admin_audit_v2(
    'product.bulk_atomic_approval_requested',
    'product_batch',
    caller_id::text,
    null,
    jsonb_build_object('requestedCount',requested_count,'approve',true,'atomic',true),
    jsonb_build_object('mode','super_admin_bulk_atomic','reason',clean_reason),
    null
  );
  ...
  perform private.write_admin_audit_v2(
    'product.bulk_atomic_approval_completed',
    'product_batch',
    caller_id::text,
    null,
    ...
    jsonb_build_object('mode','super_admin_bulk_atomic'),
    null
  );
...
revoke all on function private.super_admin_bulk_publish_products_atomic_v1(uuid[],text) from public,anon,authenticated,service_role;
grant execute on function private.super_admin_bulk_publish_products_atomic_v1(uuid[],text) to authenticated,service_role;

create or replace function public.super_admin_bulk_publish_products_atomic_v1(
  p_product_ids uuid[],
  p_reason text default null
)
returns jsonb
...
set search_path=''
as $$
  select private.super_admin_bulk_publish_products_atomic_v1(p_product_ids,p_reason);
$$;

revoke all on function public.super_admin_bulk_publish_products_atomic_v1(uuid[],text) from public,anon,authenticated,service_role;
grant execute on function public.super_admin_bulk_publish_products_atomic_v1(uuid[],text) to authenticated,service_role;
