...
begin
  for target in
    ...
    where u.created_at<timezone('utc',now())-interval '6 hours'
      ...
      and lower(coalesce(u.email,'')) ~ '^goldenoremar\+ci-e2e-[0-9]{1,24}(-[a-z0-9-]{1,24})?@gmail\.com$'
    order by u.created_at limit 100
  loop
    candidate_count:=candidate_count+1;
    begin
      delete from private.user_roles where user_id=target.id and role in('support','content_editor','operations','moderator','admin','super_admin');
      delete from auth.users where id=target.id;
      get diagnostics deleted_one=row_count;
      deleted_count:=deleted_count+deleted_one;
    exception when foreign_key_violation then skipped_count:=skipped_count+1;
    end;
  end loop;
  if candidate_count>0 then
    ...
    values(null,'ci.e2e_stale_user_gc','automation',null,jsonb_build_object('candidateCount',candidate_count,'deletedCount',deleted_count,'skippedForeignKeyCount',skipped_count,'minimumAgeHours',6,'batchLimit',100,'staffRoleCleanup',true),gen_random_uuid(),null,jsonb_build_object('candidateCount',candidate_count,'deletedCount',deleted_count,'skippedForeignKeyCount',skipped_count));
  end if;
