...
as $$
  select auth.uid() is not null and exists(
    ...
    where p.id=p_producer_id and p.deleted_at is null and p.is_verified=true
      and ((p.store_kind<>'official' and p.owner_user_id=(select auth.uid()) and p.status in ('pending','active')) or (p.store_kind='official' and p.status='active' and coalesce(private.has_permission('product.publish'),false)))
  );
$$;
...
begin
  ...
  if not ((p.store_kind<>'official' and p.owner_user_id=caller_id and p.status in ('pending','active','suspended')) or (p.store_kind='official' and coalesce(private.has_permission('product.publish'),false))) then raise exception 'store_branding_access_required' using errcode='42501'; end if;
  can_edit:=private.store_branding_can_edit_v1(p.id);
  return jsonb_build_object('producerId',p.id,'displayName',p.display_name,'storeKind',p.store_kind,'status',p.status,'verified',p.is_verified,'logoPath',p.logo_path,'coverPath',p.cover_path,'canEdit',can_edit,'logoBinaryVerified',private.catalog_media_binary_verified_path_v2(p.logo_path) is not null,'coverBinaryVerified',private.catalog_media_binary_verified_path_v2(p.cover_path) is not null);
end;
$$;

alter policy storage_catalog_brand_insert_owner_or_official_v1 on storage.objects
with check (
  bucket_id='catalog-public' and (storage.foldername(name))[2]='profile'
  ...
  and exists(select 1 from public.producers p where p.id::text=(storage.foldername(name))[1] and p.deleted_at is null and p.is_verified=true and ((p.store_kind<>'official' and p.owner_user_id=(select auth.uid()) and p.status in ('pending','active')) or (p.store_kind='official' and p.status='active' and coalesce(private.has_permission('product.publish'),false))))
);
...
as $$
declare
  p public.producers%rowtype; business jsonb; identity_ready boolean:=false; name_ready boolean:=false; about_ready boolean:=false; logo_ready boolean:=false; cover_ready boolean:=false; email_ready boolean:=false; phone_ready boolean:=false; address_ready boolean:=false; business_ready boolean:=false; ready boolean:=false; digits text;
begin
  select * into p from public.producers where id=p_producer_id and deleted_at is null;
  ...
  name_ready:=char_length(btrim(coalesce(p.display_name,''))) between 2 and 120; about_ready:=char_length(btrim(coalesce(p.description,''))) between 40 and 1200;
...
begin
  ...
  select * into application from public.producer_applications where id=p_application_id; if application.id is null then raise exception 'producer_application_not_found' using errcode='P0002'; end if;
  if p_status='approved' then
    ...
    for product_item in select value from jsonb_array_elements(application.planned_products) loop if not (coalesce(product_item->>'category','')=any(application.product_categories)) then raise exception 'planned_product_outside_category_scope' using errcode='55000'; end if; end loop;
  end if;
  ...
  if p_status='approved' then
    ...
    if producer_id is not null then
      update public.producers producer set status='pending',storefront_status='draft',storefront_published_at=null,storefront_published_by=null,production_country_code=application.production_country_code,production_province=application.production_province,production_district=application.production_district,production_village=application.production_village,production_village_is_custom=application.production_village_is_custom,production_latitude=application.production_latitude,production_longitude=application.production_longitude,origin_verified=true,origin_verified_at=timezone('utc',now()),origin_verification_basis='application_review',activity_types=application.activity_types,approved_category_slugs=application.product_categories,trust_badge_status='active',trust_badge_granted_at=timezone('utc',now()),trust_badge_review_due_at=coalesce(producer.verification_due_at,timezone('utc',now())+interval '365 days'),trust_badge_revoked_at=null,trust_badge_reason='KYC, üretim menşei, faaliyet alanı ve kategori kapsamı yönetim tarafından onaylandı.',updated_at=timezone('utc',now()) where producer.id=producer_id returning trust_badge_review_due_at into review_due;
      insert into private.producer_trust_badge_events(producer_id,actor_user_id,action,reason,review_due_at) values(producer_id,caller_id,'granted','KYC, üretim menşei, faaliyet alanı ve kategori kapsamı yönetim tarafından onaylandı.',review_due);
      update public.notifications n set title='Satıcı başvurunuz onaylandı',message='Doğrulamanız tamamlandı. Mağaza adınızı, profil ve kapak görsellerinizi, iletişim, adres ve işletme bilgilerinizi tamamlayıp mağazanızı yayınladıktan sonra ürün ekleyebilirsiniz.',action_url='/?tab=account' where n.id=(select n2.id from public.notifications n2 where n2.user_id=application.applicant_user_id and n2.metadata->>'application_id'=application.id::text and n2.metadata->>'status'='approved' order by n2.created_at desc limit 1);
      insert into private.outbox_events(aggregate_type,aggregate_id,event_type,payload) values('storefront',producer_id,'storefront.setup_required',jsonb_build_object('producerId',producer_id,'applicationId',application.id,'ownerUserId',application.applicant_user_id));
    end if;
  end if;
  return result;
end;
