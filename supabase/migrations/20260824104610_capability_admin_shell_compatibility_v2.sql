...
begin
  if caller_id is null then return jsonb_build_object('is_admin',false,'roles','[]'::jsonb,'capabilityBased',true); end if;
  ...
  if allowed and not has_legacy_admin_marker then
    roles:=roles||jsonb_build_array('admin');
  end if;

  return jsonb_build_object(
    'is_admin',allowed,
    'roles',case when allowed then roles else '[]'::jsonb end,
    'capabilityBased',true,
    'legacyAdminShellMarker',allowed and not has_legacy_admin_marker
  );
end;
$$;