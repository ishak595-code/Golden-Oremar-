...
const ADMIN_TABS=new Set(['dashboard','production-readiness','business-compliance','release-setup','appearance','official-store-products','product-health','products','product-approvals','product-removal','orders','returns','stock','shipping-readiness','finance','producer-payouts','payment-controls','transactional-emails','users','account-erasure','role-governance','system-errors','content','settings','categories','vendors','storefronts','store-follow-simulation','reviews','campaigns','notifications','vendor-applications','events','producer-event-submissions']);
function safeAdminTab(value:unknown){const tab=String(value||'dashboard').trim();return ADMIN_TABS.has(tab)?tab:'dashboard';}
