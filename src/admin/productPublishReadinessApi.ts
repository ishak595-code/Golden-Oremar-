...
export type PublishReadinessItem={productId:string;name:string;slug:string;producerName:string|null;status:string;active:boolean;published:boolean;readyToPublish:boolean;missingRealMedia:boolean;mandatoryDataMissing:boolean;mediaBlocked:boolean;ownerApprovalRequired:boolean;reasons:PublishReadinessReason[];updatedAt:string};
export type PublishReadinessSnapshot={scannedAt:string;summary:{total:number;published:number;readyToPublish:number;missingRealMedia:number;mandatoryDataMissing:number;mediaBlocked:number;ownerApprovalRequired:number};filteredTotal:number;limit:number;offset:number;items:PublishReadinessItem[]};
...
function normalize(value:unknown):PublishReadinessSnapshot{
 ...
 const summary={total:integer(value.summary.total,'Toplam ürün'),published:integer(value.summary.published,'Yayındaki ürün'),readyToPublish:integer(value.summary.readyToPublish,'Yayına hazır ürün'),missingRealMedia:integer(value.summary.missingRealMedia,'Gerçek görseli eksik ürün'),mandatoryDataMissing:integer(value.summary.mandatoryDataMissing,'Zorunlu verisi eksik ürün'),mediaBlocked:integer(value.summary.mediaBlocked,'Medya engelli ürün'),ownerApprovalRequired:integer(value.summary.ownerApprovalRequired,'Owner onayı gereken ürün')};
 for(const count of Object.values(summary))if(count>summary.total)throw new Error('Ürün yayın hazırlığı sayıları tutarsız.');
 const items=value.items.map((item,index)=>{if(!record(item))throw new Error(`${index+1}. ürün yayın hazırlığı doğrulanamadı.`);return{productId:text(item.productId,'Ürün kimliği',80) as string,name:text(item.name,'Ürün adı',300) as string,slug:text(item.slug,'Ürün slug',220) as string,producerName:text(item.producerName,'Üretici adı',240,true),status:text(item.status,'Ürün durumu',60) as string,active:bool(item.active,'Aktiflik'),published:bool(item.published,'Yayın durumu'),readyToPublish:bool(item.readyToPublish,'Yayın hazırlığı'),missingRealMedia:bool(item.missingRealMedia,'Gerçek medya durumu'),mandatoryDataMissing:bool(item.mandatoryDataMissing,'Zorunlu veri durumu'),mediaBlocked:bool(item.mediaBlocked,'Medya engeli'),ownerApprovalRequired:bool(item.ownerApprovalRequired,'Owner onayı'),reasons:reasons(item.reasons),updatedAt:date(item.updatedAt,'Güncelleme tarihi')};});
