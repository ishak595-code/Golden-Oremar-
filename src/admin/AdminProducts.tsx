...
function ratingLabel(value: unknown) {
  ...
  if (!Number.isFinite(rating) || rating < 0 || rating > 5) return 'Doğrulanamadı';
  return rating.toLocaleString('tr-TR', { maximumFractionDigits: 2 });
}
...
export function AdminProducts({ initialView = 'all' }: { setActiveTab?: (tab: string) => void; initialView?: 'all' | 'pending' }) {
  ...
  const productDetailRef = useAccessibleDialog<HTMLDivElement>(Boolean(selected), () => {
    if (!busyId && !reviewAction) setSelected(null);
  });
  const productActionRef = useAccessibleDialog<HTMLDivElement>(Boolean(reviewAction && selected), () => {
    if (!busyId) {
  ...
  const counts = useMemo(() => ({
    ...
    lowStock: products.filter(product => ['tracked', 'seasonal'].includes(product.stock_mode) && Number(product.available_quantity) <= 5).length,
    catalogIssues: products.filter(product => Number(product.catalog_issue_count) > 0).length,
    missingWeight: products.filter(product => Number(product.missing_weight_variant_count) > 0).length,
  }), [products]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-3 xl:grid-cols-6">
        ...
        <div className="rounded-2xl border border-gray-100 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800"><div className="text-xs text-gray-500">Düşük stok</div><div className="mt-1 text-2xl font-bold text-gray-900 dark:text-white">{counts.lowStock}</div></div><div className="rounded-2xl border border-gray-100 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800"><div className="text-xs text-gray-500">Veri kontrolü gereken</div><div className="mt-1 text-2xl font-bold text-gray-900 dark:text-white">{counts.catalogIssues}</div></div><div className="rounded-2xl border border-gray-100 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800"><div className="text-xs text-gray-500">Kargo ağırlığı eksik</div><div className="mt-1 text-2xl font-bold text-gray-900 dark:text-white">{counts.missingWeight}</div></div>
      </div>

      {error && !reviewAction && <div role="alert" aria-live="assertive" className="rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-800 dark:border-red-900/50 dark:bg-red-950/30 dark:text-red-200">{error}</div>}
      ...
      {reviewAction && selected && <div className="fixed inset-0 z-[60] flex items-end justify-center bg-black/65 p-0 sm:items-center sm:p-4" onMouseDown={event => { if (event.target === event.currentTarget && !busyId) { setReviewAction(null); setError(''); } }}><div ref={productActionRef} role="dialog" aria-modal="true" aria-labelledby="product-action-title" aria-describedby="product-action-summary" tabIndex={-1} className="w-full max-w-lg rounded-t-3xl bg-white p-5 shadow-2xl outline-none dark:bg-gray-800 sm:rounded-2xl"><h3 id="product-action-title" className="text-lg font-bold text-gray-900 dark:text-white">{reviewAction === 'approve' ? 'Ürünü yayınla' : 'Ürünü satıcıya geri gönder'}</h3><p id="product-action-summary" className="mt-1 text-sm text-gray-500">{selected.name}</p>{error && <div role="alert" aria-live="assertive" className="mt-4 rounded-xl border border-red-200 bg-red-50 p-3 text-sm text-red-800 dark:border-red-900/50 dark:bg-red-950/30 dark:text-red-200">{error}</div>}<form onSubmit={submitReview} className="mt-4 space-y-4"><label><span className="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-300">{reviewAction === 'reject' ? 'Düzeltme / ret gerekçesi' : 'İnceleme notu (isteğe bağlı)'}</span><textarea required={reviewAction === 'reject'} minLength={reviewAction === 'reject' ? 8 : undefined} maxLength={2000} rows={5} value={reason} onChange={event => setReason(event.target.value)} placeholder={reviewAction === 'reject' ? 'Satıcının hangi alanları düzeltmesi gerektiğini açıkça yazın...' : 'İsterseniz denetim notu ekleyin...'} className="w-full rounded-xl border border-gray-200 bg-gray-50 px-3 py-3 outline-none focus:ring-2 focus:ring-brand-green dark:border-gray-700 dark:bg-gray-900 dark:text-white" /></label><div className="flex gap-3"><button type="button" disabled={Boolean(busyId)} onClick={() => { setReviewAction(null); setError(''); }} className="min-h-11 flex-1 rounded-xl px-4 text-gray-700 hover:bg-gray-100 disabled:opacity-50 dark:text-gray-200 dark:hover:bg-gray-700">İptal</button><button type="submit" disabled={Boolean(busyId) || (reviewAction === 'reject' && reason.trim().length < 8)} className={`min-h-11 flex-1 rounded-xl px-4 font-semibold text-white disabled:opacity-50 ${reviewAction === 'reject' ? 'bg-red-700 hover:bg-red-800' : 'bg-brand-green hover:bg-green-700'}`}>{busyId ? <span className="flex items-center justify-center gap-2"><Loader2 className="h-4 w-4 animate-spin" aria-hidden="true" /> İşleniyor</span> : 'Onayla'}</button></div></form></div></div>}
