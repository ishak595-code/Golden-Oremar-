...
export type PaymentRuntimeHealth={
  provider:'iyzico';
  runtime:{iyzicoSecretsConfigured:boolean;livePaymentsEnabled:boolean};
...
export async function getPaymentRuntimeHealth():Promise<PaymentRuntimeHealth>{
 ...
 return{provider:'iyzico',runtime:{iyzicoSecretsConfigured:bool(data.runtime.iyzicoSecretsConfigured,'iyzico gizli anahtarları'),livePaymentsEnabled:bool(data.runtime.livePaymentsEnabled,'Canlı ödeme anahtarı')},methods:{hostedCheckout:method(data.methods.hostedCheckout,'Uygulama içi iyzico ödeme'),savedCardPayment:method(data.methods.savedCardPayment,'Kayıtlı kartla ödeme'),cardEnrollment:method(data.methods.cardEnrollment,'Kart kaydetme')}};
}
