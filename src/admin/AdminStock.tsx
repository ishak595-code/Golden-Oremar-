...
function producerStatusLabel(status: string) {
  return ({ active: 'Aktif', pending: 'Onay bekliyor', suspended: 'Askıda', rejected: 'Reddedildi' } as Record<string, string>)[status] || status;
}
...
export function AdminStock({ setActiveTab }: { setActiveTab?: (tab: string) => void }) {
  ...
  const dialogRef = useAccessibleDialog<HTMLElement>(Boolean(selected), () => setSelected(null));
