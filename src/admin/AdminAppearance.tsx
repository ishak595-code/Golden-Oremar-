...
export default function AdminAppearance(){
 ...
 return<div className="space-y-6">
  <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><div className="flex items-center gap-2 text-sm font-bold uppercase tracking-wider text-brand-gold"><Palette className="h-4 w-4" aria-hidden="true"/>Süper Yönetici</div><h2 className="mt-1 text-2xl font-bold text-gray-900 dark:text-white">Görünüm ve Marka Teması</h2><p className="mt-2 max-w-3xl text-sm text-gray-500 dark:text-gray-400">Arka plan, kart yüzeyi, metinler ve marka renklerini kod açmadan yönetir. Sunucu, kritik metin ve buton kontrastlarını en az 4.5:1 olarak zorunlu tutar.</p></div><button type="button" onClick={()=>void load()} disabled={saving} className="min-h-11 rounded-xl border px-4 font-semibold disabled:opacity-50"><RefreshCw className="mr-2 inline h-4 w-4" aria-hidden="true"/>Canlı değeri yenile</button></header>
