...
export function messageModerationAdminError(error:unknown,fallback='Mesaj güvenliği ayarı tamamlanamadı.'){const message=error instanceof Error?error.message:String((error as any)?.message??'');if(message.includes('super_admin_required'))return'Bu bölüm yalnız Süper Yönetici tarafından yönetilebilir.';if(message.includes('message_moderation_configuration_missing'))return'Mesaj güvenliği yapılandırması bulunamadı.';if(message.includes('invalid_custom_blocked'))return'Özel engelli ifadeler 2-80 karakter olmalı ve en fazla 100 kayıt içermelidir.';if(message.includes('invalid_message_attachment_limit'))return'Maksimum ek sayısı 0 ile 5 arasında olmalıdır.';if(message.includes('invalid_message_attachment_size'))return'Maksimum dosya boyutu 1 ile 20 MB arasında olmalıdır.';return message&&message.length<=280?message:fallback;}
