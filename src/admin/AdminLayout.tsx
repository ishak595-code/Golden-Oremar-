...
const ADMIN_MENU_GROUPS: MenuGroup[] = [
  ...
  { title: 'E-Ticaret', items: [
    ...
    { id: 'product-health', label: 'Ürün İçerik ve Sağlık', icon: FileText },
    ...
    { id: 'storefronts', label: 'Premium Mağaza Vitrinleri', icon: Sparkles },
    { id: 'store-follow-simulation', label: 'Takip Simülasyonu', icon: Users },
  ...
  { title: 'Pazarlama ve Finans', items: [
    ...
    { id: 'producer-payouts', label: 'Satıcı Ödeme ve IBAN', icon: DollarSign },
    { id: 'payment-controls', label: 'Ödeme Altyapısı', icon: CreditCard },
    { id: 'transactional-emails', label: 'Makbuz E-posta Kuyruğu', icon: Mail },
    { id: 'notifications', label: 'Bildirim Merkezi', icon: Bell },
  ]},
  { title: 'Sahip ve Güvenlik', items: [
    { id: 'business-compliance', label: 'İşletme ve Yasal Uyum', icon: FileText },
  ...
  { title: 'İçerik ve Sistem', items: [
    ...
    { id: 'producer-event-submissions', label: 'Köylü Etkinlik Onayları', icon: CheckSquare },
...
export function AdminLayout({ children, activeTab, setActiveTab, onLogout, onBack }: AdminLayoutProps) {
  ...
  const { loading: authorizationLoading, snapshot, can } = useAuthorization();
  const sidebarRef = useAccessibleDialog<HTMLElement>(isSidebarOpen, () => setIsSidebarOpen(false));
  const visibleMenuGroups = useMemo(() => ADMIN_MENU_GROUPS.map(group => ({ ...group, items: group.items.filter(item => can(permissionForAdminTab(item.id))) })).filter(group => group.items.length > 0), [can]);
  ...
  useEffect(() => {
    ...
    if(!can(permissionForAdminTab(activeTab))){const fallback=firstAllowedAdminTab(can);if(fallback&&fallback!==activeTab)setActiveTab(fallback);}
  }, [activeTab, authorizationLoading, can, setActiveTab]);
