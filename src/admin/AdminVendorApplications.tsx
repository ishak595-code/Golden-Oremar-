...
export function AdminVendorApplications() {
  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? <div role="status" className="flex min-h-40 items-center justify-center gap-2 rounded-2xl border border-gray-100 bg-white text-gray-500 dark:border-gray-700 dark:bg-gray-800"><Loader2 className="h-5 w-5 animate-spin" aria-hidden="true" /> Başvurular yükleniyor...</div> : <div className="grid gap-4 xl:grid-cols-2">
      ...
      {selected && <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/60 p-0 backdrop-blur-sm sm:items-center sm:p-4" onMouseDown={event => { if (event.target === event.currentTarget && !busy && !sensitiveLoading) { setSelected(null); setSensitive(null); } }}><section role="dialog" aria-modal="true" aria-labelledby="application-title" className="max-h-[96dvh] w-full max-w-4xl overflow-y-auto rounded-t-3xl bg-white p-5 shadow-2xl dark:bg-gray-800 sm:rounded-2xl sm:p-6"><div className="flex items-start justify-between gap-4"><div><h3 id="application-title" className="text-xl font-bold text-gray-900 dark:text-white">{selected.brand_name}</h3><p className="mt-1 text-sm text-gray-500">{selected.legal_name || selected.public_name || selected.email}</p></div><button type="button" onClick={() => { setSelected(null); setSensitive(null); }} className="min-h-11 min-w-11 rounded-xl p-2 text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-700" aria-label="Başvuru detayını kapat"><X className="mx-auto h-5 w-5" aria-hidden="true" /></button></div>
        ...
        <section className="mt-4 rounded-2xl border border-gray-100 p-4 dark:border-gray-700"><div className="flex items-center justify-between gap-3"><div><h4 className="font-bold text-gray-900 dark:text-white">Belgeler</h4><p className="mt-1 text-xs text-gray-500">Önizlemeler 5 dakikalık özel bağlantıyla açılır. Onay için zorunlu belgelerin doğrulanmış olması gerekir.</p></div><FileSearch className="h-5 w-5 text-gray-400" aria-hidden="true" /></div><div className="mt-4 space-y-2">{selected.documents.map(document => <div key={document.id} className="flex flex-col gap-3 rounded-xl bg-gray-50 p-3 dark:bg-gray-900/60 sm:flex-row sm:items-center sm:justify-between"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className="font-medium text-gray-900 dark:text-white">{document.document_type}</span><span className={`rounded-full px-2 py-0.5 text-xs font-semibold ${documentStatusClass(document.verification_status)}`}>{documentStatusLabel(document.verification_status)}</span></div><p className="mt-1 truncate text-xs text-gray-500">{document.mime_type || 'dosya'} · {document.size_bytes ? `${Math.ceil(document.size_bytes / 1024)} KB` : 'boyut bilinmiyor'}</p></div><div className="flex flex-wrap gap-2"><button type="button" disabled={Boolean(busy)} onClick={() => void previewDocument(document)} className="min-h-11 rounded-lg border border-gray-200 px-3 py-2 text-sm font-medium text-gray-700 hover:bg-white disabled:opacity-50 dark:border-gray-700 dark:text-gray-200 dark:hover:bg-gray-800">Önizle</button><button type="button" disabled={Boolean(busy) || document.verification_status === 'verified'} onClick={() => void updateDocument(document, 'verified')} className="min-h-11 rounded-lg border border-green-200 px-3 py-2 text-sm font-medium text-green-700 hover:bg-green-50 disabled:opacity-40 dark:border-green-900/50 dark:text-green-300">Doğrula</button><button type="button" disabled={Boolean(busy) || document.verification_status === 'rejected'} onClick={() => void updateDocument(document, 'rejected')} className="min-h-11 rounded-lg border border-red-200 px-3 py-2 text-sm font-medium text-red-700 hover:bg-red-50 disabled:opacity-40 dark:border-red-900/50 dark:text-red-300">Reddet</button></div></div>)}{selected.documents.length === 0 && <p className="py-4 text-center text-sm text-gray-500">Başvuruya yüklenmiş belge yok.</p>}</div></section>

        <section className="mt-4 rounded-2xl border border-amber-100 bg-amber-50 p-4 dark:border-amber-900/40 dark:bg-amber-950/20"><div className="flex items-start gap-3"><ShieldCheck className="mt-0.5 h-5 w-5 shrink-0 text-amber-700 dark:text-amber-300" aria-hidden="true" /><div className="flex-1"><h4 className="font-bold text-amber-950 dark:text-amber-100">Hassas KYC erişimi</h4><p className="mt-1 text-xs text-amber-800 dark:text-amber-200">Tam kimlik, vergi, IBAN ve sertifika numaraları varsayılan olarak gizlidir. Açmak için denetim kaydına yazılacak erişim amacını belirtin.</p>{!sensitive ? <form onSubmit={unlockSensitive} className="mt-3 flex flex-col gap-2 sm:flex-row"><input required minLength={10} maxLength={200} value={sensitivePurpose} onChange={event => setSensitivePurpose(event.target.value)} placeholder="Örn. Satıcı KYC belge doğrulaması için..." className="min-h-11 flex-1 rounded-xl border border-amber-200 bg-white px-3 outline-none focus:ring-2 focus:ring-amber-500 dark:border-amber-900 dark:bg-gray-900 dark:text-white" /><button type="submit" disabled={sensitiveLoading || sensitivePurpose.trim().length < 10} className="min-h-11 rounded-xl bg-amber-700 px-4 font-semibold text-white disabled:opacity-50">{sensitiveLoading ? 'Açılıyor...' : 'Hassas bilgileri aç'}</button></form> : <div className="mt-4 grid gap-3 text-sm sm:grid-cols-2"><div><span className="text-xs text-amber-700 dark:text-amber-300">Kimlik</span><div className="break-all font-medium">{sensitive.national_id || 'Yok'}</div></div><div><span className="text-xs text-amber-700 dark:text-amber-300">Vergi no</span><div className="break-all font-medium">{sensitive.tax_number || 'Yok'}</div></div><div><span className="text-xs text-amber-700 dark:text-amber-300">IBAN</span><div className="break-all font-medium">{sensitive.iban || 'Yok'}</div></div><div><span className="text-xs text-amber-700 dark:text-amber-300">Hesap sahibi</span><div className="font-medium">{sensitive.bank_account_holder || 'Yok'}</div></div><div><span className="text-xs text-amber-700 dark:text-amber-300">Gıda kayıt no</span><div className="break-all font-medium">{sensitive.food_registration_number || 'Yok'}</div></div><div><span className="text-xs text-amber-700 dark:text-amber-300">Organik sertifika no</span><div className="break-all font-medium">{sensitive.organic_certificate_number || 'Yok'}</div></div><button type="button" onClick={() => { setSensitive(null); setSensitivePurpose(''); }} className="min-h-11 rounded-xl border border-amber-300 px-3 font-medium text-amber-900 dark:text-amber-100 sm:col-span-2">Hassas bilgileri kapat</button></div>}</div></div></section>
