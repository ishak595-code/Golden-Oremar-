...
export function AdminContent({ setActiveTab }: { setActiveTab?: (tab: string) => void }) {
  ...
  return <div className="space-y-6">
    ...
    <section className="rounded-2xl border border-gray-100 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800"><div className="grid gap-3 border-b p-4 dark:border-gray-700 md:grid-cols-[minmax(0,1fr)_220px]"><label className="relative"><span className="sr-only">İçerik ara</span><Search className="absolute left-3 top-1/2 h-5 w-5 -translate-y-1/2 text-gray-400"/><input type="search" value={searchTerm} onChange={event => setSearchTerm(event.target.value)} placeholder="Başlık, özet, ürün veya kısa ad ara..." className="min-h-11 w-full rounded-xl border bg-gray-50 pl-10 pr-3 dark:border-gray-700 dark:bg-gray-900 dark:text-white"/></label><label><span className="sr-only">İçerik türü</span><select value={typeFilter} onChange={event => setTypeFilter(event.target.value as typeof typeFilter)} className="min-h-11 w-full rounded-xl border bg-gray-50 px-3 dark:border-gray-700 dark:bg-gray-900 dark:text-white"><option value="all">Tüm içerikler</option><option value="blog">Sağlık yazıları</option><option value="recipe">Tarifler</option><option value="health_guide">Sağlık rehberleri</option><option value="product_health">Ürün sağlık bilgileri</option></select></label></div>{loading ? <div role="status" className="flex min-h-40 items-center justify-center gap-2 text-gray-500"><Loader2 className="h-5 w-5 animate-spin"/>İçerikler yükleniyor...</div> : <div className="divide-y dark:divide-gray-700">{filtered.map(entry => <article key={entry.id} className="flex flex-col gap-3 p-4 lg:flex-row lg:items-center"><div className="flex min-w-0 flex-1 items-start gap-3"><div className="rounded-xl bg-brand-green/10 p-2.5 text-brand-green">{typeIcon(entry.content_type)}</div><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h3 className="font-bold text-gray-900 dark:text-white">{entry.title}</h3><span className="rounded-full bg-gray-100 px-2.5 py-1 text-xs dark:bg-gray-700">{typeLabel(entry.content_type)}</span></div><p className="mt-1 text-xs text-gray-500">/{entry.slug} · {entry.locale.toUpperCase()} · {entry.updated_at ? new Date(entry.updated_at).toLocaleString('tr-TR') : 'Tarih yok'}</p><p className="mt-2 line-clamp-2 text-sm text-gray-600 dark:text-gray-300">{entry.summary || entry.content || 'İçerik metni yok'}</p>{entry.related_product_name && <p className="mt-1 text-xs font-medium text-blue-700 dark:text-blue-300">İlişkili ürün: {entry.related_product_name}</p>}</div></div><div className="flex gap-2"><button type="button" onClick={() => openEdit(entry)} className="min-h-11 rounded-xl border px-4 font-semibold text-blue-700 dark:border-gray-700 dark:text-blue-300"><Edit2 className="mr-2 inline h-4 w-4"/>Düzenle</button><button type="button" onClick={() => { setError(''); setArchiveTarget(entry); }} className="min-h-11 rounded-xl border border-red-200 px-4 font-semibold text-red-700"><Trash2 className="mr-2 inline h-4 w-4"/>Arşivle</button></div></article>)}{filtered.length === 0 && <div className="p-10 text-center text-gray-500">İçerik bulunamadı.</div>}</div>}</section>

    {editorOpen && <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/60 sm:items-center sm:p-4" onMouseDown={event => { if (event.target === event.currentTarget) closeEditor(); }}><section role="dialog" aria-modal="true" aria-labelledby="content-editor-title" className="max-h-[96dvh] w-full max-w-3xl overflow-y-auto rounded-t-3xl bg-white p-5 shadow-2xl dark:bg-gray-800 sm:rounded-2xl"><div className="flex justify-between gap-3"><div><h3 id="content-editor-title" className="text-lg font-bold dark:text-white">{editing ? 'İçeriği düzenle' : 'Yeni içerik'}</h3><p className="mt-1 text-xs text-gray-500">Sağlık metinlerinde tedavi vaadi veya doğrulanmamış sağlık sonucu yazmayın.</p></div><button type="button" disabled={Boolean(busy)} onClick={closeEditor} className="min-h-11 min-w-11 rounded-xl" aria-label="Kapat"><X className="mx-auto h-5 w-5"/></button></div><form onSubmit={submit} className="mt-5 space-y-4"><div className="grid gap-4 sm:grid-cols-2"><Field label="İçerik türü"><select disabled={Boolean(editing)} value={form.type} onChange={event => setForm({...form,type:event.target.value as AdminContentType,relatedProductId:''})}><option value="blog">Sağlık yazısı</option><option value="recipe">Tarif</option><option value="health_guide">Sağlık rehberi</option><option value="product_health">Ürün sağlık bilgisi</option></select></Field><Field label="Tarih"><input type="date" value={form.date} onChange={event => setForm({...form,date:event.target.value})}/></Field></div><Field label="Başlık"><input autoFocus required minLength={2} maxLength={240} value={form.title} onChange={event => setForm({...form,title:event.target.value})}/></Field><Field label="Özet"><textarea rows={3} maxLength={2000} value={form.summary} onChange={event => setForm({...form,summary:event.target.value})}/></Field><Field label="İçerik"><textarea required rows={10} maxLength={200000} value={form.content} onChange={event => setForm({...form,content:event.target.value})}/></Field><div className="grid gap-4 sm:grid-cols-2"><Field label="Kategori etiketi"><input maxLength={120} value={form.category} onChange={event => setForm({...form,category:event.target.value})}/></Field><Field label="Kalıcı görsel yolu veya HTTPS URL"><input maxLength={2048} value={form.image} onChange={event => setForm({...form,image:event.target.value})}/></Field></div>{form.type === 'product_health' && <Field label="İlişkili ürün"><select required value={form.relatedProductId} onChange={event => setForm({...form,relatedProductId:event.target.value})}><option value="">Ürün seçin</option>{products.map(product => <option key={product.id} value={product.id}>{product.name} - {product.producer_name}</option>)}</select></Field>}{error && <div role="alert" className="rounded-xl border border-red-200 bg-red-50 p-3 text-sm text-red-800">{error}</div>}<div className="flex gap-3"><button type="button" disabled={Boolean(busy)} onClick={closeEditor} className="min-h-11 flex-1 rounded-xl border dark:border-gray-700">Vazgeç</button><button type="submit" disabled={Boolean(busy)} className="min-h-11 flex-1 rounded-xl bg-brand-green font-semibold text-white disabled:opacity-50">{busy ? 'Kaydediliyor...' : 'Yayınla'}</button></div></form></section></div>}

    {archiveTarget && <div className="fixed inset-0 z-[60] flex items-end justify-center bg-black/65 sm:items-center sm:p-4"><section role="dialog" aria-modal="true" className="w-full max-w-md rounded-t-3xl bg-white p-5 shadow-2xl dark:bg-gray-800 sm:rounded-2xl"><h3 className="text-lg font-bold dark:text-white">İçeriği arşivle</h3><p className="mt-2 text-sm text-gray-500"><strong>{archiveTarget.title}</strong> müşteri içerik kütüphanesinden kaldırılacak. Kayıt denetim için korunur.</p>{error && <div role="alert" className="mt-4 rounded-xl border border-red-200 bg-red-50 p-3 text-sm text-red-800">{error}</div>}<div className="mt-5 flex gap-3"><button type="button" disabled={Boolean(busy)} onClick={() => { setArchiveTarget(null); setError(''); }} className="min-h-11 flex-1 rounded-xl border dark:border-gray-700">Vazgeç</button><button type="button" disabled={Boolean(busy)} onClick={() => void archive()} className="min-h-11 flex-1 rounded-xl bg-red-700 font-semibold text-white disabled:opacity-50">Arşivle</button></div></section></div>}
...
function Field({label,children}:{label:string;children:React.ReactElement}){return <label className="block"><span className="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-300">{label}</span>{React.cloneElement(children,{className:'content-field'})}</label>}
