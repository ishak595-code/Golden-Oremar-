...
export function AdminProductHealth(){
 ...
 return<div className="space-y-6">
  <header className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between"><div><div className="flex items-center gap-2"><ShieldCheck aria-hidden="true" className="h-6 w-6 text-brand-green"/><h2 className="text-2xl font-black">Ürün İçerik ve Sağlık Kontrolü</h2></div><p className="mt-2 max-w-3xl text-sm text-gray-500">Süper Yönetici bütün satıcıların ürün içeriğini, sağlık bilgisini ve yayın hazırlığını tek yüzeyde görür. Müşteriye teknik gate ifadeleri gösterilmez ve yayın kararı yalnız mevcut owner publish komutundan geçer.</p></div><button type="button" onClick={()=>void load()} disabled={loading} className="min-h-11 rounded-xl border px-4 font-semibold disabled:opacity-50"><RefreshCw aria-hidden="true" className={`mr-2 inline h-4 w-4 ${loading?'animate-spin':''}`}/>Yenile</button></header>
