...
import { Check, CheckCircle, Clock3, CreditCard, Eye, Loader2, MapPin, Package, RefreshCw, RotateCcw, Search, ShoppingBag, Truck, X, XCircle } from 'lucide-react';
import {
...
export function AdminOrders({ setActiveTab }: { setActiveTab?: (tab: string) => void }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {error && !transition && <div role="alert" className="rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-800 dark:border-red-900/50 dark:bg-red-950/30 dark:text-red-200">{error}</div>}
      ...
      {selected && <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/60 p-0 backdrop-blur-sm sm:items-center sm:p-4" onMouseDown={event => { if (event.target === event.currentTarget && !busyId) setSelected(null); }}><section role="dialog" aria-modal="true" aria-labelledby="order-detail-title" className="max-h-[96dvh] w-full max-w-4xl overflow-y-auto rounded-t-3xl bg-white p-5 shadow-2xl dark:bg-gray-800 sm:rounded-2xl sm:p-6"><div className="flex items-start justify-between gap-4"><div><h3 id="order-detail-title" className="text-xl font-bold text-gray-900 dark:text-white">{selected.orderNumber}</h3><p className="mt-1 text-sm text-gray-500">{new Date(selected.date).toLocaleString('tr-TR')} · {statusLabel(selected.status)}</p></div><button type="button" onClick={() => setSelected(null)} className="min-h-11 min-w-11 rounded-xl p-2 text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-700" aria-label="Sipariş detayını kapat"><X className="mx-auto h-5 w-5" aria-hidden="true" /></button></div>
      ...
      {transition && selected && <div className="fixed inset-0 z-[60] flex items-end justify-center bg-black/65 p-0 sm:items-center sm:p-4"><section role="dialog" aria-modal="true" aria-labelledby="order-transition-title" className="w-full max-w-lg rounded-t-3xl bg-white p-5 shadow-2xl dark:bg-gray-800 sm:rounded-2xl"><h3 id="order-transition-title" className="text-lg font-bold text-gray-900 dark:text-white">{transitionLabel(transition)}</h3><p className="mt-1 text-sm text-gray-500">{selected.orderNumber} · {statusLabel(selected.status)} → {statusLabel(transition)}</p>{transition === 'completed' && <div className="mt-4 rounded-xl bg-emerald-50 p-3 text-sm text-emerald-900 dark:bg-emerald-950/30 dark:text-emerald-100">Bu adım satıcı satış kayıtlarının bekleyen hakedişlerini otomatik olarak kullanılabilir duruma geçirir. Ödeme sağlayıcısı doğrulanmadan bu noktaya gelinemez.</div>}{transition === 'cancelled' && selected.paymentStatus !== 'unpaid' && <div className="mt-4 rounded-xl bg-red-50 p-3 text-sm text-red-900 dark:bg-red-950/30 dark:text-red-100">Bu siparişin ödeme durumu {paymentLabel(selected.paymentStatus)}. Ödemesi alınmış sipariş doğrudan iptal edilemez, geri ödeme akışı kullanılmalıdır.</div>}{error && <div role="alert" className="mt-4 rounded-xl border border-red-200 bg-red-50 p-3 text-sm text-red-800 dark:border-red-900/50 dark:bg-red-950/30 dark:text-red-200">{error}</div>}<form onSubmit={submitTransition} className="mt-4 space-y-4">{transition === 'shipped' && <label><span className="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-300">Kargo takip numarası</span><input autoFocus required minLength={4} maxLength={120} value={trackingNumber} onChange={event => setTrackingNumber(event.target.value)} className="min-h-11 w-full rounded-xl border border-gray-200 bg-gray-50 px-3 outline-none focus:ring-2 focus:ring-brand-green dark:border-gray-700 dark:bg-gray-900 dark:text-white" /></label>}<label><span className="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-300">İşlem notu (isteğe bağlı)</span><textarea autoFocus={transition !== 'shipped'} maxLength={1000} rows={4} value={note} onChange={event => setNote(event.target.value)} placeholder="Müşteriye görünür durum geçmişi için kısa bir not..." className="w-full rounded-xl border border-gray-200 bg-gray-50 px-3 py-3 outline-none focus:ring-2 focus:ring-brand-green dark:border-gray-700 dark:bg-gray-900 dark:text-white" /></label><div className="flex gap-3"><button type="button" disabled={Boolean(busyId)} onClick={() => { setTransition(null); setError(''); }} className="min-h-11 flex-1 rounded-xl px-4 text-gray-700 hover:bg-gray-100 disabled:opacity-50 dark:text-gray-200 dark:hover:bg-gray-700">Vazgeç</button><button type="submit" disabled={Boolean(busyId) || (transition === 'shipped' && trackingNumber.trim().length < 4) || (transition === 'cancelled' && selected.paymentStatus !== 'unpaid')} className={`min-h-11 flex-1 rounded-xl px-4 font-semibold text-white disabled:cursor-not-allowed disabled:opacity-50 ${transition === 'cancelled' ? 'bg-red-700 hover:bg-red-800' : 'bg-brand-green hover:bg-green-700'}`}>{busyId ? <span className="flex items-center justify-center gap-2"><Loader2 className="h-4 w-4 animate-spin" aria-hidden="true" /> İşleniyor</span> : 'Onayla'}</button></div></form></section></div>}
