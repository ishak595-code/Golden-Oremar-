...
export function AdminOrders({ setActiveTab }: { setActiveTab?: (tab: string) => void }) {
  ...
  async function submitTransition(event: React.FormEvent) {
    ...
    try {
      ...
      show(transition === 'completed' ? '✓ Sipariş tamamlandı. Satıcı hakedişi korumalı havuzda Süper Yönetici onayını bekliyor.' : `✓ Sipariş durumu ${statusLabel(transition).toLocaleLowerCase('tr-TR')} olarak güncellendi. Müşteri bilgilendirildi.`);
      setTransition(null); await load(true);
    } catch (next) { setError(orderAdminErrorMessage(next,'Durum güncellemesi tamamlanamadı. Lütfen yeniden deneyin.')); }
    finally { setBusyId(''); }
  }
  ...
  async function releaseSettlement() {
    ...
    catch (next) { setError(orderAdminErrorMessage(next,'Hakediş serbest bırakma tamamlanamadı. Lütfen yeniden deneyin.')); }
    finally { setBusyId(''); }
  }

  return <div className="space-y-6">
    <header className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between"><div><h2 className="text-2xl font-bold text-gray-900 dark:text-white">Sipariş ve Hakediş Kontrolü</h2><p className="mt-1 max-w-4xl text-sm text-gray-500">Ödeme, stok, kargo, iade ve satıcı hakedişi birbirinden ayrı doğrulanır. Sipariş tamamlanınca para otomatik olarak satıcıya açılmaz; korumalı havuzdan yalnız Süper Yönetici onayıyla çıkar.</p></div><button type="button" onClick={() => void load()} disabled={loading || Boolean(busyId)} className="min-h-11 rounded-xl border px-4 font-semibold disabled:opacity-50"><RefreshCw className={`mr-2 inline h-4 w-4 ${loading ? 'animate-spin' : ''}`} aria-hidden="true"/>Yenile</button></header>
    ...
    {loading ? <div role="status" className="flex min-h-40 items-center justify-center gap-2 rounded-2xl border bg-white dark:bg-gray-800"><Loader2 className="h-5 w-5 animate-spin text-brand-green" aria-hidden="true"/>Siparişler yükleniyor...</div> : <OrderList orders={filtered} onOpen={setSelected}/>} 
...
function OrderList({orders,onOpen}:{orders:ManagedOrder[];onOpen:(order:ManagedOrder)=>void}){if(!orders.length)return <div className="rounded-2xl border border-dashed bg-gray-50 p-12 text-center dark:border-gray-700 dark:bg-gray-800/50"><div className="mx-auto max-w-md"><ShoppingBag className="mx-auto mb-4 h-16 w-16 text-gray-300 dark:text-gray-600" aria-hidden="true"/><h4 className="font-bold text-gray-900 dark:text-gray-100">Sipariş bulunamadı</h4><p className="mt-2 text-sm text-gray-600 dark:text-gray-300">Filtrelerle eşleşen sipariş yok. Farklı durum filtresi veya arama terimlerini deneyin.</p></div></div>;return <section className="overflow-hidden rounded-2xl border bg-white shadow-sm dark:bg-gray-800" aria-label="Sipariş listesi"><div className="divide-y lg:hidden">{orders.map(order=><article key={order.id} className="p-4"><div className="flex items-start justify-between gap-3"><div><div className="font-bold">{order.orderNumber}</div><div className="mt-1 text-xs text-gray-500">{order.customer}</div></div><button type="button" onClick={()=>onOpen(order)} className="min-h-11 min-w-11 rounded-xl hover:bg-gray-100 dark:hover:bg-gray-700" aria-label={`${order.orderNumber} siparişini incele`}><Eye className="mx-auto h-5 w-5" aria-hidden="true"/></button></div><div className="mt-3 flex flex-wrap gap-2"><span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${statusClass(order.status)}`}>{statusLabel(order.status)}</span><span className="rounded-full bg-gray-100 px-2.5 py-1 text-xs dark:bg-gray-700">{paymentLabel(order.paymentStatus)}</span>{order.settlement?.status==='pending_approval'||order.settlement?.status==='failed'?<span className="rounded-full bg-amber-100 px-2.5 py-1 text-xs font-semibold text-amber-800 dark:bg-amber-950/40 dark:text-amber-200">Havuz onayı</span>:null}</div><div className="mt-4 flex justify-between gap-3"><span className="text-xs text-gray-500">{formatDate(order.date)} · {order.items.length} kalem</span><strong>{formatOrderMoney(order.totalMinor,order.currency)}</strong></div></article>)}</div><div className="hidden overflow-x-auto lg:block"><table className="w-full text-left text-sm"><thead className="bg-gray-50 text-xs uppercase text-gray-500 dark:bg-gray-900/50"><tr><th className="px-5 py-4">Sipariş</th><th className="px-5 py-4">Müşteri</th><th className="px-5 py-4">Ödeme</th><th className="px-5 py-4">Hakediş</th><th className="px-5 py-4">Tutar</th><th className="px-5 py-4 text-right">İşlem</th></tr></thead><tbody className="divide-y">{orders.map(order=><tr key={order.id}><td className="px-5 py-4"><strong>{order.orderNumber}</strong><div className="mt-1 text-xs text-gray-500">{formatDate(order.date,true)} · {statusLabel(order.status)}</div></td><td className="px-5 py-4">{order.customer}<div className="text-xs text-gray-500">{order.customerEmail||'E-posta yok'}</div></td><td className="px-5 py-4">{paymentLabel(order.paymentStatus)}{maskedPaymentLabel(order)?<div className="text-xs text-gray-500">{maskedPaymentLabel(order)}</div>:null}</td><td className="px-5 py-4 text-xs font-semibold">{order.settlement?settlementLabel(order.settlement.status):'Uygulanmıyor'}</td><td className="px-5 py-4 font-semibold">{formatOrderMoney(order.totalMinor,order.currency)}</td><td className="px-5 py-4 text-right"><button type="button" onClick={()=>onOpen(order)} className="min-h-11 min-w-11 rounded-xl hover:bg-gray-100 dark:hover:bg-gray-700" aria-label={`${order.orderNumber} siparişini incele`}><Eye className="mx-auto h-4 w-4" aria-hidden="true"/></button></td></tr>)}</tbody></table></div></section>}
