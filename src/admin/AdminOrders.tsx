...
import { Check, CheckCircle, Clock3, CreditCard, Eye, Gift, Loader2, MapPin, RefreshCw, RotateCcw, Search, ShoppingBag, Truck, X } from 'lucide-react';
import {
...
function giftOccasionLabel(value: string | null) {
  const labels: Record<string, string> = {
    just_because: 'İçimden geldi',
    birthday: 'Doğum günü',
    love: 'Sevgi',
    thank_you: 'Teşekkür',
    celebration: 'Kutlama',
  ...
  return value ? labels[value] || value : 'Özel gün belirtilmedi';
}

function giftStyleLabel(value: string | null) {
  const labels: Record<string, string> = {
    oremar_gold: 'Oremar Altın',
    mountain_warmth: 'Dağ Sıcaklığı',
    minimal_elegance: 'Minimal Zarafet',
  };
  return value ? labels[value] || value : 'Standart sunum';
}
...
export function AdminOrders({ setActiveTab }: { setActiveTab?: (tab: string) => void }) {
  ...
  const filtered = useMemo(() => {
    ...
    return orders.filter(order => {
      ...
      if (!q) return true;
      const giftSearch = order.gift ? `${order.gift.recipientName} ${order.gift.senderName || ''} ${order.gift.cardTitle || ''}` : '';
      const paymentSearch = order.paymentMethod ? `${order.paymentMethod.nickname || ''} ${order.paymentMethod.brand} ${order.paymentMethod.last4}` : '';
      return `${order.orderNumber} ${order.customer} ${order.customerEmail || ''} ${order.trackingNumber || ''} ${giftSearch} ${paymentSearch}`.toLocaleLowerCase('tr-TR').includes(q);
    });
    ...
    <div className="space-y-6">
      <header className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
        <div>
          ...
          <p className="mt-1 max-w-3xl text-sm text-gray-500 dark:text-gray-400">Sipariş durumu serbestçe değiştirilemez. Ödeme doğrulaması, stok tüketimi, kargo takibi, hediye hazırlama talimatları, müşteri bildirimi ve satıcı hakedişi sunucu tarafındaki güvenli durum makinesi tarafından yönetilir.</p>
        </div>
      ...
      {selected && <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/60 p-0 backdrop-blur-sm sm:items-center sm:p-4" onMouseDown={event => { if (event.target === event.currentTarget && !busyId && !transition) setSelected(null); }}><div ref={orderDetailRef} role="dialog" aria-modal="true" aria-labelledby="order-detail-title" aria-describedby="order-detail-summary" tabIndex={-1} className="max-h-[96dvh] w-full max-w-4xl overflow-y-auto rounded-t-3xl bg-white p-5 shadow-2xl outline-none dark:bg-gray-800 sm:rounded-2xl sm:p-6"><div className="flex items-start justify-between gap-4"><div><h3 id="order-detail-title" className="text-xl font-bold text-gray-900 dark:text-white">{selected.orderNumber}</h3><p id="order-detail-summary" className="mt-1 text-sm text-gray-500">{formatOrderDate(selected.date, true)} · {statusLabel(selected.status)}{selected.gift ? ' · Hediye siparişi' : ''}</p></div><button type="button" disabled={Boolean(busyId) || Boolean(transition)} onClick={() => { setTransition(null); setSelected(null); setError(''); }} className="min-h-11 min-w-11 rounded-xl p-2 text-gray-500 hover:bg-gray-100 disabled:opacity-50 dark:hover:bg-gray-700" aria-label="Sipariş detayını kapat"><X className="mx-auto h-5 w-5" aria-hidden="true" /></button></div>
        ...
        {selected.gift && <section className="mt-4 rounded-2xl border border-fuchsia-200 bg-fuchsia-50/70 p-4 dark:border-fuchsia-900/50 dark:bg-fuchsia-950/20" aria-labelledby="gift-fulfillment-title"><div className="flex items-start gap-3"><div className="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-fuchsia-100 text-fuchsia-700 dark:bg-fuchsia-950/50 dark:text-fuchsia-200"><Gift className="h-5 w-5" aria-hidden="true" /></div><div className="min-w-0 flex-1"><h4 id="gift-fulfillment-title" className="font-bold text-fuchsia-950 dark:text-fuchsia-100">Hediye hazırlama talimatı</h4><p className="mt-1 text-sm text-fuchsia-800 dark:text-fuchsia-200">Bu sipariş normal paket olarak gönderilmemeli. Hediye kartı ve fiyat görünürlüğü tercihi uygulanmalıdır.</p></div></div><dl className="mt-4 grid gap-3 text-sm sm:grid-cols-2"><div><dt className="text-xs font-semibold text-gray-500">Alıcı</dt><dd className="mt-1 font-semibold text-gray-900 dark:text-white">{selected.gift.recipientName}</dd></div><div><dt className="text-xs font-semibold text-gray-500">Gönderen</dt><dd className="mt-1 font-semibold text-gray-900 dark:text-white">{selected.gift.senderName || 'İsimsiz gönderim'}</dd></div><div><dt className="text-xs font-semibold text-gray-500">Özel gün</dt><dd className="mt-1 text-gray-900 dark:text-white">{giftOccasionLabel(selected.gift.occasion)}</dd></div><div><dt className="text-xs font-semibold text-gray-500">Kart stili</dt><dd className="mt-1 text-gray-900 dark:text-white">{giftStyleLabel(selected.gift.presentationStyle)}</dd></div><div><dt className="text-xs font-semibold text-gray-500">Kart başlığı</dt><dd className="mt-1 text-gray-900 dark:text-white">{selected.gift.cardTitle || 'Varsayılan başlık'}</dd></div><div><dt className="text-xs font-semibold text-gray-500">Fiyat görünürlüğü</dt><dd className={`mt-1 font-bold ${selected.gift.hidePrice ? 'text-fuchsia-700 dark:text-fuchsia-200' : 'text-gray-900 dark:text-white'}`}>{selected.gift.hidePrice ? 'Fiyatı pakette ve kartta gizle' : 'Fiyat gizleme talebi yok'}</dd></div></dl>{selected.gift.message && <div className="mt-4 rounded-xl bg-white/80 p-4 dark:bg-gray-900/70"><div className="text-xs font-semibold uppercase tracking-wide text-gray-500">Hediye notu</div><p className="mt-2 whitespace-pre-wrap text-sm font-medium text-gray-900 dark:text-white">{selected.gift.message}</p></div>}</section>}
