...
function transitionLabel(status: ManagedOrderStatus) { return ({ pending_payment: 'Ödeme aşamasına geçir', confirmed: 'Siparişi onayla', preparing: 'Hazırlamaya al', shipped: 'Kargoya ver', delivered: 'Teslim olarak işaretle', completed: 'Siparişi tamamla', cancelled: 'Siparişi iptal et' } as Partial<Record<ManagedOrderStatus, string>>)[status] || statusLabel(status); }
...
export function AdminOrders({ setActiveTab }: { setActiveTab?: (tab: string) => void }) {
  ...
  async function submitTransition(event: React.FormEvent) {
    ...
    try {
      ...
      show(transition === 'completed' ? '✓ Sipariş tamamlandı. Satıcı hakedişi korumalı havuzda Super Admin onayını bekliyor.' : `✓ Sipariş durumu ${statusLabel(transition).toLocaleLowerCase('tr-TR')} olarak güncellendi. Müşteri bilgilendirildi.`);
