...
export function AdminOrders({ setActiveTab }: { setActiveTab?: (tab: string) => void }) {
  ...
  return <div className="space-y-6">
    ...
    {toast ? <div role="status" aria-live="polite" className="fixed bottom-4 right-4 z-[80] flex max-w-md items-center gap-2 rounded-xl border border-green-200 bg-green-50 px-5 py-3 text-green-900 shadow-2xl dark:border-green-900/60 dark:bg-green-950/30 dark:text-green-100"><Check className="h-5 w-5 text-green-600 dark:text-green-400" aria-hidden="true"/>{toast}</div> : null}
  </div>;
}
