...
import { Check, CheckCircle, Eye, FileText, Loader2, RefreshCw, RotateCcw, Search, X, XCircle } from 'lucide-react';
import {
...
function formatReturnDate(value: unknown, includeTime = false) {
...
export function AdminReturns() {
  ...
  const returnDetailRef = useAccessibleDialog<HTMLDivElement>(Boolean(selected), () => {
    if (!busy && !action) setSelected(null);
  });
  const returnActionRef = useAccessibleDialog<HTMLDivElement>(Boolean(action && selected), () => {
    if (!busy) {
      setAction(null);
      setError('');
    }
  ...
  const counts = useMemo(() => ({
    ...
    refunded: rows.filter(row => Number(row.succeeded_refund_minor) > 0).length,
  }), [rows]);
