...
export default function HomeSection({searchQuery,setSearchQuery,onProductClick,onAddToCart,onToggleFavorite,favorites,onShare,onGift}:Props){
 ...
 const currencies=useMemo(()=>Array.from(new Set(products.map(product=>catalogCurrency(product.currency)).filter((value):value is string=>Boolean(value)))),[products]);
 const filterCurrency=currencies.length===1?currencies[0]:null;
 const priceFilterAvailable=currencies.length<=1&&Boolean(filterCurrency);
 ...
 const filteredProducts=useMemo(()=>{
  ...
  return[...products].filter((product:any)=>{
   const searchable=[product?.name,product?.category,product?.producerName,product?.origin,...(Array.isArray(product?.tags)?product.tags:[])].filter(value=>typeof value==='string').map(value=>value.toLocaleLowerCase('tr-TR'));
   const matchesSearch=!normalizedQuery||searchable.some(value=>value.includes(normalizedQuery));
   const matchesCategory=!activeFilter||safeText(product?.categorySlug,220)===activeFilter;
   ...
   const matchesPrice=priceRange==='all'||(priceFilterAvailable&&comparablePrice&&((priceRange==='0-250'&&price<=250)||(priceRange==='250-500'&&price>250&&price<=500)||(priceRange==='500-plus'&&price>500)));
   return matchesSearch&&matchesCategory&&matchesOrigin&&matchesPrice;
  }).sort((a:any,b:any)=>{
   if(sortOption==='price-asc'||sortOption==='price-desc'){
    const aPrice=catalogPrice(a?.price);const bPrice=catalogPrice(b?.price);const aCurrency=catalogCurrency(a?.currency);const bCurrency=catalogCurrency(b?.currency);
    if(aPrice===null||aCurrency===null){if(bPrice===null||bCurrency===null)return 0;return 1;}
 ...
 const isSellable=(product:any)=>{
  ...
  if(product?.stockMode==='tracked'||product?.stockMode==='seasonal'){const stock=catalogInteger(product?.stock);return stock!==null&&stock>0;}
  return true;
 };
 const spotlightProduct=products.find(product=>product.is_featured&&isSellable(product))||products.find(isSellable)||null;
 ...
 return<>
  <section className="mx-auto max-w-7xl px-4 pt-5 sm:px-6" aria-labelledby="home-catalog-title">
   ...
   {!searchQuery?<div className="mb-8 flex gap-4 overflow-x-auto pb-3 pt-1 snap-x" role="region" aria-label="Hızlı kategoriler">
    {quickCategories.map(category=>{
      ...
      return<button type="button" key={category.id} onClick={()=>chooseCategory(category.id)} aria-pressed={activeFilter===category.id} className={`min-w-[15rem] snap-start rounded-3xl border bg-white p-5 text-left shadow-sm transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-gold dark:bg-gray-900 ${activeFilter===category.id?'border-brand-gold ring-1 ring-brand-gold/30':'border-gray-100 dark:border-gray-800'}`}>
       <div className="flex items-start justify-between gap-3"><div className="grid h-12 w-12 place-items-center rounded-2xl bg-brand-gold/10 text-brand-gold"><Star aria-hidden="true" className="h-6 w-6"/></div>{images.length?<div className="flex -space-x-2">{images.map((product,index)=><img key={`${product.id}:${index}`} src={product.image} alt="" aria-hidden="true" loading="lazy" decoding="async" className="h-10 w-10 rounded-full border-2 border-white object-cover dark:border-gray-900"/>)}</div>:null}</div>
       <div className="mt-5 text-lg font-bold text-brand-green dark:text-brand-gold">{category.name}</div><p className="mt-1 line-clamp-2 text-sm text-gray-500">{category.description||`${category.productCount} canlı ürün`}</p>
      </button>;
    })}
   </div>:null}
   ...
   <div className="mb-8 rounded-3xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900 sm:p-5">
    <div className="flex flex-wrap items-center justify-between gap-3"><div><div className="flex items-center gap-2 font-bold"><SlidersHorizontal aria-hidden="true" className="h-5 w-5 text-brand-gold"/>Katalog filtreleri</div><p className="mt-1 text-sm text-gray-500">Kategori, köken ve doğrulanmış fiyatla canlı sonuçları daraltın.</p></div><button type="button" onClick={()=>setFiltersOpen(value=>!value)} aria-expanded={filtersOpen} aria-controls="home-filter-controls" className="min-h-11 rounded-xl border px-4 font-semibold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-gold"><Filter aria-hidden="true" className="mr-2 inline h-4 w-4"/>{filtersOpen?'Filtreleri gizle':'Filtrele'}</button></div>
  ...
  {!searchQuery&&!activeFilter?<section className="mx-auto max-w-7xl space-y-12 px-4 sm:px-6">
   ...
   {homeSections.filter((section:any)=>section.active!==false).map((section:any)=>{const items=sectionProducts(safeText(section.id,80));if(!items.length)return null;const sectionId=safeText(section.id,80);const seasonal=sectionId==='seasonal';const offers=sectionId==='offers';const preorder=sectionId==='pre_order';return<section key={sectionId} className={`rounded-3xl ${seasonal?'border border-green-100 bg-green-50/70 p-6 dark:border-green-900/40 dark:bg-green-950/10':offers?'border border-red-100 bg-red-50/60 p-6 dark:border-red-900/40 dark:bg-red-950/10':'py-2'}`} aria-labelledby={`home-section-${sectionId}`}><div className="mb-5 flex items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-xl bg-brand-gold/10 text-brand-gold">{offers?<Flame aria-hidden="true" className="h-5 w-5"/>:preorder?<Calendar aria-hidden="true" className="h-5 w-5"/>:seasonal?<Sun aria-hidden="true" className="h-5 w-5"/>:<Star aria-hidden="true" className="h-5 w-5"/>}</span><h2 id={`home-section-${sectionId}`} className="text-2xl font-bold text-brand-green dark:text-brand-gold">{safeText(section.title,160)||'Katalog seçkisi'}</h2></div><div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">{items.slice(0,8).map((product:any)=><CatalogProductCard key={product.id} product={product} onClick={()=>onProductClick(product)} onAddToCart={onAddToCart} onToggleFavorite={()=>onToggleFavorite(product)} isFavorite={favorite(product)} onShare={()=>onShare(product)} onGift={()=>onGift(product)}/>)}</div></section>;})}
  ...
  <section id="products" className="mx-auto mb-24 mt-10 max-w-7xl scroll-mt-28 px-4 sm:px-6" aria-labelledby="products-heading">
   <div className="mb-6 flex flex-wrap items-end justify-between gap-4"><div><div className="text-xs font-bold uppercase tracking-[0.16em] text-brand-gold">Canlı katalog</div><h2 id="products-heading" className="mt-1 text-2xl font-bold text-brand-green dark:text-brand-gold">{searchQuery?`“${searchQuery.trim().slice(0,160)}” sonuçları`:activeCategoryName||'Tüm ürünler'}</h2></div>{searchQuery?<button type="button" onClick={()=>setSearchQuery('')} className="min-h-11 rounded-xl border px-4 font-semibold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-gold">Aramayı temizle</button>:null}</div>
   {!filteredProducts.length?<div className="rounded-3xl border border-dashed p-10 text-center"><Search aria-hidden="true" className="mx-auto h-10 w-10 text-gray-300"/><p className="mt-3 font-semibold text-gray-600 dark:text-gray-300">Seçtiğiniz ölçütlere uyan canlı ürün bulunamadı.</p><button type="button" onClick={()=>{setSearchQuery('');clearFilters();}} className="mt-4 min-h-11 rounded-xl bg-brand-green px-5 font-bold text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-gold">Tüm ürünleri göster</button></div>:<div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{filteredProducts.map(product=><CatalogProductCard key={product.id} product={product} onClick={()=>onProductClick(product)} onAddToCart={onAddToCart} onToggleFavorite={()=>onToggleFavorite(product)} isFavorite={favorite(product)} onShare={()=>onShare(product)} onGift={()=>onGift(product)}/>)}</div>}
  </section>
 </>;
}
