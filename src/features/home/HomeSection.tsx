...
export default function HomeSection({searchQuery,setSearchQuery,onProductClick,onAddToCart,onToggleFavorite,favorites,onShare,onGift}:Props){
 ...
 return<>
  <section className="mx-auto max-w-7xl px-4 pt-4 sm:px-6" aria-labelledby="home-title">
   ...
   {!searchQuery?<div className="mb-5 px-1"><div className="flex items-start justify-between gap-4"><div className="min-w-0 max-w-3xl"><div className="inline-flex items-center gap-1.5 text-[11px] font-black uppercase tracking-[0.16em] text-brand-gold"><Leaf aria-hidden="true" className="h-3.5 w-3.5"/>Golden Oremar</div><h1 id="home-title" className="mt-1 text-2xl font-black leading-tight text-brand-green dark:text-brand-gold sm:text-3xl">{safeText(interfaceContent.heroTitle,300)}</h1><p className="mt-2 line-clamp-2 max-w-2xl text-sm leading-5 text-brand-muted">{safeText(interfaceContent.heroSubtitle,600)}</p></div><button type="button" onClick={()=>document.getElementById('products')?.scrollIntoView({behavior:prefersReducedMotion()?'auto':'smooth'})} className="hidden min-h-11 shrink-0 items-center rounded-full border border-brand-border bg-brand-card px-4 text-sm font-bold text-brand-green focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-gold sm:inline-flex">{safeText(interfaceContent.heroButtonText,120)}<ChevronRight aria-hidden="true" className="ml-1 h-4 w-4"/></button></div></div>:<h1 id="home-title" className="sr-only">Golden Oremar ürünleri</h1>}

   {renderEvents('after_hero')}

   {!searchQuery&&quickCategories.length?<section className="mb-6" aria-labelledby="home-categories-title"><div className="mb-2 flex items-center justify-between gap-3"><h2 id="home-categories-title" className="text-lg font-black text-brand-text">{safeText(interfaceContent.categoriesTitle,200)||'Kategoriler'}</h2><span className="text-xs font-semibold text-brand-muted">Kaydırarak keşfedin</span></div><div className="hide-scrollbar flex snap-x gap-2 overflow-x-auto pb-1" role="list">{quickCategories.map(category=><button type="button" role="listitem" key={category.id} onClick={()=>chooseCategory(category.id)} aria-pressed={activeCategory===category.id} className={`min-h-11 shrink-0 snap-start rounded-full border px-4 text-sm font-bold transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-gold ${activeCategory===category.id?'border-brand-green bg-brand-green text-brand-on-green':'border-brand-border bg-brand-card text-brand-text'}`}>{category.name}</button>)}</div></section>:null}
