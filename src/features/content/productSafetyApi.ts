...
export async function getProductSafety(reference:string,locale='tr'){
 ...
 const{data,error}=await supabase.rpc('get_public_product_safety_v1',{p_reference:normalizedReference,p_locale:normalizeLocale(locale)});if(error)throw error;if(data==null)return{};if(!record(data))throw new Error('Ürün sağlık bilgileri doğrulanamadı.');
