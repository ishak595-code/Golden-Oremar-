...
function normalizePublicEvents(value:unknown):PublicEventsResult{
  if(!isRecord(value)||!Array.isArray(value.items)||value.items.length>200)throw new Error('Etkinlik listesi şu anda yüklenemedi. Lütfen tekrar deneyin.');
...
export async function listMyEventReservations(limit=30):Promise<MyEventReservation[]>{if(!Number.isSafeInteger(limit)||limit<1||limit>100)throw new Error('Etkinlik kayıt liste sınırı geçersiz.');const{data,error}=await supabase.rpc('list_my_event_reservations_v1',{p_limit:limit});const rows=unwrap<unknown>(data,error);if(!Array.isArray(rows)||rows.length>limit)throw new Error('Etkinlik kayıtlarınız şu anda yüklenemedi. Lütfen tekrar deneyin.');return rows.map(normalizeReservation);}
