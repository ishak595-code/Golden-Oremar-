        ...
        <section className="rounded-3xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-gray-900" aria-labelledby="contact-form-title">
          ...
          <form onSubmit={submit} aria-busy={busy} className="mt-5 space-y-4">
            <div className="grid gap-4 sm:grid-cols-2">
              <label className="block"><span className="text-sm font-semibold">Ad Soyad</span><input name="name" required minLength={2} maxLength={120} disabled={busy} autoComplete="name" enterKeyHint="done" value={name} onChange={e => setName(e.target.value)} className="mt-1 min-h-11 w-full rounded-xl border bg-transparent px-3 disabled:opacity-60" /></label>
              <label className="block"><span className="text-sm font-semibold">E-posta</span><input name="email" required maxLength={254} disabled={busy} type="email" inputMode="email" autoComplete="email" value={email} onChange={e => setEmail(e.target.value)} className="mt-1 min-h-11 w-full rounded-xl border bg-transparent px-3 disabled:opacity-60" /></label>
            </div>
            <label className="block"><span className="text-sm font-semibold">Telefon (opsiyonel)</span><input name="phone" maxLength={40} disabled={busy} inputMode="tel" autoComplete="tel" value={phone} onChange={e => setPhone(e.target.value)} className="mt-1 min-h-11 w-full rounded-xl border bg-transparent px-3 disabled:opacity-60" /><span className="mt-1 block text-xs text-gray-500">Yazılırsa 7 ile 20 rakam içermelidir.</span></label>
            <label className="block"><span className="text-sm font-semibold">Konu</span><input name="subject" required minLength={2} value={subject} maxLength={160} disabled={busy} onChange={e => setSubject(e.target.value)} className="mt-1 min-h-11 w-full rounded-xl border bg-transparent px-3 disabled:opacity-60" /><span className="mt-1 block text-xs text-gray-500">{subject.length}/160</span></label>
            <label className="block"><span className="text-sm font-semibold">Mesaj</span><textarea name="message" required minLength={10} value={message} maxLength={5000} disabled={busy} onChange={e => setMessage(e.target.value)} rows={6} spellCheck={true} autoCapitalize="sentences" autoCorrect="on" className="mt-1 w-full rounded-xl border bg-transparent p-3 disabled:opacity-60" /><span className="mt-1 block text-xs text-gray-500">{message.length}/5000 • en az 10 karakter</span></label>
            ...
            <button disabled={busy} aria-busy={busy} className="min-h-12 w-full rounded-xl bg-brand-green px-4 font-bold text-white disabled:opacity-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-gold"><Send aria-hidden="true" className="mr-2 inline h-5 w-5" />{busy ? 'Gönderiliyor…' : 'Mesajı Gönder'}</button>
          </form>
        </section>
      </div>
    </main>
