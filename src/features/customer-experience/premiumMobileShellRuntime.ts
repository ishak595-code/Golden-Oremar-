...
export function installPremiumMobileShellRuntime(){
 ...
 const onScroll=()=>{if(ticking)return;ticking=true;requestAnimationFrame(()=>{const y=Math.max(0,window.scrollY);const delta=y-lastY;
  ...
  else if(delta>5){if(direction!=='down'){direction='down';directionAnchorY=lastY;}if(!compact&&y>COMPACT_ENTER_Y)apply(true);}
  else if(delta<-7){if(direction!=='up'){direction='up';directionAnchorY=lastY;}if(compact&&directionAnchorY-y>=EXPAND_REVERSE_TRAVEL)apply(false);}
