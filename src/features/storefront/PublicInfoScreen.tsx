  ...
  return (
    <main className="mx-auto max-w-4xl px-4 py-6 sm:py-8" aria-labelledby="public-info-title">
      <div className="mb-5 flex items-start gap-3">
        {onBack ? <button type="button" onClick={onBack} className="min-h-11 rounded-xl border px-4 font-semibold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-gold" aria-label="Önceki ekrana dön"><ArrowLeft aria-hidden="true" className="mr-2 inline h-4 w-4" />Geri</button> : null}
      ...
      <nav aria-label="Bilgilendirme sayfaları" className="mb-5 flex gap-2 overflow-x-auto pb-1">
        {(Object.keys(labels) as PageKey[]).map(key => {
          ...
          return <button key={key} type="button" disabled={!available} aria-current={key === activePage ? 'page' : undefined} aria-label={!available ? `${labels[key].label}, henüz yayınlanmadı` : labels[key].label} onClick={() => selectPage(key)} className={`min-h-11 shrink-0 rounded-xl border px-3 text-sm font-semibold disabled:cursor-not-allowed disabled:opacity-45 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-gold ${key === activePage ? 'border-brand-gold bg-brand-gold/10 text-brand-gold' : ''}`}><Icon aria-hidden="true" className="mr-2 inline h-4 w-4" />{labels[key].label}</button>;
        })}
      </nav>
      ...
      {!loading && !error && !item ? <div role="status" className="rounded-2xl border border-amber-200 bg-amber-50 p-5 text-amber-900 dark:border-amber-900/60 dark:bg-amber-950/30 dark:text-amber-100"><h2 className="font-bold">Bu metin henüz yayınlanmadı</h2><p className="mt-2 text-sm">Golden Oremar doğrulanmamış veya eksik bir hukuk metnini kullanıcıya nihai metin gibi göstermiyor.</p></div> : null}
      {!loading && !error && item ? <article className="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-gray-900 sm:p-7">{source ? <SafePublishedBody source={source} /> : <div role="status" className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-amber-900 dark:border-amber-900/60 dark:bg-amber-950/30 dark:text-amber-100">Yayın kaydının içerik gövdesi doğrulanamadı.</div>}<p className="mt-6 border-t pt-4 text-xs text-gray-500">Yayın kaydı: {publicationDate || 'Tarih doğrulanamadı'}</p></article> : null}
    </main>
  );
}
