...
export default function StaffMfaGate({factorEnrolled,onVerified,onLogout}:Props){
 ...
 return<div className="min-h-screen bg-brand-main px-4 py-8 text-brand-text sm:px-6">
  <main className="mx-auto max-w-xl rounded-3xl border border-brand-border bg-brand-card p-5 shadow-xl sm:p-7" aria-labelledby="staff-mfa-title" aria-describedby="staff-mfa-intro">
   ...
   {error?<div id="staff-mfa-error" role="alert" aria-live="assertive" aria-atomic="true" className="mt-5 rounded-2xl border border-red-200 bg-red-50 p-4 text-sm font-semibold text-red-800 dark:border-red-900/60 dark:bg-red-950/30 dark:text-red-200">{error}</div>:null}
   {success?<div id="staff-mfa-success" role="status" aria-live="polite" aria-atomic="true" className="mt-5 rounded-2xl border border-green-200 bg-green-50 p-4 text-sm font-semibold text-green-800 dark:border-green-900/60 dark:bg-green-950/30 dark:text-green-200">İkinci faktör doğrulandı. Güvenli yönetim oturumu hazırlanıyor…</div>:null}

   {mode==='loading'?<div role="status" aria-live="polite" className="mt-6 flex min-h-32 items-center justify-center gap-2 text-sm font-semibold text-brand-muted"><Loader2 aria-hidden="true" className="h-5 w-5 animate-spin"/>Authenticator durumu doğrulanıyor…</div>:null}

   {mode==='enroll-start'?<section className="mt-6 space-y-4" aria-labelledby="staff-mfa-enroll-title"><div className="rounded-2xl border border-brand-border p-4"><h2 id="staff-mfa-enroll-title" className="font-black">Authenticator uygulaması gerekli</h2><p className="mt-2 text-sm leading-6 text-brand-muted">Google Authenticator, 1Password, Authy veya cihazınızdaki uyumlu TOTP uygulaması kullanılabilir. Kurulum tamamlanmadan yönetim paneli açılmaz.</p></div><button type="button" onClick={()=>void startEnrollment()} disabled={busy} className="min-h-12 w-full rounded-xl bg-brand-green px-4 font-black text-brand-on-green outline-none focus-visible:ring-2 focus-visible:ring-brand-green focus-visible:ring-offset-2 disabled:opacity-50">{busy?'Kurulum hazırlanıyor…':'Güvenli kurulumu başlat'}</button></section>:null}

   {mode==='enroll'&&enrollment?<section className="mt-6 space-y-4" aria-labelledby="staff-mfa-qr-title"><div className="rounded-2xl border border-brand-border bg-white p-4 dark:bg-gray-950"><h2 id="staff-mfa-qr-title" className="sr-only">Authenticator QR kurulumu</h2><img src={enrollment.qrCode} alt="Golden Oremar yönetim authenticator kurulum QR kodu" className="mx-auto h-auto w-full max-w-[280px]"/></div><div className="rounded-2xl border border-brand-border p-4"><div id="staff-mfa-secret-label" className="text-xs font-black uppercase tracking-[0.12em] text-brand-muted">Elle kurulum anahtarı</div><code aria-labelledby="staff-mfa-secret-label" className="mt-2 block break-all rounded-xl bg-black/5 p-3 text-sm font-bold dark:bg-white/5">{enrollment.secret}</code><p className="mt-2 text-xs leading-5 text-brand-muted">Bu anahtarı kimseyle paylaşmayın. Golden Oremar bu değeri uygulama veritabanında ayrıca saklamaz.</p></div><CodeForm code={code} setCode={setCode} busy={busy} onVerify={verify} label="Kurulumu doğrula" hasError={Boolean(error)}/><button type="button" onClick={()=>void cancelEnrollment()} disabled={busy} className="min-h-11 w-full rounded-xl border border-brand-border px-4 font-bold outline-none focus-visible:ring-2 focus-visible:ring-brand-green focus-visible:ring-offset-2 disabled:opacity-50">Kurulumu iptal et</button></section>:null}
...
function CodeForm({code,setCode,busy,onVerify,label,hasError}:{code:string;setCode:(value:string)=>void;busy:boolean;onVerify:()=>Promise<void>;label:string;hasError:boolean}){return<form onSubmit={event=>{event.preventDefault();void onVerify();}} className="space-y-3"><label className="block"><span className="mb-1 block text-sm font-bold">6 haneli doğrulama kodu</span><input required autoFocus type="text" inputMode="numeric" enterKeyHint="done" autoComplete="one-time-code" autoCapitalize="none" spellCheck={false} pattern="[0-9]{6}" minLength={6} maxLength={6} value={code} onChange={event=>setCode(event.target.value.replace(/\D/g,'').slice(0,6))} disabled={busy} className="min-h-12 w-full rounded-xl border border-brand-border bg-transparent px-4 text-center text-xl font-black tracking-[0.3em] outline-none focus-visible:ring-2 focus-visible:ring-brand-green focus-visible:ring-offset-2" aria-label="Authenticator doğrulama kodu" aria-describedby={`staff-mfa-code-help${hasError?' staff-mfa-error':''}`} aria-invalid={hasError||undefined} aria-errormessage={hasError?'staff-mfa-error':undefined}/></label><p id="staff-mfa-code-help" className="text-xs leading-5 text-brand-muted">Kod yalnızca rakamlardan oluşur ve 6 hanedir. Kodunuzu veya kurulum anahtarınızı kimseyle paylaşmayın.</p><button type="submit" disabled={busy||!/^[0-9]{6}$/.test(code)} className="min-h-12 w-full rounded-xl bg-brand-green px-4 font-black text-brand-on-green outline-none focus-visible:ring-2 focus-visible:ring-brand-green focus-visible:ring-offset-2 disabled:opacity-50">{busy?'Doğrulanıyor…':label}</button></form>;}
