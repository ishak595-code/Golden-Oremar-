...
function recoveryErrorMessage(raw: string) {
  ...
  if (value.includes('session') || value.includes('jwt')) return 'Şifre sıfırlama bağlantısının oturumu geçersiz veya süresi dolmuş. Yeni bir bağlantı isteyin.';
...
export default function PasswordRecoveryScreen({
  ...
  async function submit(event: React.FormEvent) {
    ...
    if (password.length < 8 || password.length > 72) {
      reportError('Şifre 8-72 karakter arasında olmalıdır.');
      return;
    }
    ...
    <main className="mx-auto flex min-h-[70vh] max-w-lg items-center p-4 sm:p-6" aria-labelledby="password-recovery-title">
      <section className="w-full rounded-3xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-gray-900 sm:p-7">
        <div className="text-center">
          ...
          <p className="mt-2 text-sm text-gray-500">Hesabınız için yeni bir şifre oluşturun. Bu ekran yalnız geçerli şifre sıfırlama bağlantısından sonra açılır.</p>
        </div>

        <form onSubmit={submit} className="mt-6 space-y-4" noValidate>
          {error ? (
            <div ref={errorRef} tabIndex={-1} role="alert" className="rounded-xl border border-red-200 bg-red-50 p-3 text-sm text-red-800 dark:border-red-900 dark:bg-red-950/40 dark:text-red-200">
              {error}
            </div>
          ) : null}
          ...
          <label className="block">
            ...
            <div className="relative mt-1">
              <input
                ...
                className="min-h-12 w-full rounded-xl border bg-transparent px-3 pr-12"
              />
              <button
                type="button"
                onClick={() => setShowPassword(value => !value)}
                aria-label={showPassword ? 'Şifreyi gizle' : 'Şifreyi göster'}
                className="absolute right-1 top-1/2 min-h-11 min-w-11 -translate-y-1/2 rounded-lg p-2 text-gray-500"
              >
                {showPassword ? <EyeOff className="mx-auto h-5 w-5" /> : <Eye className="mx-auto h-5 w-5" />}
              </button>
            </div>
          ...
          <button type="submit" disabled={busy} className="min-h-12 w-full rounded-xl bg-brand-green px-4 font-bold text-white disabled:opacity-50">
            {busy ? 'Şifre güncelleniyor…' : 'Şifremi Güncelle'}
          </button>
          <button type="button" disabled={busy} onClick={cancelRecovery} className="min-h-11 w-full rounded-xl border px-4 text-sm font-semibold disabled:opacity-50">
            İşlemi İptal Et ve Çıkış Yap
          </button>
        </form>
      </section>
    </main>
