...
export function useDialogA11y(onClose:()=>void){
 ...
 useEffect(()=>{const previous=document.activeElement as HTMLElement|null;const node=dialogRef.current;if(!node)return;const first=node.querySelector<HTMLElement>(focusable);(first||node).focus();
