...
export function Panel({ title, children, description }: { title: string; description?: string; children: React.ReactNode }) {
  ...
  return (
    <section
      ...
      className="rounded-2xl border-2 border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-900 sm:p-6"
    >
      <h2 id={titleId} data-account-panel-heading tabIndex={-1} className="text-xl font-black text-brand-green outline-none dark:text-brand-gold">{title}</h2>
      {description ? <p id={descriptionId} className="mt-1 text-sm leading-relaxed text-gray-500 dark:text-gray-400">{description}</p> : null}
...
export function LoadingState({ label = 'Yükleniyor' }: { label?: string }) {
  return <div role="status" aria-live="polite" aria-atomic="true" className="py-8 text-center font-semibold text-gray-500 dark:text-gray-400">{label}…</div>;
}
...
export function EmptyState({ title, body, action }: { title: string; body: string; action?: React.ReactNode }) {
  return (
    <div className="rounded-2xl border-2 border-dashed border-gray-300 bg-gray-50 p-7 text-center dark:border-gray-700 dark:bg-gray-800/50">
...
export function Money({ minor, currency }: { minor: number; currency?: string | null }) {
  ...
  if (!validAmount || !validCurrency) {
    return <span role="status" className="font-semibold text-red-700 dark:text-red-300">Tutar doğrulanamadı</span>;
  }
