...
function normalizeOverview(value:any):AccountOverview{
 ...
 return{...value,profile:{...profile,id:profileId},summary,addresses:Array.isArray(value.addresses)?value.addresses:[],roles:Array.isArray(value.roles)?value.roles.map((role:any)=>String(role||'').trim()).filter((role:string)=>accountRoles.has(role)):[],recent_orders:Array.isArray(value.recent_orders)?value.recent_orders:[],producer,account_closure:closure}as AccountOverview;
}
...
export default function AccountCenter({
 ...
 const recentOrders=Array.isArray((overview as any).recent_orders)?(overview as any).recent_orders:[];const latestOrder=recentOrders.find((item:any)=>item&&typeof item==='object'&&!Array.isArray(item)&&safeText(item.id,160));
 ...
 return<section aria-labelledby="account-title" className="space-y-7">
  ...
  {latestOrder?<section aria-labelledby="recent-account-activity-title" className="rounded-3xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900 sm:p-5"><div className="flex items-start justify-between gap-3"><div><div className="text-xs font-bold uppercase tracking-wide text-brand-gold">Son hareket</div><h2 id="recent-account-activity-title" className="mt-1 text-lg font-bold">Son siparişiniz</h2></div><Package aria-hidden="true" className="h-5 w-5 text-brand-green"/></div><div className="mt-3 rounded-2xl bg-gray-50 p-4 dark:bg-gray-800"><div className="font-bold">{safeText(latestOrder.order_number,120)||'Sipariş numarası doğrulanamadı'}</div><div className="mt-1 text-sm text-gray-500">{orderStatusLabel(latestOrder.status)} • {safeDateLabel(latestOrder.placed_at||latestOrder.created_at)}</div><button type="button" onClick={()=>{setOrderDetailId(safeText(latestOrder.id,160));setView('orders');}} className="mt-3 min-h-11 w-full rounded-xl border border-brand-green/30 bg-white px-4 font-semibold text-brand-green focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-gold dark:bg-gray-900">Sipariş detayını aç</button></div></section>:null}
