...
export default function OrdersPanel({initialOrderId}:{initialOrderId?:string|null}){
 ...
 async function open(id:string){const normalized=id.trim();if(!normalized||openingId)return;try{setOpeningId(normalized);setQuestionContext(null);setError('');setListStatus('');setDetailError('');setDetailStatus('');setDetail(await getOrderDetail(normalized));}catch(e:unknown){setError(e instanceof Error&&e.message?e.message:'Sipariş detayı yüklenemedi.');}finally{setOpeningId(null);}}
 ...
 async function confirmCancel(){if(!cancelCandidate||cancelBusy)return;try{setCancelBusy(true);setDetailError('');setDetailStatus('');await cancelOrder(cancelCandidate.id);setCancelCandidate(null);setQuestionContext(null);setDetail(null);await load(true);setListStatus('Sipariş iptal edildi.');}catch(e:unknown){setCancelCandidate(null);setDetailError(e instanceof Error&&e.message?e.message:'Sipariş iptal edilemedi.');}finally{setCancelBusy(false);}}
