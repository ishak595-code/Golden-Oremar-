...
export default function OrdersPanel({initialOrderId}:{initialOrderId?:string|null}){
 ...
 async function load(reset=true){
  ...
  }catch(e:unknown){setError(e instanceof Error&&e.message?e.message:'Siparişler yüklenemedi.');}
 ...
 async function open(id:string){const normalized=id.trim();if(!normalized||openingId)return;try{setOpeningId(normalized);setError('');setListStatus('');setDetailError('');setDetailStatus('');setDetail(await getOrderDetail(normalized));}catch(e:unknown){setError(e instanceof Error&&e.message?e.message:'Sipariş detayı yüklenemedi.');}finally{setOpeningId(null);}}
 async function refreshDetail(id:string){try{setDetailError('');setDetailStatus('');setDetail(await getOrderDetail(id));await load(true);setDetailStatus('Sipariş detayı güncellendi.');}catch(e:unknown){setDetailError(e instanceof Error&&e.message?e.message:'Sipariş detayı yenilenemedi.');}}
 async function confirmCancel(){if(!cancelCandidate||cancelBusy)return;try{setCancelBusy(true);setDetailError('');setDetailStatus('');await cancelOrder(cancelCandidate.id);setCancelCandidate(null);setDetail(null);await load(true);setListStatus('Sipariş iptal edildi.');}catch(e:unknown){setCancelCandidate(null);setDetailError(e instanceof Error&&e.message?e.message:'Sipariş iptal edilemedi.');}finally{setCancelBusy(false);}}
 if(loading)return<LoadingState label="Siparişler yükleniyor"/>;
 if(!page)return<Panel title="Siparişlerim" description="Sipariş, ödeme, kargo, iade ve geri ödeme durumlarını tek yerden izleyin."><ErrorState message={error||'Sipariş listesi doğrulanamadı.'} onRetry={()=>void load(true)}/></Panel>;
 ...
 return<Panel title="Siparişlerim" description="Sipariş, ödeme, kargo, iade ve geri ödeme durumlarını tek yerden izleyin.">
  ...
  {detail?<div role="dialog" aria-modal="true" aria-labelledby="order-detail-title" className="fixed inset-0 z-50 overflow-y-auto bg-black/60 p-4"><div ref={orderDialogRef} tabIndex={-1} className="mx-auto mt-6 max-w-2xl rounded-2xl bg-white p-5 outline-none dark:bg-gray-900">
   <div className="flex items-start justify-between gap-3"><div><h3 id="order-detail-title" className="text-xl font-bold">{detail.orderNumber}</h3><p className="text-sm text-gray-500">{statusText[detail.status]}</p><p className="mt-1 text-xs text-gray-500">Ödeme: {paymentText[detail.paymentStatus]}</p></div><button type="button" onClick={()=>setDetail(null)} aria-label="Sipariş detayını kapat" className="min-h-11 rounded-lg border px-4 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-gold">Kapat</button></div>
