...
export default function OrdersPanel({initialOrderId}:{initialOrderId?:string|null}){
 ...
 async function refreshDetail(id:string){try{setDetailError('');setDetailStatus('');const updated=await getOrderDetail(id);setDetail(updated);await load(true);setDetailStatus('✓ Sipariş detayı güncellendi.');}catch{setDetailError('Sipariş detayını şu anda yenileyemiyoruz. Lütfen yeniden deneyin.');}}
