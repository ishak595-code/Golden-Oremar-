...
function normalizeAccountOverview(value:unknown):AccountOverview{
  if(!isRecord(value)||!isRecord(value.profile)||!isRecord(value.summary)||!Array.isArray(value.roles)||!Array.isArray(value.addresses)||!Array.isArray(value.recent_orders))throw new Error('Hesap özeti şu anda yüklenemedi. Lütfen tekrar deneyin.');
...
function normalizeFavorites(value:unknown):FavoriteItem[]{if(!Array.isArray(value))throw new Error('Favori listesi şu anda yüklenemedi. Lütfen tekrar deneyin.');return value.map(normalizeFavorite);}
...
function normalizeOrderDetail(value:unknown){
  if(!isRecord(value)||!Array.isArray(value.items)||!Array.isArray(value.statusHistory)||!Array.isArray(value.shipments)||!Array.isArray(value.returns)||!Array.isArray(value.refunds)||!Array.isArray(value.payments)||!isRecord(value.shippingAddress))throw new Error('Sipariş detayı şu anda yüklenemedi. Lütfen tekrar deneyin.');
