...
export default function GiftsPanel({ onStartGift }: { onStartGift?: () => void }) {
  ...
  return <Panel title="Hediye Ettiklerim" description="Hediye olarak verdiğiniz gerçek siparişleri, alıcı, ürün ve sipariş durumuyla birlikte izleyin.">
    ...
    {!items.length?<div className="flex min-h-60 flex-col items-center justify-center gap-4 rounded-2xl border-2 border-dashed border-gray-200 py-10 dark:border-gray-800"><div className="grid h-20 w-20 place-items-center rounded-3xl bg-gradient-to-br from-gray-100 to-gray-200 dark:from-gray-800 dark:to-gray-900"><Gift aria-hidden="true" className="h-10 w-10 text-gray-400"/></div><div className="text-center"><div className="text-lg font-bold text-brand-text">Henüz hediye siparişiniz yok</div><div className="mt-2 max-w-sm px-4 text-sm leading-6 text-brand-muted">Ürün veya sepet ekranından Hediye Et seçeneğiyle başlayabilirsiniz.</div></div>{onStartGift?<button type="button" onClick={onStartGift} className="mt-2 inline-flex min-h-11 items-center gap-2 rounded-xl border-2 border-brand-gold bg-brand-gold px-4 font-bold text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-green"><Gift aria-hidden="true" className="h-4 w-4"/>Hediye seç</button>:null}</div>:
