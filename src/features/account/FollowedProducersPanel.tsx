    ...
    <div className="grid gap-4 sm:grid-cols-2">{items.map(p=>{
      ...
      return <article key={id} aria-busy={busy} className="rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-900">
        <div className="flex gap-3">
          {p.logoPath?<img src={catalogPublicUrl(p.logoPath)} alt={`${p.displayName||'Üretici'} mağaza logosu`} loading="lazy" decoding="async" className="h-16 w-16 shrink-0 rounded-full object-cover"/>:<div role="img" aria-label={`${p.displayName||'Üretici'} için mağaza logosu eklenmedi`} className="grid h-16 w-16 shrink-0 place-items-center rounded-full bg-brand-green/10 text-xl font-bold text-brand-green">{String(p.displayName||'?').charAt(0).toUpperCase()}</div>}
        ...
        <div className="mt-4 grid grid-cols-[1fr_auto] gap-2">
          <button type="button" disabled={!onOpenProducer||!p.slug} onClick={()=>onOpenProducer?.(p.slug)} className="min-h-11 rounded-xl bg-brand-green px-3 font-bold text-white disabled:cursor-not-allowed disabled:opacity-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-gold">Mağazayı aç</button>
