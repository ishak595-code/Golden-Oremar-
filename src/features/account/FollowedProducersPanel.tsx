...
export default function FollowedProducersPanel({ onOpenProducer }: { onOpenProducer?: (slug: string) => void }) {
  ...
  async function load(silent=false){
    try{
      ...
      setError('');
      const next=await listFollowedProducers();
  ...
  async function unfollow(producer:any){
    const id=String(producer?.id||'').trim();
    if(!id||busyId)return;
    try{
    ...
    <div className="grid gap-4 sm:grid-cols-2">{items.map((p,index)=>{
      ...
      return <article key={id||`${slug||'producer'}-${index}`} aria-busy={busy} className="rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-900">
        ...
        <div className="mt-4 grid grid-cols-[1fr_auto] gap-2">
          <button type="button" disabled={!onOpenProducer||!slug} onClick={()=>onOpenProducer?.(slug)} className="min-h-11 rounded-xl bg-brand-green px-3 font-bold text-white disabled:cursor-not-allowed disabled:opacity-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-gold">Mağazayı aç</button>
          <button type="button" disabled={busy||!id} onClick={()=>void unfollow(p)} aria-label={`${displayName} üreticisini takip etmeyi bırak`} className="min-h-11 rounded-xl border border-gray-200 px-3 font-semibold disabled:opacity-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-gold dark:border-gray-700">{busy?'Bırakılıyor…':'Takibi bırak'}</button>
        </div>
      </article>;
    })}</div>}
  </Panel>;
