...
export default function SupportPanel({locale='tr',onOpenMessages,onOpenContact}:{locale?:string;onOpenMessages?:()=>void;onOpenContact?:()=>void}){
 ...
 async function load(silent=false){try{if(!silent)setLoading(true);setError('');const next=await getAccountHelpContent(locale);if(!isRecord(next))throw new Error('Yardım içerikleri sunucudan doğrulanamadı.');setData(next);}catch(e:any){setError(e?.message||'Yardım içerikleri yüklenemedi.');if(!silent)setData(null);}finally{if(!silent)setLoading(false);}}
 ...
 return<Panel title="Yardım & Destek" description="SSS, destek kanalları, iade bilgileri, gizlilik, kullanım koşulları ve Golden Oremar bilgilendirmeleri tek merkezde.">
   ...
   <div className="space-y-6">
     ...
     <section aria-labelledby="help-policies-title" className="space-y-3"><div><h2 id="help-policies-title" className="text-lg font-bold">Yasal & Bilgilendirme</h2><p className="mt-1 text-sm text-gray-500">{publishedCount}/4 doğrulanmış yayın kaydı kullanılabilir. Yayınlanmamış metinler uydurulmaz.</p></div>{(Object.keys(meta)as Key[]).map(key=>{const config=meta[key];const item=publishedItem(data?.[key]);const Icon=config.Icon;if(!item)return<div key={key} className="rounded-2xl border border-dashed border-gray-300 bg-gray-50 p-4 dark:border-gray-700 dark:bg-gray-800/50"><span className="flex items-start gap-3"><span aria-hidden="true" className="rounded-xl bg-gray-100 p-2 text-gray-500 dark:bg-gray-800"><Icon className="h-5 w-5"/></span><span><span className="block font-bold">{config.label}</span><span className="mt-1 block text-sm text-gray-500">Doğrulanmış yayın kaydı henüz yok. Uydurma metin gösterilmiyor.</span></span></span></div>;return<button type="button" key={key} onClick={()=>setSelected(key)} className="min-h-20 w-full rounded-2xl border border-gray-200 bg-white p-4 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-gold dark:border-gray-700 dark:bg-gray-900"><span className="flex items-start gap-3"><span aria-hidden="true" className="rounded-xl bg-brand-gold/10 p-2 text-brand-gold"><Icon className="h-5 w-5"/></span><span className="min-w-0 flex-1"><span className="block font-bold">{item.title||config.label}</span><span className="mt-1 block text-sm text-gray-500">{item.summary||config.description}</span><span className="mt-2 block text-xs font-semibold text-brand-green dark:text-brand-gold">Yayınlanmış belgeyi aç</span></span><ChevronRight aria-hidden="true" className="mt-2 h-5 w-5 text-gray-400"/></span></button>;})}</section>
   </div>
 </Panel>;
}
