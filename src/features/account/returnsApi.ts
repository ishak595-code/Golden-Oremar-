...
function normalizeReturnOptions(value:unknown,expectedOrderId:string):OrderReturnOptions{
 if(!isRecord(value)||!Array.isArray(value.items)||!Array.isArray(value.reasonCodes))throw new Error('İade seçenekleri şu anda yüklenemedi. Lütfen tekrar deneyin.');
...
function normalizeReturnDetail(value:unknown,expectedReturnId:string):ReturnDetail{
 if(!isRecord(value)||!Array.isArray(value.items)||!Array.isArray(value.refunds))throw new Error('İade detayı şu anda yüklenemedi. Lütfen tekrar deneyin.');const id=requiredUuid(value.id,'İade talebi');if(id!==expectedReturnId)throw new Error('İade detayı başka kayda ait.');
