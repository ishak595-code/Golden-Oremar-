...
function RecommendationGroup({eyebrow,title,description,items,onOpen,actionLabel,onAction}:{eyebrow:string;title:string;description:string;items:LegacyHomeProduct[];onOpen:(product:LegacyHomeProduct)=>void;actionLabel?:string;onAction?:()=>void}){return<section className="go-product-recommendations__group"><div className="go-product-recommendations__group-head"><div><span>{eyebrow}</span><h3>{title}</h3><p>{description}</p></div>{actionLabel&&onAction?<button type="button" onClick={onAction} aria-label={actionLabel}>{/* On phones the heading already names the category, so the visible label is the short, conventional "Tümünü gör"; the full label returns from the sm breakpoint. The accessible name is always the full sentence. */}<span className="sm:hidden">Tümünü gör</span><span className="hidden sm:inline">{actionLabel}</span><ChevronRight aria-hidden="true"/></button>:null}</div><div className="go-product-recommendations__rail hide-scrollbar">{items.map(product=><RecommendationCard key={product.id} product={product} onOpen={()=>onOpen(product)}/>)}</div></section>;}
