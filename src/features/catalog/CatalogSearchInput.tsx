...
export default function CatalogSearchInput({value,onChange,onSubmit,onVoice,onFocus,onBlur,listening=false,autoFocus=false}:Props){
 ...
 return<form role="search" onSubmit={event=>{event.preventDefault();const query=normalized.trim();if(query)onSubmit(query);}} className="relative w-full">
  <label className="block"><span className="sr-only">Katalogda ara</span><Search aria-hidden="true" className="pointer-events-none absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-gray-400"/><input type="search" autoFocus={autoFocus} value={normalized} onChange={event=>onChange(event.target.value.slice(0,100))} onFocus={onFocus} onBlur={onBlur} placeholder="Ne arıyorsun? Bal, peynir, tereyağı..." aria-label="Katalogda ara" enterKeyHint="search" autoComplete="off" className={`min-h-12 w-full rounded-2xl border border-gray-200 bg-white pl-11 text-base outline-none transition focus:border-brand-gold focus:ring-2 focus:ring-brand-gold/30 dark:border-gray-700 dark:bg-gray-900 ${normalized?'pr-[6.25rem]':'pr-14'}`}/></label>
