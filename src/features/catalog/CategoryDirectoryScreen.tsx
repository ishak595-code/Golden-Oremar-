...
export default function CategoryDirectoryScreen({ onOpenProduct, onAddToCart }: Props) {
  ...
  return (
    <section className="mx-auto max-w-7xl px-4 py-6 sm:px-6 sm:py-8" aria-labelledby="category-directory-title">
        ...
        <div className="flex min-w-max gap-3" role="list">
          <div role="listitem">
            <button
              ...
              className={`min-h-16 min-w-[150px] rounded-2xl border p-3 text-left transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-gold ${selected === null ? 'border-brand-gold bg-brand-gold/10' : 'border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900'}`}
            >
          ...
          {categories.map(category => (
            <div key={category.id} role="listitem">
              <button
                ...
                className={`min-h-16 min-w-[190px] max-w-[230px] rounded-2xl border p-3 text-left transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-gold ${selected === category.slug ? 'border-brand-gold bg-brand-gold/10' : 'border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900'}`}
              >
      ...
      {!productLoading && !productError && !items.length ? (
        <div className="mt-5 rounded-2xl border border-dashed border-gray-300 p-9 text-center dark:border-gray-700">
          <PackageSearch aria-hidden="true" className="mx-auto h-10 w-10 text-gray-300 dark:text-gray-600" />
          <h3 className="mt-3 font-bold text-brand-text">Bu seçimde ürün bulunamadı</h3>
          <p className="mt-1 text-sm text-gray-500">Başka bir kategori seçin veya stok filtresini kapatın.</p>
        </div>
      ) : null}
