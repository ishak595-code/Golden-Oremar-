...
export default function ProducerFinancePanel({ onBack }: { onBack: () => void }) {
  ...
  return <div className="space-y-5">
    <div className="flex items-center justify-between gap-3"><button type="button" onClick={onBack} className="min-h-11 rounded-xl border-2 border-gray-200 bg-white px-4 font-bold shadow-sm transition-all hover:border-gray-300 hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-900 dark:hover:border-gray-600 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-gold" aria-label="Satıcı paneline dön"><ArrowLeft aria-hidden="true" className="mr-2 inline h-4 w-4"/>Geri</button><button type="button" onClick={() => void load(true)} disabled={loadingMore || revealingBank || Boolean(payoutBusy)} className="min-h-11 rounded-xl border-2 border-gray-200 bg-white px-4 font-bold shadow-sm transition-all hover:border-gray-300 hover:bg-gray-50 disabled:cursor-not-allowed disabled:opacity-50 disabled:shadow-none dark:border-gray-700 dark:bg-gray-900 dark:hover:border-gray-600 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-gold" aria-label="Finans bilgilerini yenile"><RefreshCw aria-hidden="true" className="mr-2 inline h-4 w-4"/>Yenile</button></div>
    ...
    <Panel title="Ödeme Geçmişi" description="iyzico pazaryeri aktarımları ve manuel banka çekimleri aynı finans geçmişinde, ancak ayrı kanallar olarak izlenir.">
      {!payouts.length ? <div className="rounded-xl border-2 border-dashed border-gray-200 bg-gray-50 p-6 text-center dark:border-gray-700 dark:bg-gray-900/50"><p className="font-semibold text-gray-600 dark:text-gray-300">Henüz ödeme kaydı yok</p><p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Ödeme talepleri ve aktarım geçmişi burada görünecek</p></div> : <div className="space-y-3">{payouts.map(payout => <article key={payout.id} className="rounded-xl border-2 border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-900"><div className="flex flex-wrap items-center justify-between gap-2"><div><div className="font-black text-brand-green"><Money minor={payout.amount_minor} currency={payout.currency}/></div><div className="mt-1 text-xs font-bold text-gray-500 dark:text-gray-400">{payoutChannelLabel[payout.channel]}</div></div><span className="rounded-full border-2 border-gray-200 bg-gray-100 px-3 py-1 text-xs font-black dark:border-gray-700 dark:bg-gray-800">{payoutLabel[payout.status]}</span></div>{payout.destination_account_holder || payout.destination_iban_masked ? <div className="mt-3 rounded-lg border-2 border-gray-200 bg-gray-50 p-3 text-sm dark:border-gray-700 dark:bg-gray-900/60"><div className="font-bold">Ödeme hedefi</div>{payout.destination_account_holder ? <div className="font-semibold">{payout.destination_account_holder}</div> : null}{payout.destination_iban_masked ? <div className="font-mono font-semibold">{payout.destination_iban_masked}</div> : null}</div> : null}<div className="mt-2 text-sm font-semibold text-gray-500 dark:text-gray-400">Oluşturulma: {formatDate(payout.created_at)}</div>{payout.requested_at ? <div className="text-sm font-semibold text-gray-500 dark:text-gray-400">Talep: {formatDate(payout.requested_at)}</div> : null}{payout.provider_settlement_released_at ? <div className="text-sm font-semibold text-gray-500 dark:text-gray-400">Sağlayıcıya serbest bırakıldı: {formatDate(payout.provider_settlement_released_at)}</div> : null}{payout.processed_at ? <div className="text-sm font-semibold text-gray-500 dark:text-gray-400">Sonuçlandı: {formatDate(payout.processed_at)}</div> : null}{payout.provider_reference ? <div className="mt-1 break-all text-sm font-semibold text-gray-500 dark:text-gray-400">Transfer referansı: {payout.provider_reference}</div> : null}{payout.note ? <div className={`mt-2 text-sm font-semibold leading-relaxed ${payout.status === 'failed' ? 'text-red-700 dark:text-red-300' : 'text-gray-600 dark:text-gray-300'}`}>{payout.note}</div> : null}{payout.channel === 'manual_bank_transfer' && payout.status === 'requested' && payout.request_source === 'producer' ? <button type="button" disabled={Boolean(payoutBusy)} onClick={() => void cancelWithdrawal(payout)} className="mt-3 min-h-11 rounded-xl border-2 border-red-200 bg-white px-4 font-bold text-red-700 transition-all hover:border-red-300 hover:bg-red-50 disabled:cursor-not-allowed disabled:opacity-50 dark:border-red-900/60 dark:bg-gray-900 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-gold"><XCircle aria-hidden="true" className="mr-2 inline h-4 w-4"/>{payoutBusy === payout.id ? 'İptal ediliyor...' : 'Bekleyen talebi iptal et'}</button> : null}</article>)}
      {loadMoreError ? <div role="alert" className="rounded-xl border-2 border-amber-200 bg-amber-50 p-3 text-sm font-semibold leading-relaxed text-amber-900 dark:border-amber-900/60 dark:bg-amber-950/30 dark:text-amber-100"><p>⚠️ {loadMoreError}</p><button type="button" disabled={loadingMore} onClick={() => void loadMore()} className="mt-2 min-h-11 rounded-xl border-2 border-amber-700 bg-white px-4 font-bold text-amber-900 transition-all hover:bg-amber-50 disabled:cursor-not-allowed disabled:opacity-50 dark:border-amber-900/60 dark:bg-amber-950/30 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-gold">Tekrar dene</button></div> : null}
      {hasMore ? <button type="button" onClick={() => void loadMore()} disabled={loadingMore} className="min-h-11 w-full rounded-xl border-2 border-brand-green bg-white px-4 font-bold text-brand-green shadow-sm transition-all hover:bg-brand-green/5 disabled:cursor-not-allowed disabled:opacity-50 disabled:shadow-none dark:border-brand-green/60 dark:bg-gray-900 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-gold">{loadingMore ? 'Daha eski ödemeler yükleniyor...' : 'Daha eski ödemeleri göster'}</button> : null}<div className="sr-only" aria-live="polite">{loadingMore ? 'Daha eski ödeme kayıtları yükleniyor.' : loadMoreError || `${payouts.length} ödeme kaydı gösteriliyor.`}</div></div>}
    </Panel>
...
function Metric({ label, children }: { label: string; children: React.ReactNode }) { return <div className="rounded-xl border-2 border-gray-200 bg-gray-50 p-3 shadow-sm dark:border-gray-700 dark:bg-gray-800"><div className="text-xs font-bold text-gray-500 dark:text-gray-400">{label}</div><div className="mt-1 font-black text-brand-green">{children}</div></div>; }
function BankField({label,value,mono=false}:{label:string;value:string;mono?:boolean}) { return <div><dt className="text-xs font-black uppercase tracking-wider text-gray-500 dark:text-gray-400">{label}</dt><dd className={`mt-1 break-words font-black ${mono ? 'font-mono tracking-wide' : ''}`}>{value}</dd></div>; }
