...
export default function ProducerTraceabilityPanel({ onBack, onChanged }: { onBack: () => void; onChanged?: () => Promise<void> | void }) {
  ...
  const products = useMemo(() => {
    ...
    for (const row of dashboard?.inventory || []) {
      if (!map.has(row.productId)) map.set(row.productId, { id: row.productId, slug: row.productSlug, name: row.productName, variants: [] });
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between gap-3">
        <button onClick={onBack} className="min-h-11 rounded-xl border px-4 font-semibold" aria-label="Satıcı paneline dön"><ArrowLeft className="mr-2 inline h-4 w-4" />Geri</button>
        <button onClick={() => void load()} className="min-h-11 rounded-xl border px-4 font-semibold" aria-label="Lot listesini yenile"><RefreshCw className="mr-2 inline h-4 w-4" />Yenile</button>
      </div>

      <Panel title="Lot & İzlenebilirlik" description="Ürünün hasat/üretim partisini kaydedin. Trace kodu yalnız admin doğrulamasından sonra müşteriye açılır.">
        {!products.length ? <div role="status" className="mb-4 rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900">Lot oluşturmak için önce satıcı hesabınızda en az bir ürün varyantı bulunmalıdır.</div> : null}
        <button onClick={() => setEditing({ mode: 'new' })} disabled={!products.length} className="min-h-11 rounded-xl bg-brand-green px-4 font-bold text-white disabled:opacity-50"><Plus className="mr-2 inline h-4 w-4" />Yeni lot oluştur</button>
        {!batches.length ? <p className="mt-4 text-sm text-gray-500">Henüz lot kaydı yok.</p> : <div className="mt-4 space-y-3">{batches.map((batch: any) => (
          <button key={batch.id} onClick={() => setEditing({ mode: 'existing', id: batch.id })} className="min-h-16 w-full rounded-2xl border p-4 text-left">
      ...
      <div className="rounded-2xl border border-brand-green/20 bg-brand-green/5 p-4 text-sm"><div className="flex gap-3"><ShieldCheck className="mt-0.5 h-5 w-5 shrink-0 text-brand-green" /><p>QR/trace kodu taslak veya incelemedeki lotlarda müşteriye gösterilmez. Yayın kararı admin doğrulamasından sonra verilir; uygulama sahte lot ya da QR üretmez.</p></div></div>
    </div>
  );
}
...
function BatchEditor({ initialBatchId, products, onBack, onChanged }: { initialBatchId: string | null; products: any[]; onBack: () => void; onChanged: () => Promise<void> | void }) {
  ...
  function hydrate(data: any) {
    ...
    setCountry(data.origin?.countryCode || 'TR');
  ...
  function validate() {
    ...
    if (batchCode.trim().length < 2) return 'Lot kodu en az 2 karakter olmalıdır.';
    if (!harvestDate && !productionDate) return 'Hasat tarihi veya üretim tarihi zorunludur.';
    if (!country || province.trim().length < 2 || district.trim().length < 2 || village.trim().length < 2) return 'Menşe ülke, il, ilçe ve köy bilgilerini tamamlayın.';
    if (method.trim().length < 3) return 'Üretim yöntemini açıklayın.';
    if ((quantity && !unit.trim()) || (!quantity && unit.trim())) return 'Miktar ve birim birlikte girilmelidir.';
    if (quantity && Number(quantity) <= 0) return 'Başlangıç miktarı sıfırdan büyük olmalıdır.';
    return '';
  }
  ...
  async function save() {
    ...
    try {
      ...
      const id = result?.id || result?.batchId || result?.batch_id || batch?.id;
  ...
  async function submitReview() {
    if (!batch?.id) return;
    if (!(batch.events || []).some((event: any) => event.visibility === 'public')) { setError('Admin incelemesine göndermeden önce en az bir public izlenebilirlik olayı ekleyin.'); return; }
  ...
  return <div className="space-y-5">
    <div className="flex items-center justify-between gap-3"><button onClick={onBack} className="min-h-11 rounded-xl border px-4 font-semibold"><ArrowLeft className="mr-2 inline h-4 w-4" />Lot listesine dön</button>{batch?.status ? <span className="rounded-full bg-gray-100 px-3 py-1 text-sm font-semibold dark:bg-gray-800">{statusLabel[batch.status] || batch.status}</span> : null}</div>
    ...
    {message ? <div role="status" className="rounded-xl bg-green-50 p-3 text-sm text-green-800">{message}</div> : null}

    <Panel title={batch ? `Lot: ${batch.batchCode}` : 'Yeni Lot'} description="Ürün, menşe ve üretim bilgilerini gerçek partiye göre girin.">
      <fieldset disabled={!editable || busy} className="space-y-4 disabled:opacity-70"><legend className="sr-only">Lot bilgileri</legend>
        ...
        <Field label="Lot kodu"><input value={batchCode} onChange={e => setBatchCode(e.target.value)} maxLength={80} className="min-h-11 w-full rounded-xl border bg-transparent px-3" /></Field>
        ...
        <fieldset className="grid gap-3 sm:grid-cols-2"><legend className="col-span-full font-bold">Menşe / Köy</legend><Field label="Ülke kodu"><input value={country} onChange={e => setCountry(e.target.value.toUpperCase())} maxLength={2} className="min-h-11 w-full rounded-xl border bg-transparent px-3" /></Field><Field label="İl"><input value={province} onChange={e => setProvince(e.target.value)} className="min-h-11 w-full rounded-xl border bg-transparent px-3" /></Field><Field label="İlçe"><input value={district} onChange={e => setDistrict(e.target.value)} className="min-h-11 w-full rounded-xl border bg-transparent px-3" /></Field><Field label="Köy / mezra"><input value={village} onChange={e => setVillage(e.target.value)} className="min-h-11 w-full rounded-xl border bg-transparent px-3" /></Field></fieldset>
        <Field label="Üretim yöntemi"><textarea value={method} onChange={e => setMethod(e.target.value)} rows={3} maxLength={1000} className="w-full rounded-xl border bg-transparent p-3" /></Field>
        <div className="grid gap-3 sm:grid-cols-2"><Field label="Başlangıç miktarı"><input inputMode="decimal" value={quantity} onChange={e => setQuantity(e.target.value)} className="min-h-11 w-full rounded-xl border bg-transparent px-3" /></Field><Field label="Birim"><input value={unit} onChange={e => setUnit(e.target.value)} placeholder="kg, adet, litre…" className="min-h-11 w-full rounded-xl border bg-transparent px-3" /></Field></div>
        <Field label="Müşteriye açık lot notu"><textarea value={notes} onChange={e => setNotes(e.target.value)} rows={3} maxLength={2000} className="w-full rounded-xl border bg-transparent p-3" /></Field>
    ...
    {batch?.status === 'released' && batch.traceCode ? <Panel title="Yayınlanmış Trace Kodu" description="Bu kod yalnız yayınlanmış lot için müşteriye açılır."><div className="flex flex-wrap items-center gap-3"><code className="rounded-xl bg-gray-100 px-3 py-2 dark:bg-gray-800">{batch.traceCode}</code><button onClick={() => void copyTrace()} className="min-h-11 rounded-xl border px-4 font-semibold"><ClipboardCopy className="mr-2 inline h-4 w-4" />Kopyala</button></div></Panel> : null}
    {batch?.id && editable ? <button onClick={() => void submitReview()} disabled={busy} className="min-h-12 w-full rounded-xl bg-brand-gold px-4 font-bold text-white disabled:opacity-50">Admin İncelemesine Gönder</button> : null}
    {batch?.reviewReason ? <div role="status" className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900">İnceleme notu: {batch.reviewReason}</div> : null}
  </div>;
}

function EventPanel({ events, disabled, onAdd }: { events: any[]; disabled: boolean; onAdd: (input: any) => Promise<void> }) {
  const [type, setType] = useState('hasat');
  const [at, setAt] = useState(() => new Date().toISOString().slice(0, 16));
  const [location, setLocation] = useState('');
  const [note, setNote] = useState('');
  const [visibility, setVisibility] = useState<'public' | 'private'>('public');
  async function add() { if (type.trim().length < 2) return; await onAdd({ eventType: type, eventAt: new Date(at).toISOString(), locationLabel: location, publicNote: note, visibility }); setNote(''); }
  return <Panel title="İzlenebilirlik Olayları" description="Hasat, üretim, paketleme veya sevkiyat gibi gerçek parti olaylarını kaydedin.">
    <div className="space-y-3">{events.map((event: any) => <div key={event.id} className="rounded-xl border p-3"><div className="flex flex-wrap justify-between gap-2"><strong>{event.eventType}</strong><span className="text-xs">{event.visibility === 'public' ? 'Müşteriye açık' : 'Özel'}</span></div><div className="mt-1 text-sm text-gray-500">{formatDate(event.eventAt)}{event.locationLabel ? ` · ${event.locationLabel}` : ''}</div>{event.publicNote ? <p className="mt-1 text-sm">{event.publicNote}</p> : null}</div>)}</div>
    <fieldset disabled={disabled} className="mt-4 grid gap-3 sm:grid-cols-2 disabled:opacity-60"><legend className="col-span-full font-bold">Yeni olay</legend><Field label="Olay türü"><input value={type} onChange={e => setType(e.target.value)} className="min-h-11 w-full rounded-xl border bg-transparent px-3" /></Field><Field label="Tarih ve saat"><input type="datetime-local" value={at} onChange={e => setAt(e.target.value)} className="min-h-11 w-full rounded-xl border bg-transparent px-3" /></Field><Field label="Konum etiketi"><input value={location} onChange={e => setLocation(e.target.value)} className="min-h-11 w-full rounded-xl border bg-transparent px-3" /></Field><Field label="Görünürlük"><select value={visibility} onChange={e => setVisibility(e.target.value as any)} className="min-h-11 w-full rounded-xl border bg-transparent px-3"><option value="public">Müşteriye açık</option><option value="private">Yalnız yönetim</option></select></Field><div className="sm:col-span-2"><Field label="Olay notu"><textarea value={note} onChange={e => setNote(e.target.value)} rows={2} maxLength={1000} className="w-full rounded-xl border bg-transparent p-3" /></Field></div><button type="button" onClick={() => void add()} className="min-h-11 rounded-xl border px-4 font-semibold sm:col-span-2">Olayı Ekle</button></fieldset>
  </Panel>;
}
...
function friendly(err: any) { const raw = String(err?.message || err || 'İşlem tamamlanamadı.'); if (raw.includes('batch_not_editable')) return 'Bu lot artık düzenlenemez.'; if (raw.includes('batch_not_ready_for_review')) return 'Menşe, üretim yöntemi, tarih ve en az bir müşteriye açık olay tamamlanmalıdır.'; if (raw.includes('batch_code')) return 'Lot kodunu kontrol edin; aynı üreticide tekrar edemez.'; if (raw.includes('certification')) return 'Sertifika bu ürün için geçerli değil veya süresi dolmuş.'; return raw; }
