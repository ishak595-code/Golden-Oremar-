...
function friendlyBlockingReason(reason?: string | null) {
  switch (reason) {
    case 'stock_unverified': return 'Ürünün güncel stok bilgisi alınamadı. Lütfen biraz sonra tekrar deneyin.';
    case 'insufficient_stock': return 'Seçtiğiniz adet için yeterli stok kalmadı.';
    case 'international_shipping_weight_missing': return 'Bu ürün için yurt dışı gönderim bilgileri eksik olduğu için bu ülkeye hediye gönderimi açılamıyor.';
    case 'manual_shipping_quote_required': return 'Bu ülke için kargo fiyatı ayrıca belirlenmelidir.';
    case 'shipping_rate_not_configured': return 'Bu ülke için otomatik kargo ücreti henüz tanımlanmadı.';
    case 'shipping_zone_not_configured': return 'Bu ülkeye gönderim henüz kullanıma açık değil.';
    ...
    case 'payment_method_required': return 'Ödeme için aktif bir kart seçin.';
    ...
    case 'payment_method_provider_mismatch': return 'Seçilen kart şu anda kullanılamıyor. Başka bir kart seçin veya kartı yeniden ekleyin.';
    default: return reason ? 'Hediye siparişi şu anda tamamlanamıyor.' : '';
  }
}
...
export default function GiftOrderFlow({ productReference, onClose, onCreated, onOpenPayments }: Props) {
  ...
  function validate() {
    if (!product || !canonicalProductReference) return 'Ürün bilgisi şu anda kullanılamıyor.';
    if (!variant || !safeText(variantId, 160)) return 'Ürün seçeneği şu anda kullanılamıyor.';
    if (!purchaseReady) return !priceReady ? 'Ürün fiyatı şu anda alınamadığı için hediye siparişi oluşturulamıyor.' : !stockReady ? 'Ürün stok bilgisi şu anda alınamadığı için hediye siparişi oluşturulamıyor.' : soldOut ? 'Seçilen ürün stokta yok.' : 'Seçilen ürün seçeneği satışa açık değil.';
    ...
    if (senderName.trim().length > 120 || /[\u0000-\u001F\u007F]/.test(senderName)) return 'Gönderen adını kontrol edin.';
    ...
    if (!/^[A-Z]{2}$/.test(countryCode)) return 'Ülke kodunu iki harfle girin. Örneğin Türkiye için TR.';
    ...
    if (appliedCoupon && !/^[A-Z0-9_-]{1,64}$/.test(appliedCoupon)) return 'Kupon kodunu kontrol edin.';
    if (livePayments && !selectedPaymentMethod) return 'Ödeme için aktif bir kayıtlı kart seçin veya yeni kart ekleyin.';
    if (!preview) return 'Sipariş toplamı henüz hesaplanmadı.';
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      setSubmitting(true);
      setError('');
      setStatus('Sipariş, stok, kargo ve kampanya son kez kontrol ediliyor.');
      ...
      const friendly = raw.includes('invalid_shipping_country') ? 'Teslimat ülkesini açıkça seçin.'
        ...
        : raw.includes('shipping_not_available') ? 'Seçilen ülkeye gönderim henüz açık değil.'
        ...
        : raw.includes('invalid_shipping_address') || raw.includes('invalid_address') ? 'Teslimat adresini veya telefonunu kontrol edin.'
        : raw.includes('address_limit_exceeded') ? 'Hesabınıza en fazla 20 teslimat adresi kaydedebilirsiniz.'
        : friendlyBlockingReason(code) || 'Hediye siparişi şu anda oluşturulamadı. Lütfen yeniden deneyin.';
      setError(friendly);
    } finally {
      setSubmitting(false);
    }
  ...
  if (created) {
    ...
    return <div className="fixed inset-0 z-[90] grid place-items-center overflow-y-auto bg-black/70 p-4" onMouseDown={event => { if (event.target === event.currentTarget) onClose(); }}>
      <div ref={successDialogRef} role="dialog" aria-modal="true" aria-labelledby="gift-success-title" aria-describedby="gift-success-description" tabIndex={-1} className="w-full max-w-lg rounded-3xl bg-white p-6 text-center text-brand-text outline-none shadow-2xl dark:bg-gray-900">
        ...
        <p className="mt-2 text-sm text-gray-500">Sipariş no: <strong>{safeText(created.orderNumber, 120) || 'Sipariş numarası alınamadı'}</strong></p>
        <p id="gift-success-description" className="mt-4 text-sm leading-6 text-gray-600 dark:text-gray-300">Ürün, teslimat bilgileri ve hazırladığınız hediye kartı siparişle birlikte güvenli biçimde kaydedildi. Ödeme tamamlanmadan sipariş ödendi olarak gösterilmez.</p>
    ...
    <div ref={formDialogRef} role="dialog" aria-modal="true" aria-labelledby="gift-title" aria-describedby="gift-description" tabIndex={-1} className="mx-auto my-2 w-full max-w-4xl rounded-3xl bg-white text-brand-text outline-none shadow-2xl dark:bg-gray-900 sm:my-4">
      ...
      <form onSubmit={submit} className="space-y-6 p-5">
        <section aria-labelledby="gift-product-title" className="rounded-3xl border border-gray-200 p-4 dark:border-gray-800 sm:p-5">
          ...
          {!purchaseReady ? <div role="status" className="mt-3 rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900 dark:border-amber-900/60 dark:bg-amber-950/30 dark:text-amber-100">{!priceReady ? 'Bu ürünün fiyat bilgisi şu anda alınamadı.' : !stockReady ? 'Bu ürünün stok bilgisi şu anda alınamadı.' : soldOut ? 'Seçilen ürün stokta yok.' : 'Seçilen ürün seçeneği satışa açık değil.'}</div> : null}
        </section>
        ...
        <section aria-labelledby="gift-address-title" className="rounded-3xl border border-gray-200 p-4 dark:border-gray-800 sm:p-5">
          ...
          {!validCountry ? <div role="status" className="mt-4 rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900 dark:border-amber-900/60 dark:bg-amber-950/30 dark:text-amber-100">Kargo ve toplamın hesaplanabilmesi için ülke kodunu iki harfle girin. Örneğin Türkiye için TR.</div> : null}
        </section>
        ...
        <section aria-labelledby="gift-payment-title" className="rounded-3xl border border-gray-200 p-4 dark:border-gray-800 sm:p-5">
          ...
          <div className={`mt-3 rounded-xl border p-3 text-sm ${livePayments ? 'border-green-200 bg-green-50 text-green-900 dark:border-green-900/60 dark:bg-green-950/30 dark:text-green-100' : 'border-amber-200 bg-amber-50 text-amber-900 dark:border-amber-900/60 dark:bg-amber-950/30 dark:text-amber-100'}`}>{livePayments ? 'Kartla ödeme kullanıma hazır. Seçtiğiniz kayıtlı kart bu hediye siparişinde kullanılacak.' : 'Kartla ödeme şu anda kullanıma hazır değil.'}</div>
        ...
        <section aria-labelledby="gift-summary-title" className="rounded-3xl border-2 border-gray-200 bg-white p-4 shadow-lg dark:border-gray-700 dark:bg-gray-900 sm:p-5">
          ...
          <div className="mt-4"><label htmlFor="gift-coupon" className="block"><span className="text-sm font-bold text-gray-700 dark:text-gray-300">İndirim kuponu (opsiyonel)</span><div className="mt-1 flex gap-2"><input id="gift-coupon" value={couponInput} onChange={event => setCouponInput(event.target.value.toUpperCase().replace(/[^A-Z0-9_-]/g, '').slice(0, 64))} maxLength={64} placeholder="Örnek: KAMPANYA2024" disabled={submitting} className="input min-w-0 flex-1" aria-describedby="gift-coupon-help"/><button type="button" onClick={() => void applyCoupon()} disabled={submitting || previewBusy || !validCountry || !couponInput.trim()} className="min-h-12 rounded-xl border-2 border-brand-green bg-brand-green px-4 font-bold text-white shadow-sm transition-all hover:bg-green-700 disabled:cursor-not-allowed disabled:opacity-50 disabled:shadow-none focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-gold"><TicketPercent aria-hidden="true" className="mr-2 inline h-4 w-4" />Uygula</button></div><span id="gift-coupon-help" className="mt-1 block text-xs leading-relaxed text-gray-500 dark:text-gray-400">{appliedCoupon?`✓ Kupon uygulandı: ${appliedCoupon}${preview&&preview.discountMinor>0?' — İndiriminiz hesaplandı':' — Geçerli kupon değil'}`:' Hediye kampanya kodunuz varsa buraya yazın.'}</span></label></div>
          ...
          <button type="submit" disabled={submitting || previewBusy || !purchaseReady || !preview?.canCheckout || !validCountry || (livePayments && !selectedPaymentMethod)} className="mt-5 min-h-12 w-full rounded-xl border-2 border-brand-green bg-brand-green px-4 font-bold text-white shadow-lg transition-all hover:bg-green-700 disabled:cursor-not-allowed disabled:opacity-50 disabled:shadow-none focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-gold">{submitting ? 'Hediye siparişi oluşturuluyor…' : 'Hediye Siparişini Oluştur'}</button>
        </section>
      </form>
    </div>
