      ...
      try {
        ...
        if (draft) {
          ...
          setForm(previous => ({
            ...
            activityTypes: Array.isArray(valueFrom(draft, 'activity_types')) ? valueFrom(draft, 'activity_types') : [],
            ...
            productCategories: Array.isArray(valueFrom(draft, 'product_categories')) ? valueFrom(draft, 'product_categories') : [],
            legalName: valueFrom(draft, 'legal_name') || '', identifier: valueFrom(draft, 'identifier') || '',
            taxOffice: valueFrom(draft, 'tax_office') || '', mersisNumber: valueFrom(draft, 'mersis_number') || '',
            taxExemptionNumber: valueFrom(draft, 'tax_exemption_number') || '', iban: valueFrom(draft, 'iban') || '',
            phone: valueFrom(draft, 'phone') || '', contactEmail: valueFrom(draft, 'contact_email') || currentUser?.email || '',
            ...
            fulfillmentMethods: Array.isArray(valueFrom(draft, 'fulfillment_methods')) ? valueFrom(draft, 'fulfillment_methods') : ['cargo'],
            averageDispatchDays: Number(valueFrom(draft, 'average_dispatch_days') || 2),
            coldChainCapable: !!valueFrom(draft, 'cold_chain_capable'),
            plannedProducts: Array.isArray(valueFrom(draft, 'planned_products')) && valueFrom(draft, 'planned_products').length ? valueFrom(draft, 'planned_products') : previous.plannedProducts,
        ...
        } else setForm(previous => ({ ...previous, contactEmail: currentUser?.email || '' }));
      } catch (err: any) {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      listProductionLocationSuggestions(form.countryCode, form.province, form.district)
        .then(rows => { if (active) setLocationSuggestions(Array.isArray(rows) ? rows : []); })
  ...
  function validateStep(index: number) {
    ...
    if (index === 2) {
      ...
      for (const item of form.plannedProducts) {
        if (item.name.trim().length < 2 || item.name.trim().length > 120 || !form.productCategories.includes(item.category) || !sourceModelKeys.has(item.source_model) || item.unit.trim().length < 1 || item.unit.trim().length > 30 || Number(item.estimated_quantity) <= 0) return 'Her ürün kendi onay kapsamınızdaki kategoriye bağlı olmalı ve size, ailenize veya kooperatifinize ait gerçek üretim olmalıdır.';
      }
    }
    if (index === 3) {
