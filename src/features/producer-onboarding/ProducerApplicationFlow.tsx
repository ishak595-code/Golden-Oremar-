...
import { ArrowLeft, CheckCircle2, FileCheck2, Loader2, Plus, Trash2 } from 'lucide-react';
import {
...
const initialForm = {
  ...
  countryCode: 'TR', province: '', district: '', village: '', villageIsCustom: false,
  ...
  foodComplianceStatus: '', foodRegistrationNumber: '',
  fulfillmentMethods: [] as string[], averageDispatchDays: 0, coldChainCapable: false,
  plannedProducts: [] as ProducerPlannedProduct[],
  organicClaimStatus: '', organicCertifierName: '', organicCertificateNumber: '', organicCertificateExpiresOn: '',
...
export default function ProducerApplicationFlow({ currentUser, onBack }: { currentUser: any; onBack?: () => void }) {
  ...
  if (loading) return <div role="status" className="mx-auto max-w-4xl p-8 text-center text-sm font-semibold text-gray-500">Satıcı başvurusu yükleniyor...</div>;
  if (success || ['submitted', 'under_review', 'approved'].includes(form.status)) return <main className="mx-auto max-w-3xl p-4 py-8"><div className="rounded-3xl border-2 border-brand-green bg-white p-7 text-center shadow-lg dark:border-brand-green/50 dark:bg-gray-900"><CheckCircle2 className="mx-auto h-16 w-16 text-brand-green"/><h1 className="mt-4 text-2xl font-black">Satıcı Başvurunuz Alındı!</h1><p className="mt-3 text-sm leading-relaxed text-gray-600 dark:text-gray-300"><span className="font-bold">Durum:</span> {success?.status || form.status}</p><div className="mt-4 rounded-xl border-2 border-blue-200 bg-blue-50 p-4 text-left text-sm leading-relaxed text-blue-900 dark:border-blue-900/60 dark:bg-blue-950/30 dark:text-blue-200"><strong>Sonraki Adımlar:</strong><ul className="mt-2 list-disc space-y-1 pl-5"><li>Üretim faaliyeti, köy/menşe, ürün kapsamı ve belgeler doğrulanacak</li><li>Doğrulama tamamlandıktan sonra satıcı hesabınız aktif olacak</li><li>E-posta ile bilgilendirileceksiniz</li></ul></div>{onBack ? <button type="button" onClick={onBack} className="mt-5 min-h-11 rounded-xl border-2 border-brand-green bg-brand-green px-5 font-bold text-white shadow-lg transition-all hover:bg-green-700">Hesabıma Dön</button> : null}</div></main>;
  ...
  return <main className="mx-auto max-w-4xl px-4 py-6 sm:py-8" aria-labelledby="producer-application-title">
    <div className="mb-5 flex items-center gap-3">{onBack ? <button type="button" onClick={onBack} className="min-h-11 rounded-xl border-2 border-gray-200 px-4 font-bold transition-all hover:border-brand-gold/30 hover:bg-brand-gold/5 dark:border-gray-700"><ArrowLeft aria-hidden="true" className="mr-2 inline h-4 w-4"/>Geri</button> : null}<div><h1 id="producer-application-title" className="text-2xl font-black text-brand-green dark:text-brand-gold">Golden Oremar Satıcı Başvurusu</h1><p className="mt-1 text-sm leading-relaxed text-gray-500 dark:text-gray-400">Bu başvuru bağımsız gerçek köy üreticisi içindir. Golden Oremar resmi mağazasının konum veya lojistik bilgileri burada varsayılan olarak kullanılmaz.</p></div></div>
    <ol className="mb-6 grid grid-cols-2 gap-2 sm:grid-cols-3" aria-label="Başvuru adımları">{steps.map((label,index)=><li key={label} aria-current={step===index?'step':undefined} className={`rounded-xl border-2 p-2.5 text-xs font-bold transition-all ${step===index?'border-brand-gold bg-brand-gold/10 text-brand-gold shadow-sm':'border-gray-200 text-gray-600 dark:border-gray-700 dark:text-gray-400'}`}>{index+1}. {label}</li>)}</ol>
    {error ? <div ref={errorRef} tabIndex={-1} role="alert" className="mb-5 rounded-xl border-2 border-red-200 bg-red-50 p-4 font-semibold text-red-800 dark:border-red-900/60 dark:bg-red-950/30 dark:text-red-200">⚠ {error}</div> : null}

    <section className="rounded-3xl border-2 border-gray-200 bg-white p-5 shadow-lg dark:border-gray-700 dark:bg-gray-900 sm:p-7">
      {step===0 ? <div className="space-y-4"><h2 className="text-xl font-black">Üretici kimliği ve faaliyet alanı</h2><div className="rounded-xl border-2 border-brand-green/20 bg-brand-green/5 p-4 text-sm leading-relaxed dark:border-brand-green/30 dark:bg-brand-green/10"><strong>Yetki sınırı:</strong> Onaylandığınızda yalnız kendi üretici hesabınıza bağlı ürünleri ve yönetimin onayladığı kategorileri yönetebilirsiniz. Super Admin araçları bu panelde bulunmaz.</div><Field label="Satıcı türü"><select value={form.sellerClassification} onChange={e=>setForm({...form,sellerClassification:e.target.value})} className="input"><option value="" disabled>Seçin</option>{sellerClasses.map(([value,label])=><option key={value} value={value}>{label}</option>)}</select></Field><fieldset><legend className="font-bold">Hangi üretimleri yapıyorsunuz? *</legend><p className="mt-1 text-sm leading-relaxed text-gray-500 dark:text-gray-400">Gerçekte yaptığınız tüm faaliyetleri seçin. Bu bilgi üretici profilinizde ve yönetim incelemesinde kullanılır.</p><div className="mt-3 grid gap-2 sm:grid-cols-2">{activityTypes.map(([value,label,description])=><label key={value} className="flex min-h-16 items-start gap-3 rounded-xl border-2 border-gray-200 p-3 transition-all hover:border-brand-gold/30 hover:bg-brand-gold/5 dark:border-gray-700"><input type="checkbox" checked={form.activityTypes.includes(value)} onChange={e=>setForm({...form,activityTypes:e.target.checked?[...form.activityTypes,value]:form.activityTypes.filter(item=>item!==value)})} className="mt-1 h-5 w-5"/><span><span className="block font-bold">{label}</span><span className="mt-0.5 block text-xs leading-relaxed text-gray-500 dark:text-gray-400">{description}</span></span></label>)}</div></fieldset><Field label="Halka açık üretici/mağaza adı"><input maxLength={160} value={form.publicName} onChange={e=>setForm({...form,publicName:e.target.value})} className="input" placeholder="Müşterilerin göreceği mağaza adınız"/></Field><Field label="Marka adı (varsa)"><input maxLength={160} value={form.brandName} onChange={e=>setForm({...form,brandName:e.target.value})} className="input" placeholder="Varsa kayıtlı marka adınız"/></Field><Field label="Üretiminizi anlatın"><textarea rows={5} maxLength={4000} value={form.description} onChange={e=>setForm({...form,description:e.target.value})} className="input" placeholder="Nerede, kimlerle, hangi yöntemlerle üretim yaptığınızı ve ürünlerinizi anlatın. En az 40 karakter."/></Field></div> : null}

      {step===1 ? <div className="space-y-4"><h2 className="text-xl font-bold">Köy ve ürün kategori kapsamı</h2><div className="grid gap-3 sm:grid-cols-2"><Field label="Ülke kodu"><input maxLength={2} value={form.countryCode} onChange={e=>setForm({...form,countryCode:e.target.value.toUpperCase().slice(0,2)})} className="input"/></Field><Field label="İl / bölge"><input maxLength={100} value={form.province} onChange={e=>setForm({...form,province:e.target.value})} className="input" placeholder="Gerçek üretim iliniz"/></Field><Field label="İlçe / şehir"><input maxLength={100} value={form.district} onChange={e=>setForm({...form,district:e.target.value})} className="input" placeholder="Gerçek üretim ilçeniz"/></Field><Field label="Köy / mezra"><input maxLength={160} list="golden-oremar-village-suggestions" value={form.village} onChange={e=>{const value=e.target.value;setForm({...form,village:value,villageIsCustom:!locationSuggestions.some(row=>row.village.toLocaleLowerCase('tr-TR')===value.toLocaleLowerCase('tr-TR'))});}} className="input"/><datalist id="golden-oremar-village-suggestions">{locationSuggestions.map(row=><option key={`${row.province}-${row.district}-${row.village}`} value={row.village}>{row.village} - {row.district}/{row.province}</option>)}</datalist><span className="mt-1 block text-xs text-gray-500">Listede yoksa gerçek köy veya mezra adını manuel yazabilirsiniz.</span></Field></div><label className="flex min-h-11 items-center gap-3 rounded-xl border p-3"><input type="checkbox" checked={form.villageIsCustom} onChange={e=>setForm({...form,villageIsCustom:e.target.checked})} className="h-5 w-5"/><span>Köy listede yoktu, adı manuel girdim</span></label><fieldset><legend className="font-semibold">Üretip satacağınız kategoriler *</legend><p className="mt-1 text-sm text-gray-500">Onaydan sonra ürün paneliniz yalnız bu kapsamla açılır. Yeni kategori gerektiğinde yönetim incelemesi gerekir.</p><div className="mt-2 grid gap-2 sm:grid-cols-2">{categories.map(category=><label key={category.id} className="flex min-h-11 items-center gap-3 rounded-xl border p-3"><input type="checkbox" checked={form.productCategories.includes(category.slug)} onChange={e=>toggleCategory(category.slug,e.target.checked)} className="h-5 w-5"/><span>{category.name}</span></label>)}</div></fieldset></div> : null}

      {step===2 ? <div className="space-y-4"><div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900 dark:border-amber-900/50 dark:bg-amber-950/30 dark:text-amber-100"><strong>Aracılık yok:</strong> Başka satıcılardan ürün toplayıp kendi ürününüz gibi listelemek bu hesapta izinli değildir. Kaynak yalnız kendi, aile veya onaylı kooperatif üretiminiz olabilir.</div><div className="flex items-center justify-between gap-3"><h2 className="text-xl font-bold">Satacağınız ürünler</h2><button type="button" disabled={form.plannedProducts.length>=30} onClick={()=>setForm({...form,plannedProducts:[...form.plannedProducts,{name:'',category:'',source_model:'',unit:'',estimated_quantity:0}]})} className="min-h-11 rounded-xl border px-3 font-semibold disabled:opacity-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-gold"><Plus aria-hidden="true" className="mr-1 inline h-4 w-4"/>Ürün ekle</button></div>{form.plannedProducts.length===0?<p className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900 dark:border-amber-900/50 dark:bg-amber-950/30 dark:text-amber-100">Kayıtlı ürün planı bulunmuyor. Gerçek üretiminizi kendiniz eklemeden devam edemezsiniz.</p>:null}{form.plannedProducts.map((item,index)=><fieldset key={index} className="rounded-2xl border p-4"><legend className="px-2 font-bold">Ürün {index+1}</legend><div className="grid gap-3 sm:grid-cols-2"><Field label="Ürün adı"><input maxLength={120} value={item.name} onChange={e=>updateProduct(index,'name',e.target.value)} className="input"/></Field><Field label="Onay kapsamındaki kategori"><select value={item.category} onChange={e=>updateProduct(index,'category',e.target.value)} className="input"><option value="">Seçin</option>{selectedCategories.map(category=><option key={category.id} value={category.slug}>{category.name}</option>)}</select></Field><Field label="Üretim kaynağı"><select value={item.source_model} onChange={e=>updateProduct(index,'source_model',e.target.value)} className="input"><option value="" disabled>Seçin</option>{sourceModels.map(([value,label])=><option key={value} value={value}>{label}</option>)}</select></Field><Field label="Birim"><input maxLength={30} value={item.unit} onChange={e=>updateProduct(index,'unit',e.target.value)} className="input" placeholder="kg, litre, adet"/></Field><Field label="Tahmini miktar"><input type="number" min="0.01" max="1000000" step="0.01" value={item.estimated_quantity > 0 ? item.estimated_quantity : ''} onChange={e=>updateProduct(index,'estimated_quantity',e.target.value ? Number(e.target.value) : 0)} className="input" placeholder="Gerçek tahmini miktar"/></Field></div>{form.plannedProducts.length>1?<button type="button" onClick={()=>setForm({...form,plannedProducts:form.plannedProducts.filter((_,i)=>i!==index)})} className="mt-3 min-h-11 rounded-xl border border-red-200 px-3 text-red-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-gold"><Trash2 aria-hidden="true" className="mr-1 inline h-4 w-4"/>Ürünü kaldır</button>:null}</fieldset>)}</div> : null}
      ...
      <div className="mt-6 flex gap-3 border-t-2 border-gray-200 pt-5 dark:border-gray-700">{step>0?<button type="button" disabled={busy} onClick={()=>{setError('');setStep(value=>Math.max(0,value-1));window.scrollTo({top:0,behavior:scrollBehavior()});}} className="min-h-11 flex-1 rounded-xl border-2 border-gray-200 font-bold transition-all hover:border-brand-gold/30 hover:bg-brand-gold/5 disabled:cursor-not-allowed disabled:opacity-50 dark:border-gray-700">Geri</button>:null}{step<5?<button type="button" disabled={busy} onClick={next} className="min-h-11 flex-1 rounded-xl border-2 border-brand-green bg-brand-green font-bold text-brand-on-green shadow-lg transition-all hover:bg-green-700 disabled:cursor-not-allowed disabled:opacity-50 disabled:shadow-none">Devam</button>:<button type="button" disabled={busy} onClick={()=>void submit()} aria-busy={busy} className="min-h-11 flex-1 rounded-xl border-2 border-brand-green bg-brand-green font-bold text-brand-on-green shadow-lg transition-all hover:bg-green-700 disabled:cursor-not-allowed disabled:opacity-50 disabled:shadow-none">{busy?<><Loader2 aria-hidden="true" className="mr-1 inline h-4 w-4 animate-spin"/>Gönderiliyor...</>:'Başvuruyu Gönder'}</button>}</div>
    </section>
  </main>;
}
...
function Commitment({ checked, onChange, title, body }: { checked: boolean; onChange: (checked: boolean) => void; title: string; body: string }) {
  return <label className="flex items-start gap-3 rounded-xl border-2 border-gray-200 p-4 transition-all hover:border-brand-gold/30 hover:bg-brand-gold/5 dark:border-gray-700"><input type="checkbox" checked={checked} onChange={event=>onChange(event.target.checked)} className="mt-1 h-5 w-5"/><span><span className="block font-black">{title}</span><span className="mt-1 block text-sm leading-relaxed text-gray-500 dark:text-gray-400">{body}</span></span></label>;
}
