  ...
  {currentTab==='home'?<header className="sticky top-0 z-40 border-b border-gray-100 bg-white/95 shadow-sm backdrop-blur-xl dark:border-gray-800 dark:bg-gray-900/95" style={{paddingTop:'env(safe-area-inset-top, 0px)'}}>
   ...
   <div className="mx-auto max-w-7xl px-4 sm:px-6">
    <div className="flex min-h-16 items-center gap-3 md:min-h-20">
     ...
     <div className="relative min-w-0 flex-1 md:mx-auto md:max-w-2xl"><Search aria-hidden="true" className="pointer-events-none absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-gray-400"/><input type="search" aria-label="Ürün, üretici, kategori veya köy ara" placeholder="Ürün, üretici, kategori veya köy ara..." value={searchQuery} onChange={event=>setSearchQuery(event.target.value.slice(0,160))} onFocus={()=>setIsSearchFocused(true)} onBlur={()=>window.setTimeout(()=>setIsSearchFocused(false),180)} onKeyDown={event=>{if(event.key==='Enter'&&searchQuery.trim()){event.preventDefault();openSearch(searchQuery);}}} className="h-11 w-full rounded-xl border border-transparent bg-gray-100 pl-11 pr-24 text-sm outline-none transition focus:bg-white focus:ring-2 focus:ring-brand-gold/50 dark:bg-gray-800 dark:focus:bg-gray-800"/>{searchQuery?<button type="button" onMouseDown={event=>event.preventDefault()} onClick={()=>setSearchQuery('')} aria-label="Aramayı temizle" className="absolute right-12 top-1/2 grid min-h-11 min-w-11 -translate-y-1/2 place-items-center rounded-lg text-gray-500 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-gold"><X aria-hidden="true" className="h-5 w-5"/></button>:null}<button type="button" onMouseDown={event=>event.preventDefault()} onClick={triggerVoiceSearch} aria-label="Sesli arama" className="absolute right-1 top-1/2 grid min-h-11 min-w-11 -translate-y-1/2 place-items-center rounded-lg text-gray-500 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-gold"><Mic aria-hidden="true" className="h-5 w-5"/></button></div>
