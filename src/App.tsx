...
function BottomNavButton({icon:Icon,label,active,onClick,badge=0}:{icon:any;label:string;active:boolean;onClick:()=>void;badge?:number}){return<button type="button" onClick={onClick} aria-current={active?'page':undefined} aria-label={badge>0?`${label}, ${badge} öğe`:label} className={`relative flex h-full min-w-0 flex-auto flex-col items-center justify-center gap-1.5 rounded-2xl px-1 transition-all min-[360px]:px-2 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-brand-gold ${active?'bg-brand-gold/10 text-brand-gold shadow-sm':'text-gray-500 hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-gray-800/50'}`}><span className="relative"><Icon aria-hidden="true" className={`transition-transform ${active?'h-7 w-7':'h-6 w-6'}`}/>{badge>0?<span aria-hidden="true" className="absolute -right-2.5 -top-2.5 flex h-5 min-w-5 items-center justify-center rounded-full border-2 border-white bg-red-500 px-1 text-[10px] font-black leading-none text-white shadow-sm dark:border-gray-900">{badge>99?'99':badge}</span>:null}</span><span className={`block max-w-full truncate text-[11px] font-bold leading-tight transition-all ${active?'text-brand-gold':'text-gray-600 dark:text-gray-400'}`}>{label}</span></button>;}
