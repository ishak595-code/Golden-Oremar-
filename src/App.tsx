...
const ProductRecommendations=React.lazy(()=>import('./features/catalog/ProductRecommendations'));
...
function AppContent(){
 ...
 const replaceWithHome=useCallback(()=>{
  const url=tabUrl('home');routeDepthRef.current=0;
 ...
 const navigateToTab=useCallback((tab:Tab)=>{
  if(tab==='admin'&&(!authReady||!adminSession.checked)){pushRoute(tabUrl('admin'),'admin');return;}
  if(tab==='admin'&&!isAdminLoggedIn){showToast('Bu alan için doğrulanmış yönetici yetkisi gerekiyor.');setAccountView('menu');pushRoute(tabUrl('account'),'account');return;}
 ...
 const openProduct=useCallback((reference:string)=>{
  const normalized=String(reference||'').trim();if(!normalized){showToast('Ürün referansı bulunamadı.');return;}
 ...
 const openProducer=useCallback((reference:string)=>{
  const normalized=String(reference||'').trim();if(!normalized){showToast('Üretici referansı bulunamadı.');return;}
 ...
 const openSearch=useCallback((query:string,categorySlug:string|null=null,producerId:string|null=null)=>{
  const normalized=String(query||'').trim().slice(0,100);if(!normalized&&!categorySlug&&!producerId)return;
 ...
 useEffect(()=>{
  const applyLocation=(event:PopStateEvent)=>{
   const route=parsePublicRoute();const next=normalizeInitialTab(route,safeTab(route.tab));const depth=routeDepthFromState(event.state);
   routeDepthRef.current=depth;setRouteDepth(depth);setSelectedProductReference(route.productReference);setSelectedProducerReference(route.producerReference);
   setSearchQuery(route.query);setSearchCategorySlug(route.categorySlug);setSearchProducerId(route.producerId);
   if(next==='account')setAccountView(route.accountView||'menu');if(next==='admin')setAdminView(route.adminView||'dashboard');
 ...
 useEffect(()=>{
  ...
  const canonical=window.location.href;let canonicalElement=document.querySelector<HTMLLinkElement>('link[rel="canonical"]');if(!canonicalElement){canonicalElement=document.createElement('link');canonicalElement.rel='canonical';document.head.appendChild(canonicalElement);}canonicalElement.href=canonical;setMeta('meta[property="og:url"]','property','og:url',canonical);
 ...
 useEffect(()=>{if(restoreSequence===0)return;showToast('✓ İnternet bağlantısı geri geldi! Güncel veriler yükleniyor.');if(currentUser){void fetchCart();void refreshUnreadCount();}},[restoreSequence,currentUser?.id,fetchCart,refreshUnreadCount,showToast]);
 ...
 const renderContent=()=>{
  ...
  if(currentTab==='cart'){if(!authReady)return<RouteLoading label="Sepet oturumunuz doğrulanıyor"/>;if(!currentUser)return<AuthScreen title="Sepetinizi kullanmak için hesabınıza giriş yapın." description="Sepetiniz, stok durumunuz ve siparişiniz hesabınıza güvenli şekilde bağlanır."/>;return<CartCheckoutFlow onBack={goBack} onCartChanged={applyServerCartSnapshot} onOpenAddresses={()=>{setAccountView('addresses');navigateToTab('account');}} onOpenPayments={()=>{setAccountView('payments');navigateToTab('account');}} onOrderCreated={()=>{setCart([]);setCartItemCount(0);setAccountView('orders');navigateToTab('account');}}/>;}
 ...
 return<div className="min-h-screen bg-brand-main pb-28 font-sans text-brand-text">
  ...
  {toast.visible?<div role="status" aria-live="polite" aria-atomic="true" className="fixed left-1/2 z-[130] flex max-w-[90vw] -translate-x-1/2 items-center gap-2 rounded-full border-2 border-brand-green bg-brand-green px-5 py-3 text-sm font-bold text-brand-on-green shadow-2xl" style={{top:'calc(4rem + env(safe-area-inset-top, 0px))'}}><CheckCircle aria-hidden="true" className="h-5 w-5 shrink-0 text-brand-gold"/><span>{toast.message}</span></div>:null}
 </div>;
}

export default function App(){return<AppContent/>;}
function HeaderAction({label,onClick,icon,badge=0}:{label:string;onClick:()=>void;icon:React.ReactNode;badge?:number}){return<button type="button" onClick={onClick} aria-label={label} className="relative grid min-h-11 min-w-11 place-items-center rounded-full text-gray-500 transition-all hover:bg-gray-100 hover:text-brand-gold active:scale-95 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-gold dark:hover:bg-gray-800">{icon}{badge>0?<span aria-hidden="true" className="absolute right-0 top-0 flex h-5 min-w-5 items-center justify-center rounded-full border-2 border-white bg-red-500 px-1 text-[10px] font-black leading-none text-white shadow-sm dark:border-gray-900">{badge>99?'99':badge}</span>:null}</button>;}
function BottomNavButton({icon:Icon,label,active,onClick,badge=0}:{icon:any;label:string;active:boolean;onClick:()=>void;badge?:number}){return<button type="button" onClick={onClick} aria-current={active?'page':undefined} aria-label={badge>0?`${label}, ${badge} öğe`:label} className={`relative flex h-full min-w-[64px] flex-1 flex-col items-center justify-center gap-1.5 rounded-2xl px-2 transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-brand-gold ${active?'bg-brand-gold/10 text-brand-gold shadow-sm':'text-gray-500 hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-gray-800/50'}`}><span className="relative"><Icon aria-hidden="true" className={`transition-transform ${active?'h-7 w-7':'h-6 w-6'}`}/>{badge>0?<span aria-hidden="true" className="absolute -right-2.5 -top-2.5 flex h-5 min-w-5 items-center justify-center rounded-full border-2 border-white bg-red-500 px-1 text-[10px] font-black leading-none text-white shadow-sm dark:border-gray-900">{badge>99?'99':badge}</span>:null}</span><span className={`text-[11px] font-bold leading-tight transition-all ${active?'text-brand-gold':'text-gray-600 dark:text-gray-400'}`}>{label}</span></button>;}
