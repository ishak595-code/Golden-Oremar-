...
expect(densityCss.includes('.go-product-card--compact')&&densityCss.includes('@media(max-width:360px)'),'Dense storefront must explicitly adapt compact cards to narrow phones.');
...
expect(home.includes('go-premium-product-rail--compact'),'Managed home collections must use phone-efficient compact marketplace rails.');
...
expect(card.includes('compact=false')&&card.includes('if(compact)return<article'),'Catalog cards must retain a compact variant.');
...
console.log('Golden Oremar customer UI contract audit passed: Home-only header, top category discovery, live DB-managed compact marketplace sections, real-offer campaign gating, truthful dynamic product states, native-safe detail commerce, focused account settings and refined premium notification sounds are locked in.');
