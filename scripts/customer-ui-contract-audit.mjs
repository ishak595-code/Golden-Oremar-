...
expect(referenceCss.includes('.go-reference-category-card')&&referenceCss.includes('.go-reference-product-rail'),'Approved category and product rail base presentation must exist.');
...
expect(home.includes('data-reference-layout="approved-recording-marketplace"'),'Home must retain the exact approved recording marketplace layout marker.');
expect(home.includes('go-reference-product-rail'),'Managed home collections must use the approved recording visual product rails.');
expect(home.includes('<CatalogProductCard homeRail'),'Primary Home collections must use the dedicated recording card variant.');
...
expect(home.includes('section.items.length>0')||home.includes('section.items.length'),'Managed product rails must derive visibility/count truth from live section items.');
expect(home.includes('showcaseProducts=sortedProducts.slice'),'Bottom collection must be derived from the live sorted catalog rather than a static list.');
...
expect(recordingCss.includes('width: min(24rem,72vw)')&&recordingCss.includes('width: min(22.5rem,78vw)'),'Approved recording rails must keep a partially visible next card for discoverability.');
...
expect(recordingCss.includes('background: rgb(17 36 29 / .97)'),'Home bottom navigation must retain the dark floating recording surface.');
...
expect(card.includes('homeRail=false')&&card.includes('if(homeRail)return<article'),'Home cards must retain the dedicated approved recording variant.');
expect(card.includes('coldChain')&&card.includes('Soğuk Zincir'),'Cold-chain products must retain truthful handling visibility.');
expect(card.includes('seasonal')&&card.includes('Mevsimlik Üretim'),'Seasonal stock mode must remain visibly and truthfully distinct.');
...
console.log('Golden Oremar customer UI contract audit passed: Home-only header, approved recording category/product rails, live notification/cart header, truthful dynamic product states, native-safe detail commerce, focused account settings and refined premium notification sounds are locked in.');
