...
requireMatch(homeProductCard.includes('data-product-id={item.id}')&&homeProductCard.includes('data-product-reference={item.slug}')&&homeProductCard.includes('data-product-link="true"'),'Every Home product row must expose stable identity/reference and explicit link markers.');
...
requireMatch(searchInput.includes('aria-label="Sesli mikrofon"')&&!searchInput.includes('aria-pressed=')&&searchInput.includes('data-listening={listening')&&searchInput.includes('role="status" aria-live="polite"')&&searchInput.includes('Sesli arama dinleniyor.')&&searchInput.includes('Mic')&&!searchInput.includes('MicOff')&&!searchInput.includes('Mikrofon kapalı'),'Voice search must keep the stable Sesli mikrofon control name, avoid toggle-state off narration, and announce listening only through the live status region.');
