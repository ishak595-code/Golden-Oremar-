...
const created=[];
const PNG_1200=Buffer.from('','base64');
...
async function verifyOwnerCatalogMedia(c){const{data:userData,error:userError}=await c.auth.getUser();assert.ifError(userError);const userId=userData.user?.id;assert.ok(userId);const dir=`admin/${userId}/official-products`;const validPath=`${dir}/${randomUUID()}.png`;const fakePath=`${dir}/${randomUUID()}.png`;const uploaded=[];try{let result=await c.storage.from('catalog-public').upload(validPath,PNG_1200,{contentType:'image/png',upsert:false,cacheControl:'60'});assert.ifError(result.error);uploaded.push(validPath);let verification=await invokeCatalogVerifier(c,validPath);assert.equal(verification.response.status,200);assert.equal(verification.body?.ok,true);assert.equal(verification.body?.detectedMime,'image/png');assert.equal(verification.body?.width,1200);assert.equal(verification.body?.height,1200);assert.equal(verification.body?.byteSize,PNG_1200.byteLength);
