...
async function assertAal1StaffDenied(c,expectedRole){const snap=await context(c);assert.ok(Array.isArray(snap.roles)&&snap.roles.includes(expectedRole));assert.equal(snap.staffMfaRequired,true);assert.equal(snap.mfaSatisfied,false);assert.equal(snap.authenticatorAssuranceLevel,'aal1');assert.equal(snap.canAccessAdmin,false);assert.deepEqual(snap.permissions,['mfa.self_manage']);for(const p of ['admin.access','product.moderate','product.publish','product.health_manage','seller.approve','refund.approve','refund.execute','payout.release','role.manage','system.configure','storefront.lifecycle_manage'])assert.equal(await can(c,p),false,`${expectedRole} AAL1 leaked ${p}`);
  ...
  await expectPermissionDenied(c.rpc('super_admin_store_directory_v1',{p_query:null,p_state:'all',p_limit:1,p_offset:0}),'store lifecycle directory');
  return snap;
}
...
async function assertStoreDirectoryAllowed(c){const{data,error}=await c.rpc('super_admin_store_directory_v1',{p_query:null,p_state:'all',p_limit:5,p_offset:0});assert.ifError(error);assert.ok(data&&typeof data==='object'&&!Array.isArray(data));assert.ok(data.summary&&typeof data.summary==='object');assert.ok(Number.isInteger(Number(data.summary.totalStores)));assert.ok(Array.isArray(data.items));for(const store of data.items){assert.match(String(store.storeNumber||''),/^GO-STORE-[0-9]{8}$/);assert.equal(typeof store.productCount==='number'||typeof store.productCount==='string',true);}}
...
try{
  ...
  const admin=await roleScenario('admin','mfa-admin',['admin.access','user.manage','refund.execute'],['product.publish','product.health_manage','role.manage','payout.release','system.configure','security.manage','storefront.lifecycle_manage']);
  await expectPermissionDenied(admin.client.rpc('super_admin_store_directory_v1',{p_query:null,p_state:'all',p_limit:1,p_offset:0}),'admin store lifecycle directory');
  const owner=await roleScenario('super_admin','mfa-super',['admin.access','product.publish','product.health_manage','role.manage','payout.release','system.configure','security.manage','payment.manage','user.erase','product.remove','mfa.self_manage','storefront.lifecycle_manage'],[]);
  assert.ok(owner.snapshot.permissions.length>=78,'Super Admin does not receive the complete active capability set');
  await assertStoreDirectoryAllowed(owner.client);
