...
function permissionDenied(error){return Boolean(error&&(error.code==='42501'||/permission_required|admin_required|super_admin_required|access_denied|not_allowed|aal2_verified_factor_required|mfa_factor_not_owned|staff_mfa_event_required/i.test(String(error.message||''))));}
...
async function moderatorScenario(){
  ...
  await expectNotAuthorizationDenied(a.rpc('admin_review_product_v3',{p_product_id:randomUUID(),p_approve:false,p_reason:'CI authorized moderator probe',p_ownership_checked:false,p_image_checked:false,p_scope_checked:false,p_origin_checked:false}),'moderator rejection endpoint');
  await expectPermissionDenied(a.rpc('admin_review_product_v3',{p_product_id:randomUUID(),p_approve:true,p_reason:'CI moderator owner publish attack',p_ownership_checked:true,p_image_checked:true,p_scope_checked:true,p_origin_checked:true}),'moderator owner publication attack');
  await expectPermissionDenied(a.rpc('super_admin_list_product_health_changes_v1'),'moderator product health owner endpoint');
...
try{
  ...
  const admin=await roleScenario('admin','mfa-admin',['admin.access','user.manage','refund.execute'],['product.publish','product.health_manage','role.manage','payout.release','system.configure','security.manage']);
  await expectPermissionDenied(admin.client.rpc('admin_review_product_v3',{p_product_id:randomUUID(),p_approve:true,p_reason:'CI admin owner publish attack',p_ownership_checked:true,p_image_checked:true,p_scope_checked:true,p_origin_checked:true}),'admin owner publication attack');
  await expectPermissionDenied(admin.client.rpc('super_admin_list_product_health_changes_v1'),'admin product health owner endpoint');
  ...
  assert.ok(owner.snapshot.permissions.length>=79,'Super Admin does not receive the complete active capability set');
  await expectNotAuthorizationDenied(owner.client.rpc('admin_review_product_v3',{p_product_id:randomUUID(),p_approve:true,p_reason:'CI owner publication permission probe',p_ownership_checked:true,p_image_checked:true,p_scope_checked:true,p_origin_checked:true}),'Super Admin product publication endpoint');
  const healthQueue=await owner.client.rpc('super_admin_list_product_health_changes_v1');assert.ifError(healthQueue.error);assert.ok(Array.isArray(healthQueue.data),'Super Admin product health queue did not return an array');
