...
if(finalMigration){
  requireText(finalMigration,"u.created_at < timezone('utc',now()) - interval '30 minutes'",'CI garbage collection must retain the final 30-minute safety buffer.');
  requireText(finalMigration,"coalesce(u.raw_user_meta_data->>'source','')='github-actions-e2e'",'CI garbage collection must require the dedicated github-actions-e2e metadata source.');
  requireText(finalMigration,"coalesce(u.raw_user_meta_data->>'e2e_run_id','') ~ '^[0-9]{1,24}$'",'CI garbage collection must validate the GitHub run id format.');
  requireText(finalMigration,"'^goldenoremar\\\\+ci-e2e-[0-9]{1,24}(-[a-z0-9-]{1,24})?@gmail\\\\.com$'",'CI garbage collection must restrict candidates to the run-and-optional-slot CI email namespace.');
  requireText(finalMigration,"u.raw_user_meta_data->>'e2e_slot'",'CI garbage collection must bind an optional disposable staff slot through metadata.');
  requireText(finalMigration,"'goldenoremar+ci-e2e-'||(u.raw_user_meta_data->>'e2e_run_id')||",'CI garbage collection must derive the exact candidate email from run id metadata.');
  requireText(finalMigration,"then '-'||(u.raw_user_meta_data->>'e2e_slot')",'CI garbage collection must include the exact optional slot in candidate identity binding.');
  ...
  requireText(finalMigration,'delete from auth.users where id=target.id;','CI garbage collection must delete only the exact age-and-identity-qualified Auth user.');
  ...
  requireText(finalMigration,"'golden-oremar-ci-e2e-user-gc'",'CI garbage collection must retain its unique cron job identity.');
  requireText(finalMigration,"'7,22,37,52 * * * *'",'CI garbage collection must run every 15 minutes at the final schedule.');
