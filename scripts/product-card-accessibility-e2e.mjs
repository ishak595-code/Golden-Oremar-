...
async function declaredAccessibleName(card,button){const labelledBy=(await button.getAttribute('aria-labelledby')||'').trim();if(labelledBy){const parts=[];for(const id of labelledBy.split(/\s+/)){const text=(await card.locator(`[id=${JSON.stringify(id)}]`).textContent()||'').trim();if(text)parts.push(text);}if(parts.length)return parts.join(' ');}return(await button.getAttribute('aria-label')||'').trim();}
