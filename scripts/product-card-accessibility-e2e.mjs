...
async function verifyCard(page,{cardSelector,buttonSelector,titleSelector,priceSelector,screen}){
 ...
 const button=card.locator(buttonSelector).first();const label=(await button.getAttribute('aria-label')||'').trim();if(!label)throw new Error(`${screen}_ARIA_LABEL_MISSING`);
 requireIncludes(label,title,`${screen}_ARIA_NAME_MISSING_TITLE`);
 const priceNode=card.locator(priceSelector).first();if(await priceNode.count()){const price=(await priceNode.textContent()||'').trim();if(price)requireIncludes(label,price,`${screen}_ARIA_NAME_MISSING_PRICE`);}
...
console.log('Golden Oremar product-card runtime accessibility passed for Home, Category and Search.');
