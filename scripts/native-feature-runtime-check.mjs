...
const roots={android:'android/app/src/main/assets/public',ios:'ios/App/App/public'};
const relativeRoot=roots[platform];
if(!relativeRoot){console.error('Usage: node scripts/native-feature-runtime-check.mjs <android|ios>');process.exit(2);}
const root=path.resolve(process.cwd(),relativeRoot);
if(!fs.existsSync(root)||!fs.statSync(root).isDirectory()){console.error(`[native-feature-runtime] ${platform} synced asset directory is missing: ${relativeRoot}`);process.exit(1);}
if(!fs.existsSync(path.join(root,'index.html'))){console.error(`[native-feature-runtime] ${platform} synced index.html is missing.`);process.exit(1);}
...
console.log(`Native feature runtime check passed for ${platform}: premium theme/sound, FAQ, product safety, search accessibility, app updates, video-reference storefront, live recommendations and cold-chain card UX are present in the synced native shell assets (${textFiles.length} text assets checked).`);
