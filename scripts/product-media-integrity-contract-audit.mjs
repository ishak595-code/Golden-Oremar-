...
requireMatch(producerApi,/uploadProducerProductImages[\s\S]*await validateProducerProductImageFile\(file\)[\s\S]*uploadDirectMedia\('product-image'/,'Producer upload must run the shared image validator before every upload.');
...
// Paths are generated by the server now (media-upload), always a fresh UUID in
// the store's own folder, and the ledger refuses a second reservation of the
// same key, so an upload can never overwrite an existing file.
const mediaUpload=fs.readFileSync('supabase/functions/media-upload/index.ts','utf8');
const r2Migration=fs.readFileSync('supabase/migrations/20260926180000_r2_single_home_for_public_media_v1.sql','utf8');
requireMatch(mediaUpload,/case "product-image": case "product-video": return `\$\{producerId\}\/products\/\$\{id\}\.\$\{ext\}`/,'Producer uploads must use randomized producer-owned catalog paths.');
requireMatch(mediaUpload,/pathFor\(kind, userId, producerId, contentType, crypto\.randomUUID\(\)\)/,'Every upload path must be a fresh random UUID generated by the server.');
requireMatch(r2Migration,/exception when unique_violation then\s+return 'invalid_name';/,'Product media uploads must remain immutable and non-overwriting.');
requireMatch(producerApi,/if\(uploaded\.length\)await cancelDirectMedia\(uploaded\)/,'Partial upload failures must clean up newly uploaded orphan objects.');
