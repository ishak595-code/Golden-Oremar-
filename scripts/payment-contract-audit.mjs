...
const control=read('src/admin/paymentControlApi.ts'),admin=read('src/admin/AdminPaymentControls.tsx'),commerce=read('src/features/payments/commerceApi.ts'),eventPayment=read('src/features/payments/eventPaymentApi.ts'),hostedDialog=read('src/features/payments/HostedPaymentDialog.tsx'),vault=read('supabase/functions/payment-method-vault/index.ts'),checkout=read('src/features/cart/CartCheckoutFlow.tsx'),hostedInit=read('supabase/functions/iyzico-checkout-form/index.ts'),webhook=read('supabase/functions/iyzico-payment-webhook/index.ts'),shared=read('supabase/functions/_shared/iyzico.ts'),emailWorker=read('supabase/functions/transactional-email-worker/index.ts'),emailAdmin=read('src/admin/transactionalEmailAdminApi.ts'),emailQueueMigration=read('supabase/migrations/20260819162242_add_transactional_email_receipt_queue.sql'),paymentPrivateMigration=read('supabase/migrations/20260819163517_move_payment_privileged_cores_private.sql');
const active=[control,admin,commerce,eventPayment,hostedDialog,vault,checkout,hostedInit,webhook,emailWorker,emailAdmin].join('\n');
...
assert(!/redirect_required/.test(active),'payment runtime must not reintroduce external redirect state');
assert(!/returnUrl|paymentReturnUrl/.test(control+admin),'customer-facing payment return URL controls must not return');
...
assert(commerce.includes("'embedded_required'")&&eventPayment.includes("'embedded_required'")&&hostedInit.includes('state:"embedded_required"'),'order and event checkout must use the in-app embedded state');
assert(hostedDialog.includes('<iframe')&&hostedDialog.includes('IYZICO_HOSTS')&&!hostedDialog.includes('window.open'),'hosted iyzico checkout must remain embedded inside Golden Oremar');
...
assert(emailAdmin.includes('super_admin_list_transactional_email_jobs_v1')&&emailAdmin.includes('super_admin_retry_transactional_email_job_v1'),'receipt email operations must remain Super Admin RPC controlled');
assert(paymentPrivateMigration.includes('security invoker')&&paymentPrivateMigration.includes('private.super_admin_get_payment_control_v1'),'privileged payment cores must stay private behind invoker wrappers');
