...
async function openPublicProductFromHome(page){const search=await openHomeShell(page);const appearance=await page.evaluate(()=>({theme:document.documentElement.getAttribute('data-theme'),background:getComputedStyle(document.documentElement).getPropertyValue('--bg-main').trim()}));if(appearance.theme!=='custom'||!appearance.background)throw new Error(`MANAGED_BRAND_APPEARANCE_NOT_APPLIED:${JSON.stringify(appearance)}`);mark('managed_brand_appearance_runtime',true);const managedSectionVisible=await visible(page.getByText('Golden Oremar seçkisi',{exact:true}).first(),3000);const seasonalVisible=await visible(page.getByText(/Mevsim/i).first(),1500);const campaignVisible=await visible(page.getByText(/Fırsat|Kampanya|Teklif/i).first(),1500);const newVisible=await visible(page.getByText(/Yeni/i).first(),1500);mark('home_dynamic_sections',managedSectionVisible||seasonalVisible||campaignVisible||newVisible);await search.fill(productName);await search.press('Enter');await page.getByText(productName,{exact:true}).first().waitFor({state:'visible',timeout:15000});mark('catalog_search',true);await shot(page,'03-search');await page.getByText(productName,{exact:true}).first().click();await page.getByRole('heading',{name:productName,exact:true}).waitFor({state:'visible',timeout:15000});mark('product_detail',true);if(await visible(page.getByText('Ürün görseli yakında',{exact:true}),1000))report.blockers.push('REAL_PRODUCT_IMAGE_STORAGE_OBJECT_MISSING');}
...
try{
 ...
 if(!oidcToken){
  report.blockers.push('AUTHENTICATED_E2E_EXPLICITLY_DISABLED');await openPublicProductFromHome(page);await page.getByRole('button',{name:'Ürünü paylaş',exact:true}).click();await page.getByText(/Ürün bağlantısı kopyalandı|Paylaşım menüsü açıldı|Ürün bağlantısı panoya kopyalandı|Ürün paylaşımı tamamlandı/).waitFor({state:'visible',timeout:10000});mark('public_product_share',true);await verifyPublicSections(page);if(report.pageErrors.length)throw new Error(`PAGE_ERRORS:${report.pageErrors.join(' | ')}`);console.log('Golden Oremar explicit public-only Chromium E2E passed.');
 }else{
  const accountReady=await provisionAndLogin(page);await shot(page,'01-account-created');await accountReady.click();await page.getByRole('heading',{name:'Profilimi Düzenle'}).waitFor({state:'visible',timeout:12000});const profileName=page.getByLabel('Ad Soyad');await profileName.fill(`Golden Oremar Test Müşterisi ${runId}`);await page.getByRole('button',{name:'Değişiklikleri Kaydet'}).click();await page.getByText('Profil bilgileriniz güncellendi.').waitFor({state:'visible',timeout:12000});mark('profile_update_roundtrip',true);await shot(page,'02-profile');
  await openPublicProductFromHome(page);await page.getByRole('button',{name:'Favorilere ekle'}).click();await page.getByText(/Favorilerinize eklendi\.|Ürün favorilerinize eklendi\./).waitFor({state:'visible',timeout:10000});mark('favorite_add',true);await page.getByRole('button',{name:'Ürünü paylaş',exact:true}).click();await page.getByText(/Ürün bağlantısı kopyalandı|Paylaşım menüsü açıldı|Ürün bağlantısı panoya kopyalandı|Ürün paylaşımı tamamlandı/).waitFor({state:'visible',timeout:10000});mark('share',true);await page.getByRole('button',{name:/Sepete Ekle|Ön Siparişe Ekle/}).click();await page.getByText(/adet ürün sepetinize eklendi/).waitFor({state:'visible',timeout:10000});mark('cart_add',true);await shot(page,'04-product-actions');
  await page.goto(`${baseUrl}/?tab=account&view=favorites`,{waitUntil:'networkidle',timeout:30000});await assertGlobalHomeHeaderHidden(page,'favorites_global_header_hidden');await page.getByRole('heading',{name:'Favorilerim'}).waitFor({state:'visible',timeout:12000});await page.getByText(productName,{exact:true}).first().waitFor({state:'visible',timeout:12000});mark('favorite_roundtrip',true);
  await page.goto(`${baseUrl}/?tab=product-detail&product=${productSlug}`,{waitUntil:'networkidle',timeout:30000});await page.getByRole('heading',{name:productName,exact:true}).waitFor({state:'visible',timeout:12000});await page.getByRole('button',{name:/Hediye olarak gönder|Hediye et/}).click();await page.getByRole('heading',{name:'Bir üründen fazlasını gönderin'}).waitFor({state:'visible',timeout:12000});mark('gift_flow_opens',true);await shot(page,'05-gift');await page.getByRole('button',{name:'Hediye ekranını kapat'}).click();
  const buyNow=page.getByRole('button',{name:/Hemen Satın Al/i});if(!(await visible(buyNow,3000)))throw new Error('BUY_NOW_ACTION_MISSING');await buyNow.click();await page.getByRole('heading',{name:'Sepetim'}).waitFor({state:'visible',timeout:15000});await assertGlobalHomeHeaderHidden(page,'cart_global_header_hidden');mark('buy_now_to_cart',true);await page.getByText(productName,{exact:true}).first().waitFor({state:'visible',timeout:10000});mark('cart_roundtrip',true);
  const paymentUnavailable=page.getByText(/Şu anda tahsilata hazır iyzico yöntemi yok/);if(await visible(paymentUnavailable,8000)){mark('payment_fail_closed_without_provider_secrets',true);report.blockers.push('REAL_IYZICO_PROVIDER_SECRETS_REQUIRED');}else mark('payment_provider_ready',true);const checkoutButton=page.getByRole('button',{name:/Güvenli Ödemeye Geç|Kayıtlı Kartla Öde/});if(await visible(checkoutButton,3000))mark('checkout_action_present',true);await shot(page,'06-cart-checkout');
  await page.goto(`${baseUrl}/?tab=account&view=orders`,{waitUntil:'networkidle',timeout:30000});await assertGlobalHomeHeaderHidden(page,'orders_global_header_hidden');await page.getByRole('heading',{name:'Siparişlerim'}).waitFor({state:'visible',timeout:12000});mark('orders_panel',true);await verifyPublicSections(page);if(report.pageErrors.length)throw new Error(`PAGE_ERRORS:${report.pageErrors.join(' | ')}`);
  await import('./staff-mfa-e2e.mjs');mark('staff_mfa_authorization_e2e',true);
  console.log('Golden Oremar authenticated customer and staff MFA E2E passed with GitHub OIDC-provisioned disposable users.');
 }
