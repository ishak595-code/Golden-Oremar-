...
if(readMigration){
  ...
  const dashboardBody=readMigration.split(/drop function if exists private\.get_my_producer_dashboard_v1\(\)/i)[0];
  forbid(dashboardBody,/private\.get_my_producer_dashboard_v1\s*\(/i,'Canonical dashboard v2 must not delegate to dashboard v1.');
  ...
  const draftBody=readMigration.split(/drop function if exists public\.get_my_producer_application_draft_v2\(uuid\)/i)[0];
  forbid(draftBody,/get_my_producer_application_draft_v[2-4]\s*\(/i,'Canonical application draft v5 must not delegate to retired draft versions.');
...
if(writeMigration){
  ...
  const saveBody=writeMigration.split(/create or replace function private\.submit_producer_application_v4/i)[0];
  forbid(saveBody,/save_producer_application_draft_(?:legacy_v1|v[2-4])\s*\(/i,'Canonical application save v5 must not delegate to retired save versions.');
  const submitBody=(writeMigration.split(/create or replace function private\.submit_producer_application_v4/i)[1]||'').split(/drop function if exists public\.save_producer_application_draft_legacy_v1/i)[0];
  forbid(submitBody,/submit_producer_application_v[1-3]\s*\(/i,'Canonical application submit v4 must not delegate to retired submit versions.');
  ...
  requireMatch(writeMigration,/\/\?tab=admin&adminView=vendor-applications/i,'Producer application notification must target the canonical admin review screen.');
}
