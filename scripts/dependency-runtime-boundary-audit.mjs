...
const workflowPaths=[
  path.join(root,'.github','workflows','mobile-quality.yml'),
  path.join(root,'.github','workflows','consolidation-preflight.yml')
];
...
for(const file of [pkgPath,lockPath,...workflowPaths]){
...
const weakenedProductionAudits=[
...
for(const workflowPath of workflowPaths){
  ...
  const relative=path.relative(root,workflowPath);
  const strictMatches=workflow.split(strictProductionAudit).length-1;
  if(strictMatches!==1) fail(`${relative} must contain exactly one LOW-severity production audit gate; found ${strictMatches}`);
  for(const weakened of weakenedProductionAudits){
    if(workflow.includes(weakened)) fail(`${relative} contains a weakened production audit command: ${weakened}`);
  }
}
...
if(process.exitCode) process.exit(process.exitCode);
console.log(`Dependency runtime boundary audit passed: ${buildOnly.length} build-only packages are isolated from production and both Mobile Quality and Consolidation Preflight pin LOW-severity production advisory gates.`);
