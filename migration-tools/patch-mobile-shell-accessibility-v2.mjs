...
function replaceMany(from, to, expected, label) {
  const count = text.split(from).length - 1;
  if (count !== expected) throw new Error(`${label}: expected exactly ${expected} matches, found ${count}`);
...
console.log('Mobile shell accessibility v2 patch applied.');
